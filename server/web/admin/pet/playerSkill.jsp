					...
					if (career != null) {
						if (career.basicSkills != null) {
							for (int i = 0; i < career.basicSkills.length; i++) {
								...
								if (skill != null) {
									out.print("《基础技能》---"+skill.getName()+"--"+skill.getId()+"--"+p.getCareerBasicSkillsLevels()[i]+"/"+skill.getMaxLevel()+"<br>");
								}
							}
						}
						out.print("<hr>");
						if (career.nuqiSkills != null) {
							for (int i = 0; i < career.nuqiSkills.length; i++) {
								...
								if (skill != null) {
									out.print("《怒气技能》---"+skill.getName()+"--"+skill.getId()+"--"+p.getNuqiSkillsLevels()[i]+"/"+skill.getMaxLevel()+"<br>");
								}
							}
						}
						out.print("<hr>");
							...
							for (int ii = 0; ii < career.threads.length; ii++) {
								...
								if (ct != null) {
									...
									for (int i = 0; i < skills.length; i++) {
										skill = skills[i];
										if (skill != null) {
											out.print("《进阶技能》---"+skill.getName()+"--"+skill.getId()+"--"+p.getSkillOneLevels()[i]+"/"+skill.getMaxLevel()+"<br>");
										}
									}
								}
							}
						...
						if (career.xinfaSkills != null) {
							for (int i = 0; i < career.xinfaSkills.length; i++) {
								...
								if (skill != null) {
									out.print("《心法技能》---"+skill.getName()+"--"+skill.getId()+"--"+p.getXinfaLevels()[i]+"/"+skill.getMaxLevel()+"<br>");
								}
							}
						}
					}
