...
if(o!=null){
	...
	try{
				...
				if (prizes != null && prizes.size() == 1) {
					{
						...
						if (list != null && list.size() > 0) {
							for (PlayerPrize pp : list) {
								hasSend = true;
								CompoundReturn cr = pp.sendMail(true,true);
		...
		ActivitySubSystem.logger.error("[运营奖励异常]",t);
...
<%!/**
	...
	HashMap<String, List<PlayerPrize>> getPrize(QueyrType queyrType,String fileName) throws Throwable{
		...
		for (int i = 1 ; i < maxRow ; i++) {
			...
			//} catch (Exception e) {
			//	playerName = String.valueOf(row.getCell(0).getNumericCellValue());
			//	System.out.println("数字：" + playerName);
			//}
			playerId = 0L;
...
<BR/>
	<%
		if (queyrType == null) {
	%>
		<form name="f1" action="./sendPrizeMail_new.jsp" method="post">
		...
		<br/>
		<form name="f2" action="./sendPrizeMail_new.jsp" method="post">
	...
	%>
			...
			%>
				...
				<%
					for (int i = 0; i < list.size();i++ ) {
						...
						try {
							cr = playerPrize.sendMail(false,QueyrType.singleServer.equals(queyrType));
						} catch (Throwable e) {
							e.printStackTrace();
		...
		<form name="f4" action="./sendPrizeMail_new.jsp" method="post">
...
class PlayerPrize {
	...
	CompoundReturn sendMail (boolean realSend,boolean checkPlayer) throws Throwable{
		...
		if (!realSend) {
			...
			try {
				mailId = DefaultMailManager.getInstance().sendMail(p.getId(),aeList.toArray(new ArticleEntity[0]),numArr,mailTitle,mailContent,0L,0L,0L,"运营发送活动物品",MailSendType.后台发送,p.getName(),ip,recorder);
			} catch (Exception e){
				ActivitySubSystem.logger.error("[运营奖励异常]",e);
				return cr.setBooleanValue(false).setStringValue("[发送邮件失败] [异常:"+e.toString()+"]");
			}
			
			return cr.setBooleanValue(true).setStringValue("[playerId:" + p.getId() + "] [名字:" + playerName + "] [发送成功] [邮件ID:" + mailId + "] <BR/> ["+ss.toString()+"<br/>]");
		}
	}
}
enum OperateType {
