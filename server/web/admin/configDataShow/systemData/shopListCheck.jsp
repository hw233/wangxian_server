...
.color1{background-color:#35ca41;}
.color2{background-color:#35b2fe;}
.color3{background-color:#8b50ed;}
...
<body><div>
	...
	<div style="float:left"><table style="font-size: 12px;" border="1">
		...
		%>
		<tr>
			<td colspan="10" style="color: blue; font-weight: bold;font-size: 16px;text-align: center;"><%=shop.getName()%> [<%=shopType[shop.shopType] %>] (<%=shop.getGoods().size() %>) </td>
		</tr>
		<%
		...
		%>
		<tr style="color: <%=changePrice ? "red" : ""%>;background-color:<%=saling ? "" : "#e0e0e0" %>">
			<td class="color<%=color %>"><a href='ArticleByName.jsp?articleName=<%=g.getArticleName()%>'><%=g.getArticleName()%></a><%=shop.isBuyBinded()? "(绑)":"" %></td>
			<td class="color<%=color %>"><a href='ArticleByName.jsp?articleCNName=<%=g.getArticleName_stat()%>'><%=g.getArticleName_stat()%></a><%=shop.isBuyBinded()? "(绑)":"" %></td>
			<td><%=g.getColor()%></td>
...
<tr style="background-color: #83AAEB;font-weight: bold;"><td>商品名</td><td>问题</td></tr>
<%
