...
int calltrace = Integer.parseInt(request.getParameter("calltrace"));

ObjectCreationTracker oct = ObjectTrackerServiceHelper.getObjectCreationTracker(clazz);
ObjectCreationTracker.CallTrace ct = oct.callTraceList.get(calltrace);
...
<%
		...
		while(s.getTime() < e.getTime() + 3600000){
			...
			long nums0[] = oct.getCallTraceStatNumByeveryMinute(calltrace,day,0);
			long nums1[] = oct.getCallTraceStatNumByeveryMinute(calltrace,day,1);
			long nums2[] = oct.getCallTraceStatNumByeveryMinute(calltrace,day,2);
...
-->
	...
	<body>
		...
		<h2 style="font-weight:bold;">
			<%=startday %> ~ <%= endday %> 类<%=clazz.getName() %>,#<%=calltrace %>调用堆栈跟踪<br>存量：<%= StringUtil.addcommas(ct.currentNumInMem) %>，峰值：<%= StringUtil.addcommas(ct.peakNumInMem) %>，创建总量：<%= StringUtil.addcommas(ct.totalCreatedNum) %>
		</h2>
		<form><input type='hidden' name='cl' value='<%=clazz.getName() %>'><input type='hidden' name='calltrace' value='<%=calltrace %>'>输入开始日期：<input type='text' name='startday' value='<%=startday %>'>结束日期<input type='text' name='endday' value='<%=endday %>'>(格式：2011-01-01) 
		<br/>
		<%
		 	for(int i = 0 ; i < 3 ; i++){
