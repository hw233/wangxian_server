...
<html>
	...
	<body>
		...
		<center>
			<form action="mm.jsp" name=f0 method=post>
				基础配置:当前状态<%=openning?"开启":"关闭" %>
				<%if(openning) {%>
				<input type=button name=sub1 value=' 关 闭 ' onclick="location.replace('mm.jsp?action=close')">
				<%} else {%>
				<input type=button name=sub1 value=' 开 启 ' onclick="location.replace('mm.jsp?action=open')">				
				<%} %>
			...
			<table id="test1" align="center" width="70%" cellpadding="0"
				...
				<tr>
					...
					<td><%=ship.getBottomLevelTotalUp()/1000 %></td>
					<td><%=msm.isForbid(ship)?"<font color='red'><b>封禁</b></font>":"正常" %></td>
					<td><a href="mm_detail.jsp?id=<%=ship.getId() %>">查看详情</a></td>
				</tr>
				<%} %>
			</table><br><br>
			<%} %>
