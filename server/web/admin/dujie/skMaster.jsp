...
%>
<h2>大师技能后台</h2>
<a href='/admin/playersonline.jsp?bean=sprite_sub_system'>在线玩家列表</a>

<form action="skMaster.jsp" method="get"><input type="hidden"
	name="action" value="queryPlayerSk" /> 玩家ID<input type="text"
	name="playerId" value="<%=playerId%>" /> <input type="submit"
	value="查询该玩家大师技能信息" /></form>
<form action="skMaster.jsp" method="get"><input type="hidden"
	name="action" value="queryPlayerSk" /> 玩家名字<input type="text"
			...
			if (pid != null) {
				...
				if (p != null) {
					...
					if (bean != null) {
						...
						Skill.logger.warn("[后台设置玩家大师技能点数] [成功] [" + p.getName() + "] [新：" + Arrays.toString(bean.getLevels()) + "]");
					} else {
	...
	if ("queryPlayerSk".equals(action)) {
		do {
			...
			for (int i = 0; i < 12; i++) {
				...
				out.print("<tr>");
				out.print("<td>");
				out.println(i + "</td><td>" + as.getName() + ":" + as.getBuffName() + "</td><td>" + p.getSkillLevel(as.getId()) + "</td><td>" + bean.getLevels()[i] + " <a href='skMaster.jsp?action=addPoint&playerId=" + playerId + "&index=" + i + "'>加点</a>" + " - <a href='skMaster.jsp?action=showSkDesc&index=" + i + "&playerId=" + playerId + "'>查看技能描述</a>");
				out.print("</td>");
				out.print("<td>");
			...
			for (int i = 0; i < len; i++) {
				...
				out.println("第" + conf.getStartTimes() + " ~ " + conf.getEndTimes() + " 次 : 需经验 " + conf.getNeedExp() + " <a href='skMaster.jsp?action=exchange&playerId=" + playerId + "&times=1'>兑换</a>" + "<br/>");
			}
		} while (false);
		{//调试技能描述。 
