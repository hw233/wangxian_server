...
<%
	...
	if ("op".equals(action)) {
		if ("zengJiaVIP$%^".equals(pwd)) {
			...
			if (rmb != 0) {
				out.print("<HR/>");
				out.print("修改vip经验前:" + player.getLogString() + ",vip:" + player.getVipLevel() + ",vip经验:" + player.getRMB() + "<BR/>");
				player.setRMB(player.getRMB() + rmb);
				out.print("修改vip经验后:" + player.getLogString() + ",vip:" + player.getVipLevel() + ",vip经验:" + player.getRMB() + "<BR/>");
				out.print("<HR/>");
			}
			long chargePoints = Long.valueOf(jifen);
			if (chargePoints != 0) {
				out.print("<HR/>");
				out.print("修改积分前:" + player.getLogString() + ",积分:" + player.getChargePoints() + "<BR/>");
				player.setChargePoints(chargePoints + player.getChargePoints());
				out.print("修改积分后:" + player.getLogString() + ",积分:" + player.getChargePoints() + "<BR/>");
				out.print("<HR/>");
			}
			out.print("提交成功");
			...
			content.append("<table style='font-size: 12px;text-align: center;' border='1'>");
				...
				content.append("<tr>");
					...
					content.append("</td>");
				content.append("</tr>");	
				sendMail("[高级GM工具] [修改用户VIP和积分] [" + session.getAttribute("authorize.username") + "] [" + GameConstants.getInstance().getServerName() + "]", content.toString());
		}
	} else {
		playerName = "";
		vipExp = "0";
...
<h1>不需要修改的项写0</h1>
	<form action="./addVipExp.jsp" method="post">
		...
		修改积分:<input name="jifen" type="text" value="<%=jifen%>"><BR/>
