...
function selectionNameForMap(obj){
	if(obj.value != ""){
		window.location.href="npcsTemp1.jsp?gameName="+obj.value;
	}
}
...
NPC sprites[] = null;
	...
	for(int i = 0 ; i < sprites.length ; i++){
		...
		String zhongqiu = hmm.get(cid);
		if(zhongqiu != null && !zhongqiu.equals("")){
			sprites[i].setFlushFrequency(Long.parseLong("999999999999"));
			sprites[i].setAlive(false);
			npcm.removeNPC(sprites[i]);
		}
	...
	for(int i = 0 ; i < cids.length ; i++){
		...
		for(int j = 0 ; j < al.size() ; j++){
			...
			if(s.getCountry() == 0){
				CountryStr = "中立";
			}else if(s.getCountry() == 1){
				CountryStr = "紫微宫";
			}else if(s.getCountry() == 2){
				CountryStr = "日月盟";
			}else{
				CountryStr = "阵营错误";
			}
			out.println("<td>"+CountryStr+"</td>");
