			...
			if (players.size() > 0) {
				...
				if (allplayerExist) {//玩家都存在
					...
					if (!allArticleExist) {
						...
						List<String> list = ActivityManager.sendMailForActivity(players, activityProps, mailTitle, mailContent, "测试部发送",request);
						if (list != null) {
							//发送邮件
							for (String content : list) {
								if (content != null) {
									//调用发邮件接口
									sendMail("[高级测试功能] [发送道具] [" + session.getAttribute("authorize.username") + "] [" + GameConstants.getInstance().getServerName() + "]", content);
								}
							}
						}
...
<body>
<div>
<form name="F1" action="./ceshisendMailForGroup.jsp?op=send" method="post">
