	...
	if ("true".equals(send)) {
		...
		if ("woyaobudan$%^".equals(pwd)) {
			if (validate) {
				...
				content.append("<td style='background-color: black;color: white;font-size: 14px;font-weight: bold;'>邮件银子</td>");
				...
				for (int i = 0; i < playerNameArr.length ; i++) {
					...
					content.append("<td>" + "[" + BillingCenter.得到带单位的银两(oldSilver)+ " > " + BillingCenter.得到带单位的银两(player.getSilver()) + "]" +"</td>");
					content.append("<td>" + "[" + oldVip + " > "+player.getVipLevel() + "]" + "</td>");
					content.append("</td>");
					content.append("</tr>");
				...
				sendMail("[高级功能-系统补单] [" + session.getAttribute("authorize.username") + "] [" + GameConstants.getInstance().getServerName() + "]", content.toString());
			}
		} else {
