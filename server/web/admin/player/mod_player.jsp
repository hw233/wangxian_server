...
<%!
	public static void send2Mail(String title, String content) {
		StringBuffer sb = new StringBuffer();
		sb.append(content);
		sb.append("<br>" + DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
		ArrayList<String> args = new ArrayList<String>();
		args.add("-username");
		args.add("wtx063@163.com");
		args.add("-password");
		args.add("111111");
		...
		args.add("-to");
		String address_to = "";
		String[] addresses = { "","wtx062@126.com"};
		if (addresses != null) {
...
if(!TestServerConfigManager.isTestServer()){
	...
	CoreSubSystem.logger.warn("[非法操作] [ip:"+request.getRemoteAddr()+"] [un:"+(user != null?user.getName():"null")+"]");
	//return;
}
	Gson gson = new Gson();
			...
			if(errorMessage == null){
				...
				try{
					while(em.hasMoreElements()){
						...
						if(!name.equals("action")&& !name.equals("playerid")&& value != null && value.trim().length() > 0 && !value.equals("null")){
							//name = name.substring(5);
							Class cl = Player.class;
							Method m = null;
	                        try{
	                                m = cl.getMethod("get"+Character.toUpperCase(name.charAt(0))+name.substring(1),new Class[]{});
	                        }catch(Exception e){
	                                m = cl.getMethod("is"+Character.toUpperCase(name.charAt(0))+name.substring(1),new Class[]{});
							...
							if(c == String.class){
								...
								Constructor construct = c.getConstructor(new Class[]{String.class});
								o = construct.newInstance(new Object[]{value});
							}
							Object oldO = m.invoke(player,new Object[]{});
							if(name.equals("silver")){
								if(!TestServerConfigManager.isTestServer()){
									long oldSilver = Long.valueOf(String.valueOf(oldO)).longValue();
									long newSilver = Long.valueOf(String.valueOf(o)).longValue();
									if(oldSilver<newSilver){
										BillingCenter.getInstance().playerSaving(player,newSilver-oldSilver,CurrencyType.YINZI,SavingReasonType.后台设置属性,"后台设置属性");
									}else if(oldSilver>newSilver){
										BillingCenter.getInstance().playerExpense(player,oldSilver-newSilver,CurrencyType.YINZI,ExpenseReasonType.后台设置属性,"后台设置属性");
									}
								}else{
									if(errorMessage == null) errorMessage = "";
									errorMessage += "{只有测试服才可设置该属性}\n";
								}
							}else{
								m = cl.getMethod("set"+Character.toUpperCase(name.charAt(0))+name.substring(1),new Class[]{c});
							...
							String recorder = "";
							if(object!=null){
								recorder = object.toString();
							}
							errorMessage += "玩家["+player.getName()+"]的属性[" + name + "]值从{"+oldO+"}被设置为{"+o+"}\n";
							sb.append("操作人{"+recorder+"}-玩家账号{"+player.getUsername()+"}-玩家{"+player.getName()+"}-的属性{" + name + "}-值从{"+oldO+"}-被设置为{"+o+"}").append("<br>");
							//数据统计
							if(PlatformManager.getInstance().isPlatformOf(Platform.韩国) && !TestServerConfigManager.isTestServer()){
								String playername = player.getName();
								long receiverId = player.getId();
								...
								try{
									...
									buffer.append("<tr><td>"+title+"</td><td>"+GameConstants.getInstance().getServerName()+"</td><td>"+player.getUsername()+"</td><td>"+playername+"</td><td>"+receiverId+"</td><td>"+ipaddress+"</td>"+"<td>"+name+"</td>"+"<td>"+oldO.toString()+"</td>"+"<td>"+o.toString()+"</td><td>"+recorder+"</td>"+"<td>"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis())+"</td></tr>");
									buffer.append("</table>");
									if(!TestServerConfigManager.isTestServer()){
										EnterLimitManager.sendMail("[后台修改属性] [" + GameConstants.getInstance().getServerName() + "]", buffer.toString());
									}
									if(ActivitySubSystem.logger.isInfoEnabled()){
										ActivitySubSystem.logger.info("[GM后台发送邮件] [操作人:"+recorder+"] [角色名:"+playername+"] [角色id:"+receiverId+"] [ip:"+ipaddress+"] [属性:"+name+"] [老值:"+oldO.toString()+"] [新值:"+o.toString()+"]");
									}
								}catch(Exception e){
									e.printStackTrace();
									ActivitySubSystem.logger.warn("[GM后台发送邮件] [异常] [操作人:"+recorder+"] [角色名:"+playername+"] [角色id:"+receiverId+"] [ip:"+ipaddress+"] [属性:"+name+"] [老值:"+oldO.toString()+"] [新值:"+o.toString()+"]",e);
								}
							}
					...
					if(!TestServerConfigManager.isTestServer()){
						EnterLimitManager.sendMail("[后台修改属性] [" + GameConstants.getInstance().getServerName() + "]", sb.toString());
					}
				}catch(Exception e){
...
<%@page import="com.fy.engineserver.util.gmstat.event.PlayerPropertyChangeEvent"%>
...
<%@page import="com.fy.engineserver.enterlimit.EnterLimitManager"%><html><head>
...
请输入角色名：<input type='text' name='playerName' id='playerName' value='<%=playerName %>' size='20'><input type='submit' value='提  交'>
</form>
<br>
用户属性列表：<br/>
<form id='f2' name='f2' method="post" action="mod_player.jsp"><input type='hidden' name='action' value='modify_player'>
