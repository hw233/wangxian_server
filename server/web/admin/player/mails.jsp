...
function forceshanchufujian(mailId){
	var str = prompt("请输入强制删除的密码");
	if(str)
	{
		document.getElementById("isforce").value = "true";
		document.getElementById("forcepasswd").value = str;
		document.getElementById("deleteMailId").value = mailId;
		document.f4.submit();
	}
}
...
if(p != null){
	...
	String forcepasswd = ParamUtils.getParameter(request, "forcepasswd","");
	
	
	if(deleteMailId != null && !deleteMailId.trim().equals("")){
		if(isforce)
		{
			if("@#$qwefgh".equalsIgnoreCase(forcepasswd.trim()))	
				mmanager.deleteMail(Long.parseLong(deleteMailId));
			else
			{
...
<form name="f4" >
<input type="hidden" id="forcepasswd" name="forcepasswd">
<input type="hidden" id="isforce" name="isforce" value="false">
<input type="hidden" id="deleteMailId" name="deleteMailId">
	...
	<table id="test1" align="center" width="90%" cellpadding="2" cellspacing="1" border="0" bgcolor="#000000">
		...
		<tr>
			...
			此格空<br>
			<%} }%>
			<input type="button" value="提取" onclick="javascript:tiqufujian('<%=mail.getId() %>');"><input type="button" value="删除" onclick="javascript:shanchufujian('<%=mail.getId() %>');">
			<input type="button" value="强制删除" onclick="javascript:forceshanchufujian('<%=mail.getId() %>');">
