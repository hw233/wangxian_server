...
if(articleName != null){
	...
	if(as != null){
		...
		<tr>
		<td>物品名</td><td>等级</td><td>颜色</td><td>描述</td>
		</tr>
		<%
		if(!al.isEmpty()){
			for(Article article : al){
				if(article != null){
					%>
					<tr class="color<%=article.getColorType() %>">
					<td><a href="../ArticleByName.jsp?articleName=<%=article.getName() %>"><%=article.getName() %></a></td><td><%=article.getArticleLevel() %></td><td><%=article.getColorType() %></td><td><%=article.getDescription() %></td>
					</tr>
					<%
				}
			}
