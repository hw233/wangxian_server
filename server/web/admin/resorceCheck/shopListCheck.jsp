...
<body><div>
	...
	<div style="float:left"><table style="font-size: 12px;" border="1">
				...
				for (Goods g : list) {
					...
					if(gColor>0&&!(article instanceof Equipment || article instanceof Weapon)){
						for(Goods gg:allGoods){
							//if(g.getArticleName().equals(gg.getArticleName()) && gg.getColor()==0){
		...
		<tr style="color: <%=changePrice ? "red" : ""%>;background-color:<%=saling ? "" : "#e0e0e0" %>">
			<td class="color<%=color %>"><%=g.getArticleName()%><%=shop.isBuyBinded()? "(绑)":"" %></td>
			<td class="color<%=color %>"><%=g.getArticleName_stat()%><%=shop.isBuyBinded()? "(绑)":"" %></td>
			<td><%=g.getColor()%></td>
