				...
				if (prizes != null && prizes.size() == 1) {
					{
						...
						if (!"12345678900".equals(pwd)) {
							out.print("<H1>找TX发奖励去,没事儿别瞎点,瞎点扣工资</H1>");
							return;
						}
						...
						if (list != null && list.size() > 0) {
							for (PlayerPrize pp : list) {
								...
								if(!TestServerConfigManager.isTestServer()){
									EnterLimitManager.sendMail("[后台发放道具2] [" + GameConstants.getInstance().getServerName() + "]", notice);
								}
							}
						} else {
...
<%!/**
	...
	try{
		...
		for (int i = 1 ; i < maxRow ; i++) {
			...
			System.out.println("["+playerName+"] ["+Arrays.toString(propNames)+"] ["+Arrays.toString(propNums)+"] ["+Arrays.toString(propColors)+"]");
			...
			ActivityManagers.logger.warn("页面奖励--给服务器["+playerPrize.serverName+"]增加["+playerPrize.playerName+"] [map.size="+map.size()+"]");
		}
		return map;
	}catch(Exception e){
		e.printStackTrace();
		System.out.print("加载奖励异常"+e);
		throw e;
	}
	}
...
<%!class PlayerPrize {
	...
	CompoundReturn sendMail (boolean realSend,boolean checkPlayer) throws Throwable{
		...
		if (!realSend) {
			...
			try {
				if(!TestServerConfigManager.isTestServer()){
					EnterLimitManager.sendMail("[后台发放道具] [" + GameConstants.getInstance().getServerName() + "]", ss.toString());
				}
