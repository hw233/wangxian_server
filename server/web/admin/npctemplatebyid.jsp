...
com.fy.engineserver.datasource.skill.*,java.lang.reflect.*,com.google.gson.*,java.lang.annotation.*,com.fy.engineserver.sprite.npc.*,com.fy.engineserver.datasource.career.*"%><%! 
...
<br><br>
	...
	map.put("nPCCategoryId","NPC的种类ID，此ID在编辑的时候必须唯一");
	map.put("flushFrequency","刷新频率，表示此对应的NPC消失后，多少毫秒内出现");
	...
	map.put("activationType","激活条件，所谓激活就是攻击玩家:0  表示被攻击，准备攻击攻击他的玩家,1  表示进入NPC的视野范围，NPC看到玩家后，就开始主动攻击玩家，同时如果NPC被攻击，优先攻击攻击他的玩家,2  满足0和1，同时视野范围内的同类NPC被攻击，就开始主动攻击玩家");
	map.put("activationWidth","激活的范围宽度，以NPC为中心");
	map.put("activationHeight","激活的范围高度，以NPC为中心");
	map.put("pursueWidth","追击的范围宽度，以NPC为中心");
	map.put("pursueHeight","追击的范围高度，以NPC为中心");
	map.put("backBornPointMoveSpeedPercent","回出生点的速度是移动速度的百分之多少。150表示1.5倍");
	map.put("backBornPointHp","回程路上的补血量，每0.5秒钟的补血");
	map.put("activeSkillIds","NPC装备的技能ID列表");
	map.put("activeSkillLevels","NPC装备的技能ID列表，对应的各个技能的级别");
	map.put("scheme","NPC的avatar属性，此属性为静态属性");
	map.put("gameName","游戏中名称");
	map.put("title","NPC的称号");
	map.put("name","NPC名");
	map.put("level","NPC的等级");
	map.put("type","NPC类型，为mapping使用的");
	...
	map.put("windowId","NPC对应窗口的Id");
	map.put("politicalCamp","阵营 0中立，1紫薇宫，2日月盟");
	map.put("taskId","任务限制，如果没有任务限制，可以设置taskId为-1");
	map.put("articleName","采集NPC携带的物品名称");
	map.put("","");
	Gson gson = new Gson();
	String id = request.getParameter("spriteCategoryId");
	NPC sprite = null;
	for(MemoryNPCManager.NPCTempalte spriteTemplate : sts){
		if(id.equals(""+spriteTemplate.NPCCategoryId)){
			sprite = spriteTemplate.newNPC();
		}
	}		
	if(sprite != null){
  				...
  				<%
  					if(method.getReturnType().getName().indexOf("[") == 0){
  						if("activeSkillIds".equals(name)){
							...
							%>
							<td><%= gson.toJson(method.invoke(sprite))%></td>
