...
try {
	...
	String json = ParamUtils.getParameter(request, "json", "");
	...
	if(!StringUtils.isEmpty(json))
	{
			...
			if(player != null)
			{
				...
				try
				{
					if(PlayerManager.getInstance().getPlayer(player.getName()) != null)
					{
						...
						player.setName(player.getName()+"@" + serverName);
					}
				}
				catch(Exception e)
				{
			...
			}
			{
				/* if(linkedHashMap.get("cave") != null)
				{
					...
					if (cave != null) {
						...
						Field versionField = UnitedServerManager2.getFieldByAnnotation(Cave.class, SimpleVersion.class);
						versionField.setAccessible(true);
			...
			{
				if(linkedHashMap.get("fairyrobbery") != null){    //仙婴
	...
	ArticleManager.logger.warn("[角色转移] [异常]",e);
	out.print("error");
}
