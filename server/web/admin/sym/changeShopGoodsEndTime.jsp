...
// 			}
			...
			String goodNames[] = {"宝石湛天(2级)","宝石墨轮(2级)","宝石幽橘(2级)","宝石枳香(2级)","宝石竹清(2级)","宝石湛天(3级)","宝石墨轮(3级)","宝石幽橘(3级)","宝石枳香(3级)","宝石竹清(3级)"};
			
			List<Goods> oldgs = shop.getGoods(); 
			for(int i=0;i<oldgs.size();i++){
				for(String name:goodNames){
					if(name.equals(oldgs.get(i).getArticleName())){
						if(oldgs.get(i).getFixTimeEndLimit()>0){
							out.print(name+"<font color='yellow'>修改之前</font>的结束时间是:"+TimeTool.formatter.varChar23.format(oldgs.get(i).getFixTimeEndLimit())+"<br>");
							oldgs.get(i).setFixTimeEndLimit(TimeTool.formatter.varChar19.parse("2014-05-26 23:59:59"));
							out.print(name+" 修改之后的结束时间是:"+TimeTool.formatter.varChar23.format(oldgs.get(i).getFixTimeEndLimit())+"<br><hr>");
							ActivitySubSystem.logger.warn("[后台修改急翔如溢兑换商店时间] [ok] [结束时间："+(TimeTool.formatter.varChar23.format(oldgs.get(i).getFixTimeEndLimit()))+"] ");
						}
					}
				}
			}
