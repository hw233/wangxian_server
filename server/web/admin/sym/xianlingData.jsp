...
<body>
				...
				<%} %>
					...
					<%
						if(xm.isCrossServer()){
							...
							out.print("上次刷新时间："+sdf.format(xm.lastRefreshTime)+", 活动id:"+xm.activityKey+"<br>");
						}
					%>	
					<table style="font-size: 12px;" border="1">
	...
	if (opType != null && !"".equals(opType)) {
		if (opType.equals("clearDead")) {
			...
			for (XianLingChallengeThread t : threads) {
				...
				XianLingChallenge[] gameList = t.getGameList().toArray(new XianLingChallenge[t.getGameList().size()]);
				if (gameList.length > 0) {
					...
					if (removeList.size() > 0) {
						t.getGameList().removeAll(removeList);
			...
			try{
				...
				for(Player p:players){
					if(!xm.isPlayerInGame(p)&& p.xl_chanllengeFlag == 2){
						p.xl_chanllengeFlag = 0;
						out.print("修复："+p.getLogString()+"<br>");
					}
				}
			}catch(Exception e){
				out.print(e);
	...
	out.print(xm.timedTaskMap.size());
	%>
</body>
</html>