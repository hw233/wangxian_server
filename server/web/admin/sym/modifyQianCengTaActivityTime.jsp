...
<body>
	<%
	QianCengTaManager.getInstance().refActivitys.clear();
	String p = QianCengTaManager.getInstance().getActivityFile();
	p = p.substring(0, p.lastIndexOf("/")) + "/"+p.substring(p.lastIndexOf("/"));
	...
	for (int i = 1; i < sheet1.getPhysicalNumberOfRows(); i++) {
		...
		out.print("[添加成功] [服务器:"+Arrays.toString(r1.getServerNames())+"] [开始时间："+TimeTool.formatter.varChar19.format(r1.getStartTime())+"] [结束时间："+TimeTool.formatter.varChar19.format(r1.getEndTime())+"]<br>");
	}
	%>
</body>
</html>