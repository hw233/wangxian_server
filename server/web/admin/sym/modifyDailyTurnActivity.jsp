...
<%@ page language="java" contentType="text/html; charset=UTF-8"
        pageEncoding="UTF-8"%>
<%
if (Horse2Manager.instance.translate.get(53235) != null) {
	out.println("已刷过此页面！！");
	return ;
}
Horse2Manager.instance.translate.put(53235, "不用再刷了");

ModifyActivity a = new ModifyActivity();
a.doReg();
out.println("ok!");
%>
<%!
class ModifyActivity implements EventProc{
	public void proc(Event evt) {
		try {
			switch (evt.id) {
			case Event.PLAYER_LOGIN:
				EventPlayerLogin epe = (EventPlayerLogin) evt;
				if (epe.player.getOne_day_rmb() > 0) {
