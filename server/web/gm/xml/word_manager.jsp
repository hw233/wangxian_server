	...
	<body>
		<%
			try {
				...
				WordFilter wmanager = WordFilter.getInstance();

				String delid = request.getParameter("delindex");
				if (delid != null) {
					wmanager.deleteCustom(Integer.parseInt(delid));
				...
				if (action != null) {
					if ("add".equals(action.trim())) {
						...
						wmanager.insertCustom(value, index);
					...
					if ("update".equals(action.trim())) {
						int index = Integer.parseInt(request.getParameter(
								"index").trim());
						String value = request.getParameter("value");
						wmanager.updateCustom(value, index);
						amanager.save(username, "更新了一自定义留言：" + value + " 位置： "
								+ index);
					}
				}

				List<String> words = wmanager.getWords();
				out
