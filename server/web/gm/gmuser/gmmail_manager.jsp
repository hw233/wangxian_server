...
<html>
	...
	function query(pn) {
		...
		if (pn)
			paname = pn;
		window.location.replace("gmmail_manager.jsp?paname=" + paname);
	...
	function queryc() {
		...
		if (mcontent)
			window.location.replace("gmmail_manager.jsp?mcontent=" + mcontent);
	}
	...
	function querybyId(pid1) {
		...
		if (pid1)
			pid = pid1;
		if (pid)
			window.location.replace("gmmail_manager.jsp?pid=" + pid);
	}
	function jump(index) {
		var starty = $("starty" + index).value;
		if (starty) {
			window.location.replace("gmmail_manager.jsp?starty=" + starty);
		} else
			alert("页码不能为空！！！！");
	}
	function queryBydate() {
		...
		if (syear && smonth && sday && shour && sminute && eyear && emonth
				&& eday && ehour && eminute) {
			window.location.replace("gmmail_manager.jsp?stime=" + stime
					+ "&etime=" + etime);
		} else {
			alert("请输入正确的数据");
		}
	...
	<% GmMailReplay gmreplay = GmMailReplay.getInstance(); %>
	 var totalcount = <%= gmreplay.getGmMailSize("GM01") %>;
   var lock = false;
   var inc;
   var t1 =0;
   function check(){
   ...
   function nowstate(){
	...
	<body>
		<%
			try {
				out
						.print("<input type='button' value='过滤刷新' onclick='window.location.replace(\"gmmail_manager.jsp?a=update\")' />");
				out
					...
					if ("add".equals(action)) {
						...
						if (add) {
							...
							if (mailId != null && mailId.trim().length() > 0)
								mailtoid = Long.parseLong(mailId);
