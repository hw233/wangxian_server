...
<html>
	...
	function query(pn) {
		...
		if (pn)
			paname = pn;
		window.location.replace("xitong_mail.jsp?paname=" + paname);
	...
	function queryc() {
		...
		if (mcontent)
			window.location.replace("xitong_mail.jsp?mcontent=" + mcontent);
	}
	...
	function querybyId(pid1) {
		...
		if(pid1)
		  pid=pid1;
		if (pid)
			window.location.replace("xitong_mail.jsp?pid=" + pid);
	}
	function jump(){
	   var starty = $("starty").value;
	   if(starty){
	    window.location.replace("xitong_mail.jsp?starty="+starty);
	    }
	    else
	    alert("页码不能为空！！！！");
	}
	function queryBydate(){
      ...
      var stime = syear+"-"+smonth+"-"+sday+" "+shour+":"+sminute;
      var etime = eyear+"-"+emonth+"-"+eday+" "+ehour+":"+eminute;
      if(syear&&smonth&&sday&&shour&&sminute&&eyear&&emonth&&eday&&ehour&&eminute){     
        window.location.replace("xitong_mail.jsp?stime="+stime+"&etime="+etime);
      }else{
        alert("请输入正确的数据");
      }
	}
	...
	<body>
		<%
			try {
				out
						.print("<input type='button' value='刷新' onclick='window.location.replace(\"gmmail_manager.jsp\")' />");
				...
				if (action != null) {
					...
					if ("add".equals(action)) {
						...
						Mail mail = null;
						int mid = Integer.parseInt(request.getParameter("mid"));//邮件ID
						if (mid != 1)
						...
						String playname = request.getParameter("playname");
							try {
								...
								if (res) {
									...
									yun = gmreplay.sendMail(mid, mailtoid,  
											items, title, content);
								} else {
									out.print(message);
								}
				...
				int start = 0;
				int size = 20;
				long totalsize = gmreplay.getGmMailSize("系统");//总数为系统的邮件
				...
				if ((pid == null || !"".equals(pid.trim()))&&(paname == null || !"".equals(paname.trim()))&&(stime==null||etime==null)
						&& (mcontent == null || !"".equals(mcontent.trim())))//如果没有过滤过程则获取固定长度的邮件
					mails = gmreplay.getGmMail("系统", start, size);
				if (paname != null && !"".equals(paname)) {//如果过滤参数不为空则获取过滤邮件
					out.print(paname);
					// paname = URLDecoder.decode(paname);
					List<Mail> ms = gmreplay.getAllGmMail("系统");//获取所有的系统邮件
				...
				if(stime!=null&&etime!=null){
				 ...
				 Date edate = DateUtil.parseDate(etime,"yyyy-MM-dd HH:mm");
				  mails = gmreplay.getBetweenMail("系统",sdate,edate);
				...
				String ss[] = DateUtil.formatDate(new Date(),"yyyy-MM-dd-HH-mm").split("-");
				out.print("开始时间：<input type='text' id='syear' name='syear' size='3' value='"+ss[0]+"' >年");
				out.print("<input type='text' id='smonth' name='smonth' size='3' value='"+ss[1]+"' >月");
				out.print("<input type='text' id='sday' name='sday' size='3' value='"+ss[2]+"' >日");
				out.print("<input type='text' id='shour' name='shour' size='3' value='"+ss[3]+"' >时");
				out.print("<input type='text' id='sminute' name='sminute' size='3' value='"+ss[4]+"' >分 ~ ");
				out.print("结束时间：<input type='text' id='eyear' name='eyear' size='3' value='"+ss[0]+"' >年");
				out.print("<input type='text' id='emonth' name='emonth' size='3' value='"+ss[1]+"' >月");
				out.print("<input type='text' id='eday' name='eday' size='3' value='"+ss[2]+"' >日");
				out.print("<input type='text' id='ehour' name='ehour' size='3' value='"+ss[3]+"' >时");
				out.print("<input type='text' id='eminute' name='eminute' size='3' value='"+ss[4]+"' >分  ");
				out.print("<input type='button' value='查询' onclick='queryBydate();' /><br/>");
				if (paname == null&&mcontent==null&&pid==null&&stime==null&&etime==null) {//如果没有过滤则显示分页
					//out.print(start + "|" + totalsize);
					out.print("<a href='?start=1'>首页</a>  ");
					if (start > 0)
						out.print("<a href='?starty=" + (start - 20)
								+ "'>上一页</a>  ");
					...
					if ((start + 20) < totalsize)
						out.print("  <a href='?start=" + (start + 20)
								+ "' >下一页</a>  ");
					out.print("  <a href='?starty="+((totalsize-1)/20+1)+"'>末页</a>");
					out
				...
				out.print("已读："+yidu+"   未读："+weidu +"回复率："+(yidu/(yidu+weidu)));
				//跳到下载页面
				...
				if (mcontent != null && !"".equals(mcontent.trim()))
					out.print("<input type='hidden' name='mcontent' value='"
							+ mcontent + "' />");
			    if(stime!=null&&etime!=null)
			       out.print("<input type='hidden' name='stime' value='"+stime+"' /><input type='hidden' name='etime' value='"+etime+"'>");
				   out.print("<input type='hidden' name='gname' id='gname' value='系统'/> ");
