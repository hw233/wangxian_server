...
<html>
	...
	function delete1(index) {
		if (window.confirm("你是否确定要删除此世界频道发言"))
			window.location.replace("broadwordmessage.jsp?delindex=" + index);

	}
</script>
	</head>
	<body>
		<%!private List<WordChatCast> wcs = new ArrayList<WordChatCast>();%>
		<%
			try {
				...
				if (delindex != null && !"".equals(delindex)) {
					...
					WordChatCast b1 = wcs.get(index);
					//暂停
					b1.stop();
					//删除
					wcs.remove(b1);
					//保存操作
					amanager.save(username, "删除 了世界频道发言 位置为 " + delindex);
					//保存历史记录
					...
					try {
						...
						if (!"#:".equals(stimestr) && !"#:".equals(etimestr)) {
							...
							long etime = DateUtil.parseDateSafely(etimestr,
									"yyyy-MM-dd#HH:mm").getTime();
							WordChatCast wc = new WordChatCast(message, stime, etime,
									dtime);//创建一个世界频道发言对象
							List<WordChatCast> wcss = wcs;
							for (int j = 0; j < wcss.size(); j++) {
								//将新创间的世界频道发言对象按照开始的时间顺序放置在正在运行的世界频道发言列表当中
								if (wc.getStartTime() > wcss.get(j)
										.getStartTime()) {
									wcss.add(j, wc);
									break;
								}
							...
							wcs = wcss;
							wc.start();//开始世界频道发言
							amanager.save(username, "添加了一个 世界频道发言  内容：" + message);
							out
									.print("添加了一个世界频道发言 " + message + "    开始时间:"
											+ new Date(stime) + "  结束时间:"
											+ new Date(etime) + " cycle time :"
											+ dtime);
						}
					} catch (Exception e) {
						out.print("世界频道发言发送失败" + e.getMessage());
					}
				}
				StringBuffer fasong = new StringBuffer(
						"<p><input type='button' value='刷新' onclick='window.location.replace(\"broadwordmessage.jsp\")' /></p>");
				fasong
						.append("<form action='?add=true' method='post' id='f1' ><table width='80%' ><caption>添加世界频道发言序列</caption><tr>");
				fasong
				...
				List<WordChatCast> wcdel = new ArrayList<WordChatCast>();
				if (wcs.size() > 0) {
					out.println(wcs.get(0).getMessage());
					long date = new Date().getTime();
					for (int i = 0; i < wcs.size(); i++) {
						//遍历已有的序列
						WordChatCast bcast = wcs.get(i);
				...
				for (WordChatCast bc : wcdel) {
				   //保存系统删除日志
					...
					bmanager.saveRecord(bcr);
					amanager.save("系统", "删除 了世界频道发言 位置为 " + bc.getMessage());
				}
				action.append("</table>");
