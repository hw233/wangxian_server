...
<html>
	...
	function queryBygmplayer(start, sname) {
		var gmname = $("gmname").value;
		var player = $("player").value;
		if (gmname && player)
			window.location.replace("?gmname=" + gmname + "&start=" + start
					+ "&player=" + player + "&sname=" + sname);
		else
			alert("请输入正确的信息");
	}
</script>
	...
	<body>
		...
		<%
			try {
				out
						.print("<input type='button' value='刷新' onclick='window.location.replace(\"chat_record.jsp\")' /><br/>");
// 				String gmusername = session.getAttribute("username").toString();
				...
				int size = 8;
				out
						.print("gm姓名<input type='text' name ='gmname' id='gmname' value='' /> <input type='button' value='根据gm查询' onclick='queryBygm("
								+ start + ",\"alonechat\")' />");
				out
						.print("角色名<input type='text' name ='player' id='player' value='' /> <input type='button' value='根据gm和玩家查询' onclick='queryBygmplayer("
								+ start + ",\"alonechat\")' /><br/>");
				if (start >= 0) {
				...
				out
						.print("<table width='80%' ><caption>聊天记录<caption><tr><th width=20% >发送人</th><th width=20% >收信人</th><th width=40% >信息</th><th width=20% >发生时间</th></tr>");
				out.print("消息的总长度为" + rms.size());
				if (rms != null && rms.size() > 0) {
					for (int i = rms.size() - 1; i > -1; i--) {
						RecordMessage r = rms.get(i);
						out.print("<tr><td>" + r.getFname() + "</td><td>"
								+ r.getToname() + "</td><td>" + r.getMessage()
								+ "</td><td>" + r.getSendDate() + "</td></tr>");
					}
				}
			} catch (Exception e) {
