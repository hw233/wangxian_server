...
<script type="text/javascript">
  ...
  function showmessage(gmname){
       chatborder.getMessageRiYue(show);
  }
  function show(mes){
      if(mes.length>0){
      ...
      for(i=0;i<mes.length;i++){
        ...
        if(s[0].indexOf("GM")==0){
          ...
          m2=m2+"<p style=\"color:green\" ><a href='user_action.jsp?playerid=" +s1+"'>"+s[0]+"</a>&nbsp;&nbsp;&nbsp;&nbsp;"+s[1]+"<br/>"+s[2]+"</p>" + m2;
        }
      } 
     if(m2!=""){ 
  ...
  function send(gmname){
     ...
     if(sendms){
       chatborder.sendMessageRiYue(gmname,sendms);
     }
  }
   function delay(){
      clearInterval(inc);
...
<body>
  ...
  <%  
      ...
      amanager.save(username,"参与了日月频道聊天");
      out.print("gmname:"+gmname);
      //String gmname = "GM02";
      out.print("<br/><input type='button' value='查看聊天记录' onclick='window.open(\"channel_record.jsp?sname=riyuechat\")' />");
      ...
      out.print("<input type='button' value='恢复' onclick='review();' />"); 
      out.print("<p></p><p></p><p align='center' ><h2>与日月盟聊天</h2></p>");
