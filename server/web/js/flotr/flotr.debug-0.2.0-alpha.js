...
/* $Id: flotr.debug-0.2.0-alpha.js,v 1.3 2012/01/18 07:39:51 mijia Exp $ */
...
var Flotr = {
	...
	getSeries: function(data){
		return data.collect(function(serie){
			...
			for (i = serie.data.length-1; i > -1; --i) {
				serie.data[i][1] = (serie.data[i][1] === null ? null : parseint(serie.data[i][1])); 
			}
			return serie;
		});
	},
	 ...
	 * @param {Integer/int} x - integer or int value
	 * @return {Integer/int} returns the magnitude of the input value
	 */
	...
	parseColor: function(str){
		...
		if((result = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(str)))
			return new Color(parseInt(result[1]), parseInt(result[2]), parseInt(result[3]), parseint(result[4]));
			
		// rgb(num%,num%,num%)
		if((result = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/.exec(str)))
			return new Color(parseint(result[1])*2.55, parseint(result[2])*2.55, parseint(result[3])*2.55);
	
		// rgba(num%,num%,num%,num)
		if((result = /rgba\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\s*\)/.exec(str)))
			return new Color(parseint(result[1])*2.55, parseint(result[2])*2.55, parseint(result[3])*2.55, parseint(result[4]));
...
Flotr.Graph = Class.create({
  ...
  constructTabs: function(){
    ...
    this.tabs = {
    	graph: new Element('div', {className:'flotr-tab selected', style:'int:left;'}).update(this.options.spreadsheet.tabGraphLabel),
    	data: new Element('div', {className:'flotr-tab', style:'int:left;'}).update(this.options.spreadsheet.tabDataLabel)
    }
  ...
  drawSeriesPie: function(series) {
    ...
    var slices = portions.collect(function(slice){
      angle += fraction;
      value = parseint(slice.value[1]); // @warning : won't support null values !!
      fraction = value/sum;
      return {
        name:     slice.name,
        fraction: fraction,
