 ...
 * 
 */
public class Option_ExchangeArticle_Title extends Option implements NeedInitialiseOption,
		NeedCheckPurview {
	...
	public void doTitle(Player player){
		int count = PlayerTitlesManager.getInstance().getKeyByType(key);
		String titleName = PlayerTitlesManager.getInstance().getValueByType(key);
		if (count == -1) {
			ActivitySubSystem.logger.error("[兑换称号活动] " + player.getLogString() + "[兑换称号:" + titleName + "] [失败] [称号不存在]");
		} else {
			PlayerTitlesManager.getInstance().addTitle(player, key, useAtonce);
