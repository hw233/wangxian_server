 ...
 *
 */
public class Option_Channel_NPC_Reward extends Option implements NeedCheckPurview{
	...
	private String articlename = "Olleh 고객 감사패키지";
	private String mailtitle = "Olleh 고객 감사패키지";
	private String mailcount = "Olleh 고객 감사패키지";
	...
	public void doSelect(Game game, Player player) {
		...
		if(ddc!=null && ddc.get(player.getId()+"Tstore渠道活动20130820")==null){
			Article a = ArticleManager.getInstance().getArticle(articlename); 
			if(a==null){
				ActivitySubSystem.logger.warn("[Tstore渠道活动] [物品不存在] [玩家信息："+player.getLogString()+"] ["+articlename+"]");
				return;
			}
			...
			try {
				...
				e.printStackTrace();
				ActivitySubSystem.logger.warn("[Tstore渠道活动] [创建物品异常] [玩家信息："+player.getLogString()+"] ["+articlename+"]",e);
				return;
			}
			...
			if(ae!=null){
				if(player.getKnapsack_common().isFull()){
					...
					try {
						mm.sendMail(player.getId(), new ArticleEntity[]{ae}, mailtitle, mailcount, 0, 0, 0, "Tstore渠道活动");
						ActivitySubSystem.logger.warn("[Tstore渠道活动] [领取礼包成功] [邮件发送] ["+player.getLogString()+"]");
					} catch (Exception e) {
						e.printStackTrace();
						ActivitySubSystem.logger.warn("[Tstore渠道活动] [通过邮件发送奖励异常] [玩家信息："+player.getLogString()+"] ["+articlename+"]",e);
					}
					player.sendError(背包满提示);
				}else{
					player.putToKnapsacks(ae, "Tstore渠道活动");
					ActivitySubSystem.logger.warn("[Tstore渠道活动] [领取礼包成功] [放入背包] ["+player.getLogString()+"]");
				}
				ddc.put(player.getId()+"Tstore渠道活动20130820", player.getId());
			}
		}else{
			player.sendError(已经领取过);
			ActivitySubSystem.logger.warn("[Tstore渠道活动] [已经领取过] ["+player.getLogString()+"]");
		}
	}
	...
	public boolean canSee(Player player) {
		...
		if(passport != null){
			...
			ActivityManagers.logger.warn("[Tstore渠道活动] [玩家渠道为空] [请确定玩家渠道查看是否补偿] [玩家信息："+player.getLogString()+"]");
			return false;
		}
