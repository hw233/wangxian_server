 ...
 *
 */
public class Option_MergeServer_Reward_gf extends Option implements NeedCheckPurview , MConsole{
	...
	private String startTime = "2013-11-26 00:00:00";
	@ChangeAble("endTime")
	private String endTime = "2015-11-21 23:59:59";
	...
	private String articlenames [] = {"合服礼包",  "合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包","合服礼包"};
	...
	public void doSelect(Game game, Player player) {
		...
		Long nowtiem = (Long)ddc.get(player.getId()+"合服活动20131114");
		if(nowtiem==null){
			ddc.put(player.getId()+"合服活动20131114", new Long(11));
			nowtiem = (Long)ddc.get(player.getId()+"合服活动20131114");
		}
		if(nowtiem!=null){
			if(!isSameDay(System.currentTimeMillis(),nowtiem.longValue())){
				...
				if(index>=0){
					...
					if(a==null){
						ActivitySubSystem.logger.warn("[合服活动20131114] [物品不存在] [玩家信息："+player.getLogString()+"] ["+articlename+"]");
						return;
					}
					...
					try {
						...
						e.printStackTrace();
						ActivitySubSystem.logger.warn("[合服活动20131114] [创建物品异常] [玩家信息："+player.getLogString()+"] ["+articlename+"]",e);
						return;
					}
					...
					if(ae!=null){
						...
						if(player.getKnapsack_common().isFull()){
							...
							try {
								mm.sendMail(player.getId(), new ArticleEntity[]{ae}, mailtitle, mailcount, 0, 0, 0, "合服活动20131114");
								ActivitySubSystem.logger.warn("[合服活动20131114] [领取礼包成功] [邮件发送] ["+player.getLogString()+"]");
								issucc = true;
							} catch (Exception e) {
								e.printStackTrace();
								ActivitySubSystem.logger.warn("[合服活动20131114] [通过邮件发送奖励异常] [玩家信息："+player.getLogString()+"] ["+articlename+"]",e);
							}
							player.sendError(背包满提示);
						}else{
							player.putToKnapsacks(ae, "合服活动20131114");
							ActivitySubSystem.logger.warn("[合服活动20131114] [领取礼包成功] [放入背包] ["+player.getLogString()+"]");
							issucc = true;
						}
						if(issucc){
							ddc.put(player.getId()+"合服活动20131114", new Long(System.currentTimeMillis()));
			...
			ActivitySubSystem.logger.warn("[合服活动20131114] [ddc==null] ["+player.getLogString()+"]");
		}
	}
