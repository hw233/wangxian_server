...
public class Option_Cave_StoreSizeUp_Wood extends CaveOption implements FaeryConfig,
		NeedCheckPurview {

	public Option_Cave_StoreSizeUp_Wood() {
	...
	public void doSelect(Game game, Player player) {
		...
		int resLevel = cave.getStorehouse().getCurrResourceLevel(FaeryConfig.FRUIT_RES_WOOD);
		...
		try {
			...
			int reLv = FaeryManager.getInstance().getStoreCfgs()[tL].getCurrResourceLevel(FaeryConfig.FRUIT_RES_WOOD);
			if (resLevel >= reLv ) {
		...
		mw.setDescriptionInUUB(Translate.translateString(Translate.text_cave_111, new String[][] { { Translate.STRING_1, FaeryConfig.FRUIT_RES_NAMES[FaeryConfig.FRUIT_RES_WOOD] }, { Translate.COUNT_1, String.valueOf((++resLevel)) }, { Translate.STRING_2, cfg.getLvUpCost().toString() } }));
		
		Option_Cave_StoreSizeUp_Res_Confirm confirm = new Option_Cave_StoreSizeUp_Res_Confirm(getNpc(), FaeryConfig.FRUIT_RES_WOOD);
