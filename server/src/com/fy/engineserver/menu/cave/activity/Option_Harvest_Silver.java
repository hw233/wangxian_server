 ...
 * 
 */
public class Option_Harvest_Silver extends CaveOption implements NeedCheckPurview {
...
//			long exp = (long) (25000 * 10 * 0.2);// 2级经验树(橙色)的20%
//			player.addExp(exp, ExperienceManager.庄园活动);
//			ActivitySubSystem.logger.error("[庄园剪刀活动] [银剪刀] [" + player.getLogString() + "] [增加经验:" + exp + "]");
			...
			if(index ==-1){
				ActivitySubSystem.logger.warn("[庄园剪刀活动] [额外奖励] [没有获得额外奖励] [银剪刀] [randomnum:"+randomnum+"] [value:"+value+"] [index:"+index+"] ["+player.getLogString()+"]");
				return;
			}
			String articlename = rewards[index];
			Article a = ArticleManager.getInstance().getArticleByCNname(articlename);
			if(a==null){
				ActivitySubSystem.logger.warn("[庄园剪刀活动] [额外奖励] [出错:物品不存在"+articlename+"] [银剪刀] [randomnum:"+randomnum+"] [value:"+value+"] [index:"+index+"] ["+player.getLogString()+"]");
				return;
			}
			try {
				...
				if(ae==null){
					ActivitySubSystem.logger.warn("[庄园剪刀活动] [额外奖励] [出错:创建物品失败"+articlename+"] [银剪刀] [randomnum:"+randomnum+"] [value:"+value+"] [index:"+index+"] ["+player.getLogString()+"]");
					return;
				}
				boolean success = player.putAll(new ArticleEntity[]{ae}, "银剪刀活动");
				if (success) {
					ArticleStatManager.addToArticleStat(player, null, ae, ArticleStatManager.OPERATION_物品获得和消耗, 0L, (byte) 0, 1L, "银剪刀活动", "");
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		public CompoundReturn doDeduct(Player player) {
			if (player.getSilver() < 50000) {
				return CompoundReturn.createCompoundReturn().setBooleanValue(false).setStringValue(Translate.元宝不足);
			}
			try {
				BillingCenter.getInstance().playerExpense(player, 50000, CurrencyType.YINZI, ExpenseReasonType.活动, "银剪刀活动");
			} catch (Exception e) {
				e.printStackTrace();
				return CompoundReturn.createCompoundReturn().setBooleanValue(false).setStringValue(Translate.元宝不足);
			}
			...
			if (Translate.text_cave_001.equals(failreason)) {
				...
				try {
					...
					if (canPickup) {
						...
						ActivitySubSystem.logger.error("[庄园剪刀活动] [银剪刀] [" + player.getLogString() + "] [摘取上限]");
						return;
					}
				} catch (Exception e) {
					ActivitySubSystem.logger.error("[庄园剪刀活动] [银剪刀] [" + player.getLogString() + "] [异常]", e);
				}
			} else {
