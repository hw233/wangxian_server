...
public class Option_Cave_StoreSizeUp_Stone extends CaveOption implements FaeryConfig,
		NeedCheckPurview {

	public Option_Cave_StoreSizeUp_Stone() {
	...
	public void doSelect(Game game, Player player) {
		...
		int resLevel = cave.getStorehouse().getCurrResourceLevel(FaeryConfig.FRUIT_RES_STONE);
		...
		try {
			...
			int reLv = FaeryManager.getInstance().getStoreCfgs()[tL].getCurrResourceLevel(FaeryConfig.FRUIT_RES_STONE);
			if (resLevel >= reLv ) {
		...
		mw.setDescriptionInUUB(Translate.translateString(Translate.text_cave_111, new String[][] { { Translate.STRING_1, FaeryConfig.FRUIT_RES_NAMES[FaeryConfig.FRUIT_RES_STONE] }, { Translate.COUNT_1, String.valueOf((++resLevel)) }, { Translate.STRING_2, cfg.getLvUpCost().toString() } }));
		// "<f>你确定要提升" + FaeryConfig.FRUIT_RES_NAMES[FaeryConfig.FRUIT_RES_STONE] + "等级到" + (++resLevel) + "级?</f>\n消耗资源:" + cfg.getLvUpCost().toString());

		Option_Cave_StoreSizeUp_Res_Confirm confirm = new Option_Cave_StoreSizeUp_Res_Confirm(getNpc(), FaeryConfig.FRUIT_RES_STONE);
