...
/**
 * 如果玩家的具有某种级别的某种声望，修理当前装备就给予相应的折扣
 * 
 *
 */
public class Option_RepairCurrentEquipmentDaZhe extends Option{
...
//						Game.logger.error("[以打折价格修理装备配置出错] [级别配置错误] ["+ls[0]+"]");
						Game.logger.error("[以打折价格修理装备配置出错] [级别配置错误] [{}]", new Object[]{ls[0]});
						continue;
					}
...
//						Game.logger.error("[以打折价格修理装备配置出错] [折扣配置错误] ["+ls[1]+"]");
						Game.logger.error("[以打折价格修理装备配置出错] [折扣配置错误] [{}]", new Object[]{ls[1]});
						continue;
					}
					
					levelMap.put(level, discount);
				}else{
//					Game.logger.error("[以打折价格修理装备配置出错] [级别的打折数据配置错误] ["+vs[i]+"]");		
					Game.logger.error("[以打折价格修理装备配置出错] [级别的打折数据配置错误] [{}]", new Object[]{vs[i]});		
				}
			}
		}else{
			Game.logger.error("[以打折价格修理装备配置出错] [没有配置每个级别的打折数据]");
		}
	}
	...
	public int getOId() {
		return OptionConstants.SERVER_FUNCTION_打折修理当前装备;
	}
	...
	public Option copy(Game game,Player p){
		Option_RepairCurrentEquipmentDaZhe op = new Option_RepairCurrentEquipmentDaZhe();
