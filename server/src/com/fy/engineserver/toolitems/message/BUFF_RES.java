 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>buffs.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
...
public class BUFF_RES implements ResponseMessage{
	...
	public BUFF_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		...
		for(int i = 0 ; i < buffs.length ; i++){
			...
			buffs[i] = new String(content,offset,len);
		offset += len;
		}
	}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			for(int i = 0 ; i < buffs.length; i++){
				byte[] tmpBytes2 = buffs[i].getBytes();
