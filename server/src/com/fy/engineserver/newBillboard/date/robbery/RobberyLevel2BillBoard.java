 ...
 *
 */
public class RobberyLevel2BillBoard extends Billboard{
	...
	public void update() throws Exception {
		...
		long[] ids = em.queryIds(TransitRobberyEntity.class, "currentLevel>=? and currentLevel<=?", new Object[] {1,2}, "passLayer desc", 1, BillboardsManager.实际条数 + 1);
		...
		if(list != null && list.size() > 0) {
			for(int i=0; i<list.size(); i++) {
				...
				if(entity != null) {
					if((entity.getCurrentLevel() == 1 && entity.getPassLayer() > 300) || (entity.getCurrentLevel() == 2 && entity.getPassLayer() >= 200 && entity.getPassLayer() < 300)) {		//判定是否参加新一重渡劫，不参加排行榜则在以前的天劫中
						continue;
					}
					if(entity.getCurrentLevel() == 2 && entity.getPassLayer() == 0) {			//兼容以前数据
						continue;
					}
