...
public class DaShiSkillBillboard extends Billboard{

	public void update()throws Exception {
		super.update();
		
		SimpleEntityManager<BillboardStatDate> em = BillboardStatDateManager.em;
		long[] ids = em.queryIds(BillboardStatDate.class, " skillChongNum > ? ",new Object[]{0},"skillChongNum desc ",1,BillboardsManager.显示条数+1);
		if(ids != null && ids.length > 0){
			...
			if(playerList != null && playerList.size() > 0){
				...
				setDates(bbds);
				BillboardsManager.logger.warn("[更新榜单数据成功] [大使技能重数] ["+this.getLogString()+"]");
