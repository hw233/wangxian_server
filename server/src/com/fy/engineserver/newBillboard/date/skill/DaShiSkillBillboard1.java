...
public class DaShiSkillBillboard1 extends Billboard{

	public void update()throws Exception {
		super.update();
		
		SimpleEntityManager<BillboardStatDate> em = BillboardStatDateManager.em;
		long[] ids = em.queryIds(BillboardStatDate.class, " skillChongNum > ? AND country = ? ",new Object[]{0,1},"skillChongNum desc ",1,BillboardsManager.显示条数+1);
			...
			if(playerList != null && playerList.size() > 0){
				...
				for(int i=0;i<playerList.size();i++){
					...
					if(info.getCountry()!=1){
						...
						BillboardsManager.logger.error("[大使技能排行榜国家更新] [昆仑] [成功] [name:"+info.getName()+"] [pid:"+info.getId()+"] [country1:"+info.getCountry()+"]");
						continue;
					}
