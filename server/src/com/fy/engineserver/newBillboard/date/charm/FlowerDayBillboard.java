...
//当日鲜花
public class FlowerDayBillboard extends Billboard {
...
//	玩家名称:国家:家族:糖果数
	public void update()throws Exception {
		...
		long[] ids = em.queryIds(BillboardStatDate.class, " dayFlowersNum > ? AND dayFlower = ? ",new Object[]{0,day},"dayFlowersNum desc ",1,BillboardsManager.实际条数+1);
		//排行榜统计数据
		if(ids != null && ids.length > 0){
			...
			if(playerList != null && playerList.size() > 0){
				...
				if(LogRecordManager.getInstance() != null){
					LogRecordManager.getInstance().活动记录日志(LogRecordManager.当日鲜花, this);
				}
			}else{
