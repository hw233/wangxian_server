...
//当日糖果
public class SweetDayBillboard extends Billboard {

	
	//于title对应
	//	private String[] dateValues;  显示内容：玩家ID、国家、家族、糖果数
	...
	public void update()throws Exception {
		...
		long[] ids = em.queryIds(BillboardStatDate.class, " daySweetsNum > ? AND daySweet = ? ",new Object[]{0,day},"daySweetsNum desc ",1,BillboardsManager.实际条数+1);
		//排行榜统计数据
		if(ids != null && ids.length > 0){
			...
			if(playerList != null && playerList.size() > 0){
				...
				if(LogRecordManager.getInstance() != null){
					LogRecordManager.getInstance().活动记录日志(LogRecordManager.当日糖果, this);
				}
			}else{
