 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>containsStr.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>containsStr[0].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>containsStr[0]</td><td>String</td><td>containsStr[0].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>containsStr[1].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>containsStr[1]</td><td>String</td><td>containsStr[1].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>containsStr[2].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>containsStr[2]</td><td>String</td><td>containsStr[2].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td colspan=4>......... 重复</td></tr>
 * </table>
 */
public class OPEN_SHOUHUN_WINDOW_RES implements ResponseMessage{
	...
	public OPEN_SHOUHUN_WINDOW_RES(){
	}

	public OPEN_SHOUHUN_WINDOW_RES(long seqNum,Shouhun4Client[] shouhunModels,String[] containsStr){
		this.seqNum = seqNum;
		this.shouhunModels = shouhunModels;
		this.containsStr = containsStr;
	}

	public OPEN_SHOUHUN_WINDOW_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		containsStr = new String[len];
		for(int i = 0 ; i < containsStr.length ; i++){
			len = (int)mf.byteArrayToNumber(content,offset,2);
			offset += 2;
			if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
			containsStr[i] = new String(content,offset,len,"UTF-8");
		offset += len;
		}
	}
	...
	public String getTypeDescription() {
		return "OPEN_SHOUHUN_WINDOW_RES";
	}
	...
	public int getLength() {
		...
		for(int i = 0 ; i < containsStr.length; i++){
			len += 2;
			try{
				len += containsStr[i].getBytes("UTF-8").length;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(containsStr.length);
			for(int i = 0 ; i < containsStr.length; i++){
				byte[] tmpBytes2 ;
				try{
				tmpBytes2 = containsStr[i].getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
