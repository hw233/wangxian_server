 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>sid.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>sid</td><td>String</td><td>sid.length</td><td>字符串对应的byte数组</td></tr>
 ...
 * </table>
 */
public class TENCENT_USER_LOGIN_REQ implements RequestMessage{
	...
	public TENCENT_USER_LOGIN_REQ(long seqNum,String uid,String sid,String channel,String clientId){
		this.seqNum = seqNum;
		this.uid = uid;
		this.sid = sid;
	...
	public TENCENT_USER_LOGIN_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		sid = new String(content,offset,len,"UTF-8");
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
