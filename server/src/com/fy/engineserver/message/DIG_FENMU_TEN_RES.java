 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>fendiIndexs.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>fendiIndexs</td><td>int[]</td><td>fendiIndexs.length</td><td>*</td></tr>
 ...
 * </table>
 */
public class DIG_FENMU_TEN_RES implements ResponseMessage{
	...
	public DIG_FENMU_TEN_RES(){
	}

	public DIG_FENMU_TEN_RES(long seqNum,String fenmuName,int leftYinChanzi,int leftJinChanzi,int leftLiuliChanzi,int[] fendiIndexs,FenDiModel4Client[] fendiModel){
		this.seqNum = seqNum;
		...
		this.fendiIndexs = fendiIndexs;
		this.fendiModel = fendiModel;
	}

	public DIG_FENMU_TEN_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		fendiIndexs = new int[len];
		for(int i = 0 ; i < fendiIndexs.length ; i++){
			fendiIndexs[i] = (int)mf.byteArrayToNumber(content,offset,4);
			offset += 4;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(fendiIndexs.length);
			for(int i = 0 ; i < fendiIndexs.length; i++){
				buffer.putInt(fendiIndexs[i]);
			}
	 ...
	 *	十连挖坟地索引
	 */
	public void setFendiIndexs(int[] fendiIndexs){
		this.fendiIndexs = fendiIndexs;
	}
