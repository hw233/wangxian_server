package com.fy.engineserver.message;

import com.xuanzhi.tools.transport.*;
import java.nio.ByteBuffer;



/**
 * 网络数据包，此数据包是由MessageComplier自动生成，请不要手动修改。<br>
 * 版本号：null<br>
 * 打开一键答谢界面<br>
 * 数据包的格式如下：<br><br>
 * <table border="0" cellpadding="0" cellspacing="1" width="100%" bgcolor="#000000" align="center">
 * <tr bgcolor="#00FFFF" align="center"><td>字段名</td><td>数据类型</td><td>长度（字节数）</td><td>说明</td></tr> * <tr bgcolor="#FFFFFF" align="center"><td>length</td><td>int</td><td>getNumOfByteForMessageLength()个字节</td><td>包的整体长度，包头的一部分</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>type</td><td>int</td><td>4个字节</td><td>包的类型，包头的一部分</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>seqNum</td><td>int</td><td>4个字节</td><td>包的序列号，包头的一部分</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>des.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>des</td><td>String</td><td>des.length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>num</td><td>int</td><td>4个字节</td><td>配置的长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>awardID.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>awardID</td><td>long[]</td><td>awardID.length</td><td>*</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>price.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>price</td><td>long[]</td><td>price.length</td><td>*</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>level</td><td>byte</td><td>1个字节</td><td>配置的长度</td></tr>
 * </table>
 */
public class FAIRY_THANK_RES implements ResponseMessage{

	static GameMessageFactory mf = GameMessageFactory.getInstance();

	long seqNum;
	String des;
	int num;
	long[] awardID;
	long[] price;
	byte level;

	public FAIRY_THANK_RES(){
	}

	public FAIRY_THANK_RES(long seqNum,String des,int num,long[] awardID,long[] price,byte level){
		this.seqNum = seqNum;
		this.des = des;
		this.num = num;
		this.awardID = awardID;
		this.price = price;
		this.level = level;
	}

	public FAIRY_THANK_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		des = new String(content,offset,len,"UTF-8");
		offset += len;
		num = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		awardID = new long[len];
		for(int i = 0 ; i < awardID.length ; i++){
			awardID[i] = (long)mf.byteArrayToNumber(content,offset,8);
			offset += 8;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		price = new long[len];
		for(int i = 0 ; i < price.length ; i++){
			price[i] = (long)mf.byteArrayToNumber(content,offset,8);
			offset += 8;
		}
		level = (byte)mf.byteArrayToNumber(content,offset,1);
		offset += 1;
	}

	public int getType() {
		return 0x70F0EF31;
	}

	public String getTypeDescription() {
		return "FAIRY_THANK_RES";
	}

	public String getSequenceNumAsString() {
		return String.valueOf(seqNum);
	}

	public long getSequnceNum(){
		return seqNum;
	}

	private int packet_length = 0;

	public int getLength() {
		if(packet_length > 0) return packet_length;
		int len =  mf.getNumOfByteForMessageLength() + 4 + 4;
		len += 2;
		try{
			len +=des.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
		len += 4;
		len += 4;
		len += awardID.length * 8;
		len += 4;
		len += price.length * 8;
		len += 1;
		packet_length = len;
		return len;
	}

	public int writeTo(ByteBuffer buffer) {
		int messageLength = getLength();
		if(buffer.remaining() < messageLength) return 0;
		int oldPos = buffer.position();
		buffer.mark();
		try{
			buffer.put(mf.numberToByteArray(messageLength,mf.getNumOfByteForMessageLength()));
			buffer.putInt(getType());
			buffer.putInt((int)seqNum);

			byte[] tmpBytes1;
				try{
			 tmpBytes1 = des.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
			buffer.putShort((short)tmpBytes1.length);
			buffer.put(tmpBytes1);
			buffer.putInt(num);
			buffer.putInt(awardID.length);
			for(int i = 0 ; i < awardID.length; i++){
				buffer.putLong(awardID[i]);
			}
			buffer.putInt(price.length);
			for(int i = 0 ; i < price.length; i++){
				buffer.putLong(price[i]);
			}
			buffer.put(level);
		}catch(Exception e){
		 e.printStackTrace();
			buffer.reset();
			throw new RuntimeException("in writeTo method catch exception :",e);
		}
		int newPos = buffer.position();
		buffer.position(oldPos);
		buffer.put(mf.numberToByteArray(newPos-oldPos,mf.getNumOfByteForMessageLength()));
		buffer.position(newPos);
		return newPos-oldPos;
	}

	/**
	 * 获取属性：
	 *	已有X人答谢描述
	 */
	public String getDes(){
		return des;
	}

	/**
	 * 设置属性：
	 *	已有X人答谢描述
	 */
	public void setDes(String des){
		this.des = des;
	}

	/**
	 * 获取属性：
	 *	当前有多少人答谢
	 */
	public int getNum(){
		return num;
	}

	/**
	 * 设置属性：
	 *	当前有多少人答谢
	 */
	public void setNum(int num){
		this.num = num;
	}

	/**
	 * 获取属性：
	 *	答谢奖励物品id
	 */
	public long[] getAwardID(){
		return awardID;
	}

	/**
	 * 设置属性：
	 *	答谢奖励物品id
	 */
	public void setAwardID(long[] awardID){
		this.awardID = awardID;
	}

	/**
	 * 获取属性：
	 *	答谢奖励单价
	 */
	public long[] getPrice(){
		return price;
	}

	/**
	 * 设置属性：
	 *	答谢奖励单价
	 */
	public void setPrice(long[] price){
		this.price = price;
	}

	/**
	 * 获取属性：
	 *	已设置的答谢档次,-1表示未设置过
	 */
	public byte getLevel(){
		return level;
	}

	/**
	 * 设置属性：
	 *	已设置的答谢档次,-1表示未设置过
	 */
	public void setLevel(byte level){
		this.level = level;
	}

}