 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>announcetype</td><td>int</td><td>4个字节</td><td>配置的长度</td></tr>
 ...
 * </table>
 */
public class ANNOUNCEMENT_REQ implements RequestMessage{
	...
	public ANNOUNCEMENT_REQ(){
	}

	public ANNOUNCEMENT_REQ(long seqNum,int announcetype,String channel,String[] infos){
		this.seqNum = seqNum;
	...
	public ANNOUNCEMENT_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		announcetype = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		int len = 0;
		...
		for(int i = 0 ; i < infos.length ; i++){
			...
			offset += 2;
			if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
			infos[i] = new String(content,offset,len);
		offset += len;
		}
	}
