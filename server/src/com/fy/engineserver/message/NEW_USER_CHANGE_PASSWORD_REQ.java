 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>newpasswd.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>newpasswd</td><td>String</td><td>newpasswd.length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>protectAnswer.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>protectAnswer</td><td>String</td><td>protectAnswer.length</td><td>字符串对应的byte数组</td></tr>
 * </table>
 */
public class NEW_USER_CHANGE_PASSWORD_REQ implements RequestMessage{
	...
	public NEW_USER_CHANGE_PASSWORD_REQ(){
	}

	public NEW_USER_CHANGE_PASSWORD_REQ(long seqNum,String clientId,String username,String passwd,String newpasswd,String protectAnswer){
		this.seqNum = seqNum;
	...
	public NEW_USER_CHANGE_PASSWORD_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		newpasswd = new String(content,offset,len,"UTF-8");
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		protectAnswer = new String(content,offset,len,"UTF-8");
		offset += len;
	}
	...
	public String getTypeDescription() {
		return "NEW_USER_CHANGE_PASSWORD_REQ";
	}
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=newpasswd.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
		len += 2;
		try{
			len +=protectAnswer.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put(tmpBytes1);
				try{
			tmpBytes1 = newpasswd.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
			...
			buffer.put(tmpBytes1);
				try{
			tmpBytes1 = protectAnswer.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
