 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>yanzhengPng.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>yanzhengPng</td><td>byte[]</td><td>yanzhengPng.length</td><td>数组实际长度</td></tr>
 ...
 * </table>
 */
public class GET_REGISTER_IMAGE_NEW_RES implements ResponseMessage{
	...
	public GET_REGISTER_IMAGE_NEW_RES(){
	}

	public GET_REGISTER_IMAGE_NEW_RES(long seqNum,byte[] yanzhengPng,String[] others){
		this.seqNum = seqNum;
		this.yanzhengPng = yanzhengPng;
		this.others = others;
	}

	public GET_REGISTER_IMAGE_NEW_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		yanzhengPng = new byte[len];
		System.arraycopy(content,offset,yanzhengPng,0,len);
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
	...
	public String getTypeDescription() {
		return "GET_REGISTER_IMAGE_NEW_RES";
	}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(yanzhengPng.length);
			buffer.put(yanzhengPng);
	 ...
	 *	验证码的PNG图片
	 */
	public void setYanzhengPng(byte[] yanzhengPng){
		this.yanzhengPng = yanzhengPng;
	}
