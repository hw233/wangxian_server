 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>groupIds.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>groupIds</td><td>long[]</td><td>groupIds.length</td><td>*</td></tr>
 ...
 * </table>
 */
public class GET_CHATGROUPS_RES implements ResponseMessage{
	...
	public GET_CHATGROUPS_RES(){
	}

	public GET_CHATGROUPS_RES(long seqNum,long[] groupIds,String[] names){
		this.seqNum = seqNum;
		this.groupIds = groupIds;
		this.names = names;
	}

	public GET_CHATGROUPS_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		groupIds = new long[len];
		for(int i = 0 ; i < groupIds.length ; i++){
			groupIds[i] = (long)mf.byteArrayToNumber(content,offset,8);
			offset += 8;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(groupIds.length);
			for(int i = 0 ; i < groupIds.length; i++){
				buffer.putLong(groupIds[i]);
			}
