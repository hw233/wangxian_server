 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>offLineTime.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>offLineTime[0].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>offLineTime[0]</td><td>String</td><td>offLineTime[0].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>offLineTime[1].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>offLineTime[1]</td><td>String</td><td>offLineTime[1].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>offLineTime[2].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>offLineTime[2]</td><td>String</td><td>offLineTime[2].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td colspan=4>......... 重复</td></tr>
 ...
 * </table>
 */
public class JIAZU_INFO_ADD_TIME_RES2 implements ResponseMessage{
	...
	public JIAZU_INFO_ADD_TIME_RES2(){
	}

	public JIAZU_INFO_ADD_TIME_RES2(long seqNum,long jiazuId,String jiazuName,int level,String slogan,int usedBedge,long constructionConsume,String[] titleAlias,JiazuMember4Client[] jiazuMembers,String[] offLineTime,JiazuBedge[] allBedge,int[] hadBedge,int switchCost,long septStationId,long maintai,long jiazuMoney,long warScore,long prosperity){
		this.seqNum = seqNum;
	...
	public JIAZU_INFO_ADD_TIME_RES2(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		offLineTime = new String[len];
		for(int i = 0 ; i < offLineTime.length ; i++){
			len = (int)mf.byteArrayToNumber(content,offset,2);
			offset += 2;
			if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
			offLineTime[i] = new String(content,offset,len,"UTF-8");
		offset += len;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
	...
	public String getTypeDescription() {
		return "JIAZU_INFO_ADD_TIME_RES2";
	}
	...
	public int getLength() {
		...
		for(int i = 0 ; i < offLineTime.length; i++){
			len += 2;
			try{
				len += offLineTime[i].getBytes("UTF-8").length;
			}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
				throw new RuntimeException("unsupported encoding [UTF-8]",e);
			}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(offLineTime.length);
			for(int i = 0 ; i < offLineTime.length; i++){
				byte[] tmpBytes2 ;
				try{
				tmpBytes2 = offLineTime[i].getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
