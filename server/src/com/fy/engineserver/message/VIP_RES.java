 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>vipDatas.length</td><td>int</td><td>4个字节</td><td>VIPData数组长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>vipDatas[0].vipLevel</td><td>int</td><td>4个字节</td><td>对象属性配置的长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>vipDatas[0].needCost</td><td>int</td><td>4个字节</td><td>对象属性配置的长度</td></tr>
 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>vipDatas[1].needCost</td><td>int</td><td>4个字节</td><td>对象属性配置的长度</td></tr>
 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>vipDatas[2].needCost</td><td>int</td><td>4个字节</td><td>对象属性配置的长度</td></tr>
 ...
 * </table>
 */
public class VIP_RES implements ResponseMessage{
	...
	public VIP_RES(long seqNum,VIPData[] vipDatas,int yinziPerRMB,boolean vipRewardFlag){
		this.seqNum = seqNum;
	...
	public VIP_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 8192) throw new Exception("object array length ["+len+"] big than the max length [8192]");
		vipDatas = new VIPData[len];
		for(int i = 0 ; i < vipDatas.length ; i++){
			vipDatas[i] = new VIPData();
			vipDatas[i].setVipLevel((int)mf.byteArrayToNumber(content,offset,4));
			offset += 4;
			vipDatas[i].setNeedCost((int)mf.byteArrayToNumber(content,offset,4));
			offset += 4;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			for(int i = 0 ; i < vipDatas.length ; i++){
				...
				buffer.putInt((int)vipDatas[i].getNeedCost());
				byte[] tmpBytes2 ;
				try{
	 ...
	 *	vip信息
	 */
	public void setVipDatas(VIPData[] vipDatas){
		this.vipDatas = vipDatas;
	}
