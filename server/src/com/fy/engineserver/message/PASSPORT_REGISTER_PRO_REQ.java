 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>realname.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>realname</td><td>String</td><td>realname.length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>shenfenzheng.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>shenfenzheng</td><td>String</td><td>shenfenzheng.length</td><td>字符串对应的byte数组</td></tr>
 ...
 * </table>
 */
public class PASSPORT_REGISTER_PRO_REQ implements RequestMessage{
	...
	public PASSPORT_REGISTER_PRO_REQ(){
	}

	public PASSPORT_REGISTER_PRO_REQ(long seqNum,String yanzhengma,String username,String passwd,String realname,String shenfenzheng,String usertype,boolean quickreg,String clientId,String whitecontent,String handset,String platform,String channel,long recommendid,String[] others){
		this.seqNum = seqNum;
	...
	public PASSPORT_REGISTER_PRO_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		shenfenzheng = new String(content,offset,len,"UTF-8");
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		...
		for(int i = 0 ; i < others.length ; i++){
			...
			offset += 2;
			if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
			others[i] = new String(content,offset,len,"UTF-8");
		offset += len;
		}
	}
	...
	public String getTypeDescription() {
		return "PASSPORT_REGISTER_PRO_REQ";
	}
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=shenfenzheng.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
		...
		for(int i = 0 ; i < others.length; i++){
			len += 2;
			try{
				len += others[i].getBytes("UTF-8").length;
			}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
				throw new RuntimeException("unsupported encoding [UTF-8]",e);
			}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put(tmpBytes1);
				try{
			tmpBytes1 = shenfenzheng.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
			...
			for(int i = 0 ; i < others.length; i++){
				byte[] tmpBytes2 ;
				try{
				tmpBytes2 = others[i].getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
