 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>isSucc</td><td>boolean</td><td>1个字节</td><td>布尔型长度,0==false，其他为true</td></tr>
 * </table>
 */
public class CHAT_VOICE_SUCC_REQ implements RequestMessage{
	...
	public CHAT_VOICE_SUCC_REQ(){
	}

	public CHAT_VOICE_SUCC_REQ(long seqNum,String voiceKeyID,boolean isSucc){
		this.seqNum = seqNum;
		this.voiceKeyID = voiceKeyID;
		this.isSucc = isSucc;
	}

	public CHAT_VOICE_SUCC_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += len;
		isSucc = mf.byteArrayToNumber(content,offset,1) != 0;
		offset += 1;
	}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put((byte)(isSucc==false?0:1));
		}catch(Exception e){
		 e.printStackTrace();
			buffer.reset();
