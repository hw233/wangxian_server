 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>secretKey.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>secretKey</td><td>String</td><td>secretKey.length</td><td>字符串对应的byte数组</td></tr>
 * </table>
 */
public class PLAYPHONE_USER_LOGIN_RES implements ResponseMessage{
	...
	public PLAYPHONE_USER_LOGIN_RES(long seqNum,byte result,String resultString,String authCode,String RecommendWelcome,String userName,String password,String secretKey){
		this.seqNum = seqNum;
		this.result = result;
	...
	public PLAYPHONE_USER_LOGIN_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		secretKey = new String(content,offset,len,"UTF-8");
		offset += len;
	}
	...
	public String getTypeDescription() {
		return "PLAYPHONE_USER_LOGIN_RES";
	}
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=secretKey.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put(tmpBytes1);
				try{
			tmpBytes1 = secretKey.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
