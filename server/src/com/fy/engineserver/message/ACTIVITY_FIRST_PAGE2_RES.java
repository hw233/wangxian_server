 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>menuNames.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>menuNames[0].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>menuNames[0]</td><td>String</td><td>menuNames[0].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>menuNames[1].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>menuNames[1]</td><td>String</td><td>menuNames[1].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>menuNames[2].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>menuNames[2]</td><td>String</td><td>menuNames[2].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td colspan=4>......... 重复</td></tr>
 ...
 * </table>
 */
public class ACTIVITY_FIRST_PAGE2_RES implements ResponseMessage{
	...
	public ACTIVITY_FIRST_PAGE2_RES(){
	}

	public ACTIVITY_FIRST_PAGE2_RES(long seqNum,int reqType,int[] noticeStat,String[] activityNames,String[] menuNames,int[] activityColors,int[] activityStst){
		this.seqNum = seqNum;
	...
	public ACTIVITY_FIRST_PAGE2_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		menuNames = new String[len];
		for(int i = 0 ; i < menuNames.length ; i++){
			len = (int)mf.byteArrayToNumber(content,offset,2);
			offset += 2;
			if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
			menuNames[i] = new String(content,offset,len,"UTF-8");
		offset += len;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
	...
	public String getTypeDescription() {
		return "ACTIVITY_FIRST_PAGE2_RES";
	}
	...
	public int getLength() {
		...
		for(int i = 0 ; i < menuNames.length; i++){
			len += 2;
			try{
				len += menuNames[i].getBytes("UTF-8").length;
			}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
				throw new RuntimeException("unsupported encoding [UTF-8]",e);
			}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(menuNames.length);
			for(int i = 0 ; i < menuNames.length; i++){
				byte[] tmpBytes2 ;
				try{
				tmpBytes2 = menuNames[i].getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
