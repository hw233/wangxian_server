 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>sessionId.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>sessionId</td><td>byte[]</td><td>sessionId.length</td><td>数组实际长度</td></tr>
 ...
 * </table>
 */
public class REMOVE_PLAYER_REQ implements RequestMessage{
	...
	public REMOVE_PLAYER_REQ(){
	}

	public REMOVE_PLAYER_REQ(long seqNum,String username,String password,byte[] sessionId,String name){
		this.seqNum = seqNum;
	...
	public REMOVE_PLAYER_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		System.arraycopy(content,offset,sessionId,0,len);
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
