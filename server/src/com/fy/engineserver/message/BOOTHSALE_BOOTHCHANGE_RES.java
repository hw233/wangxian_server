 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>changeIndex.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>changeIndex</td><td>int[]</td><td>changeIndex.length</td><td>*</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>changedEntityId.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>changedEntityId</td><td>long[]</td><td>changedEntityId.length</td><td>*</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>changedNum.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>changedNum</td><td>int[]</td><td>changedNum.length</td><td>*</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>changedPerPrice.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>changedPerPrice</td><td>long[]</td><td>changedPerPrice.length</td><td>*</td></tr>
 ...
 * </table>
 */
public class BOOTHSALE_BOOTHCHANGE_RES implements ResponseMessage{
	...
	long playerId;
	int[] changeIndex;
	long[] changedEntityId;
	int[] changedNum;
	long[] changedPerPrice;
	MessageBoardInfo4Client[] messageBoardInfo4Client;

	public BOOTHSALE_BOOTHCHANGE_RES(){
	}

	public BOOTHSALE_BOOTHCHANGE_RES(long seqNum,long playerId,int[] changeIndex,long[] changedEntityId,int[] changedNum,long[] changedPerPrice,MessageBoardInfo4Client[] messageBoardInfo4Client){
		this.seqNum = seqNum;
		this.playerId = playerId;
		this.changeIndex = changeIndex;
		this.changedEntityId = changedEntityId;
		this.changedNum = changedNum;
		this.changedPerPrice = changedPerPrice;
		this.messageBoardInfo4Client = messageBoardInfo4Client;
	}

	public BOOTHSALE_BOOTHCHANGE_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		changeIndex = new int[len];
		for(int i = 0 ; i < changeIndex.length ; i++){
			changeIndex[i] = (int)mf.byteArrayToNumber(content,offset,4);
			offset += 4;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		changedEntityId = new long[len];
		for(int i = 0 ; i < changedEntityId.length ; i++){
			changedEntityId[i] = (long)mf.byteArrayToNumber(content,offset,8);
			offset += 8;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		changedNum = new int[len];
		for(int i = 0 ; i < changedNum.length ; i++){
			changedNum[i] = (int)mf.byteArrayToNumber(content,offset,4);
			offset += 4;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		changedPerPrice = new long[len];
		for(int i = 0 ; i < changedPerPrice.length ; i++){
			changedPerPrice[i] = (long)mf.byteArrayToNumber(content,offset,8);
			offset += 8;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
	...
	public String getTypeDescription() {
		return "BOOTHSALE_BOOTHCHANGE_RES";
	}
	...
	public int getLength() {
		...
		len += 8;
		len += 4;
		len += changeIndex.length * 4;
		len += 4;
		len += changedEntityId.length * 8;
		len += 4;
		len += changedNum.length * 4;
		len += 4;
		len += changedPerPrice.length * 8;
		len += 4;
		for(int i = 0 ; i < messageBoardInfo4Client.length ; i++){
			len += 2;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(changeIndex.length);
			for(int i = 0 ; i < changeIndex.length; i++){
				buffer.putInt(changeIndex[i]);
			}
			buffer.putInt(changedEntityId.length);
			for(int i = 0 ; i < changedEntityId.length; i++){
				buffer.putLong(changedEntityId[i]);
			}
			buffer.putInt(changedNum.length);
			for(int i = 0 ; i < changedNum.length; i++){
				buffer.putInt(changedNum[i]);
			}
			buffer.putInt(changedPerPrice.length);
			for(int i = 0 ; i < changedPerPrice.length; i++){
				buffer.putLong(changedPerPrice[i]);
			}
	 ...
	 *	改变后的物品单价
	 */
	public long[] getChangedPerPrice(){
		return changedPerPrice;
	}
	 ...
	 *	改变后的物品单价
	 */
	public void setChangedPerPrice(long[] changedPerPrice){
		this.changedPerPrice = changedPerPrice;
	}
