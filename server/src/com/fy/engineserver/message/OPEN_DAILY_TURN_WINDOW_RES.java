 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>turnNames.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>turnNames[0].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>turnNames[0]</td><td>String</td><td>turnNames[0].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>turnNames[1].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>turnNames[1]</td><td>String</td><td>turnNames[1].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>turnNames[2].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>turnNames[2]</td><td>String</td><td>turnNames[2].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td colspan=4>......... 重复</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>turnIds.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>turnIds</td><td>int[]</td><td>turnIds.length</td><td>*</td></tr>
 ...
 * </table>
 */
public class OPEN_DAILY_TURN_WINDOW_RES implements ResponseMessage{
	...
	public OPEN_DAILY_TURN_WINDOW_RES(){
	}

	public OPEN_DAILY_TURN_WINDOW_RES(long seqNum,String[] turnNames,int[] turnIds,TurnModel4Client info){
		this.seqNum = seqNum;
	...
	public OPEN_DAILY_TURN_WINDOW_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		turnNames = new String[len];
		for(int i = 0 ; i < turnNames.length ; i++){
			len = (int)mf.byteArrayToNumber(content,offset,2);
			offset += 2;
			if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
			turnNames[i] = new String(content,offset,len);
		offset += len;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		...
		for(int i = 0 ; i < turnIds.length ; i++){
			turnIds[i] = (int)mf.byteArrayToNumber(content,offset,4);
			offset += 4;
		}
	...
	public String getTypeDescription() {
		return "OPEN_DAILY_TURN_WINDOW_RES";
	}
	...
	public int getLength() {
		...
		for(int i = 0 ; i < turnNames.length; i++){
			len += 2;
			len += turnNames[i].getBytes().length;
		}
		len += 4;
		len += turnIds.length * 4;
		len += 4;
		len += 2;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(turnNames.length);
			for(int i = 0 ; i < turnNames.length; i++){
				byte[] tmpBytes2 ;
				tmpBytes2 = turnNames[i].getBytes();
				buffer.putShort((short)tmpBytes2.length);
				buffer.put(tmpBytes2);
			}
			buffer.putInt(turnIds.length);
			for(int i = 0 ; i < turnIds.length; i++){
				buffer.putInt(turnIds[i]);
			}
	 ...
	 *	转盘id
	 */
	public void setTurnIds(int[] turnIds){
		this.turnIds = turnIds;
	}
