 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>data.length</td><td>int</td><td>2</td><td>数组长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>data</td><td>byte[]</td><td>data.length</td><td>数组实际长度</td></tr>
 * </table>
 */
public class OBSERVER_GETGAMEDATA_RES implements ResponseMessage{
	...
	public OBSERVER_GETGAMEDATA_RES(long seqNum,byte result,String resultString,byte[] data){
		this.seqNum = seqNum;
		this.result = result;
	...
	public OBSERVER_GETGAMEDATA_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 64000) throw new Exception("array length ["+len+"] big than the max length [64000]");
		data = new byte[len];
	...
	public String getTypeDescription() {
		return "OBSERVER_GETGAMEDATA_RES";
	}
