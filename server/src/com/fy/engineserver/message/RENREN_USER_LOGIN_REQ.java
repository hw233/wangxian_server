 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>accessToken.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>accessToken</td><td>String</td><td>accessToken.length</td><td>字符串对应的byte数组</td></tr>
 ...
 * </table>
 */
public class RENREN_USER_LOGIN_REQ implements RequestMessage{
	...
	public RENREN_USER_LOGIN_REQ(long seqNum,String clientId,String accessToken,String channel){
		this.seqNum = seqNum;
	...
	public RENREN_USER_LOGIN_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		accessToken = new String(content,offset,len,"UTF-8");
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=accessToken.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put(tmpBytes1);
				try{
			tmpBytes1 = accessToken.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
