 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>querySkill.needLongExp.length</td><td>int</td><td>4个字节</td><td>对象属性为数组，数组的长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>querySkill.needLongExp</td><td>long[]</td><td>querySkill.needLongExp.length</td><td>*</td></tr>
 ...
 * </table>
 */
public class NEW_QUERY_CAREER_INFO_BY_ID_RES implements ResponseMessage{
	...
	public NEW_QUERY_CAREER_INFO_BY_ID_RES(){
	}

	public NEW_QUERY_CAREER_INFO_BY_ID_RES(long seqNum,SkillInfo querySkill){
		this.seqNum = seqNum;
		this.querySkill = querySkill;
	}

	public NEW_QUERY_CAREER_INFO_BY_ID_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		querySkill.setNeedLongExp(needLongExp_0003);
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
	...
	public String getTypeDescription() {
		return "NEW_QUERY_CAREER_INFO_BY_ID_RES";
	}
	...
	public int getLength() {
		...
		len += 4;
		len += querySkill.getNeedLongExp().length * 8;
		len += 4;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(querySkill.getNeedLongExp().length);
			long[] needLongExp_0008 = querySkill.getNeedLongExp();
