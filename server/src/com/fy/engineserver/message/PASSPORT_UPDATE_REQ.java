 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>birthday.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>birthday</td><td>String</td><td>birthday.length</td><td>字符串对应的byte数组</td></tr>
 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>province</td><td>long</td><td>8个字节</td><td>配置的长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>city</td><td>long</td><td>8个字节</td><td>配置的长度</td></tr>
...
public class PASSPORT_UPDATE_REQ implements RequestMessage{
	...
	public PASSPORT_UPDATE_REQ(long seqNum,long id,String username,String passwd,String oldpasswd,String email,String idcard,String realname,String authquestions,String authanswers,int sex,String birthday,String mobile,String qq,long province,long city,String phone,String address,String postcode){
		this.seqNum = seqNum;
		this.id = id;
	...
	public PASSPORT_UPDATE_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		...
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		birthday = new String(content,offset,len,"UTF-8");
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		...
		offset += len;
		province = (long)mf.byteArrayToNumber(content,offset,8);
		offset += 8;
		city = (long)mf.byteArrayToNumber(content,offset,8);
		offset += 8;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=birthday.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.putInt(sex);
				try{
			tmpBytes1 = birthday.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
