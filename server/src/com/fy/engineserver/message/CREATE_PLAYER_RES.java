 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>needBindMobile</td><td>boolean</td><td>1个字节</td><td>布尔型长度,0==false，其他为true</td></tr>
 * </table>
 */
public class CREATE_PLAYER_RES implements ResponseMessage{
	...
	public CREATE_PLAYER_RES(){
	}

	public CREATE_PLAYER_RES(long seqNum,byte result,String resultString,boolean needBindMobile){
		this.seqNum = seqNum;
		this.result = result;
	...
	public CREATE_PLAYER_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		offset += len;
		needBindMobile = mf.byteArrayToNumber(content,offset,1) != 0;
		offset += 1;
	}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put((byte)(needBindMobile==false?0:1));
		}catch(Exception e){
		 e.printStackTrace();
			buffer.reset();
	...
	/**
	 * 获取属性：
	 *	创建精灵的结果，0表示成功，其他表示失败：<br> 1表示名称已经存在，2表示以达到最大创建精灵的个数，其他值为未知错误
	 */
	...
	/**
	 * 获取属性：
	 *	仅当系统发现此角色为推荐账户创建的，并且没有绑定手机，需要客户端发上行绑定手机才返回true
	 */
