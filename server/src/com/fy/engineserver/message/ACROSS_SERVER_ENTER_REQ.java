 ...
 * </table>
 */
public class ACROSS_SERVER_ENTER_REQ implements RequestMessage{
	...
	public ACROSS_SERVER_ENTER_REQ(){
	}

	public ACROSS_SERVER_ENTER_REQ(long seqNum,String sessionId,String username,long playerId){
		this.seqNum = seqNum;
	...
	public ACROSS_SERVER_ENTER_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		sessionId = new String(content,offset,len);
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
	...
	public String getTypeDescription() {
		return "ACROSS_SERVER_ENTER_REQ";
	}
