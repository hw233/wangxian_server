 ...
 * <tr bgcolor="#FFFFFF" align="center"><td>smsbody.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>smsbody</td><td>String</td><td>smsbody.length</td><td>字符串对应的byte数组</td></tr>
 * </table>
 */
public class TAIWAN_USER_LOGIN_RES implements ResponseMessage{
	...
	public TAIWAN_USER_LOGIN_RES(long seqNum,byte result,String resultString,String authCode,String smsbody){
		this.seqNum = seqNum;
		this.result = result;
	...
	public TAIWAN_USER_LOGIN_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		offset += 2;
		if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
		smsbody = new String(content,offset,len,"UTF-8");
		offset += len;
	}
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=smsbody.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put(tmpBytes1);
				try{
			tmpBytes1 = smsbody.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
