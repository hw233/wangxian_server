 ...
 * </table>
 */
public class QUERY_COUNTRY_VOTE_RES implements ResponseMessage{
	...
	public QUERY_COUNTRY_VOTE_RES(){
	}

	public QUERY_COUNTRY_VOTE_RES(long seqNum,String totalVoteRateDes,String[] officerNames,String[] officerVoteRatesDes){
		this.seqNum = seqNum;
	...
	public QUERY_COUNTRY_VOTE_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			byte[] tmpBytes1;
				try{
			 tmpBytes1 = totalVoteRateDes.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
			...
			for(int i = 0 ; i < officerNames.length; i++){
				byte[] tmpBytes2 ;
				try{
				tmpBytes2 = officerNames[i].getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
			...
			for(int i = 0 ; i < officerVoteRatesDes.length; i++){
				byte[] tmpBytes2 ;
				try{
				tmpBytes2 = officerVoteRatesDes[i].getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
