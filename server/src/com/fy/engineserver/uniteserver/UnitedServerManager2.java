 ...
 * 
 */
public class UnitedServerManager2 implements MConsole {
	...
	private static UnitedServerManager2 instance;
	...
	private List<UnitedServerThread2> currentUnitedThreads = new ArrayList<UnitedServerThread2>();// 当前正在执行合并的Class
...
//		clearClasses.add(Faery.class);
	...
	public static UnitedServerManager2 getInstance() {
		return instance;
	}

	public List<UnitedServerThread2> getCurrentUnitedThreads() {
		return currentUnitedThreads;
	}

	public void setCurrentUnitedThreads(List<UnitedServerThread2> currentUnitedThreads) {
					...
					try {
						...
						for (int i = 0; i < id50.size(); i++) {
							if (i == id50.size() - 1) {
								...
								sq += "id=? or ";// + id50.get(i) + " or ";
							}
							objattr = Arrays.copyOf(objattr, objattr.length+1);
							objattr[objattr.length-1] = id50.get(i);
						}
						List<ArticleEntity> entitys = manager.query(ArticleEntity.class, sq, objattr, "", 1, id50.size() + 1);
	...
	private void getArticleEntitys() {
		try {
			...
			// 先去删除主服中的无用数据
			UnitedServerThread2 deleteArticleEntity = new UnitedServerThread2("deleteArticleEntity", null, null);
						...
						while (true) {
							{
								...
								while (itor.hasNext()) {
									...
									if (version != 0) {
										...
										itor.remove();
										if (UnitedServerManager2.logger.isInfoEnabled()) {
											UnitedServerManager2.logger.info("[移除已经存储的物品] [id:" + id + "]");
										}
									}
								}
							...
							for (ArticleEntity entity : entitys) {
								...
								if (UnitedServerManager2.logger.isInfoEnabled()) {
									UnitedServerManager2.logger.info("[通知底层存储物品] [notifyNewObject] [id:" + id + "]");
								}
					...
					while (noticeSimpleJpa.size() > 0) {// 通知底层完成，但是还没存储完毕
						...
						while (itor.hasNext()) {
							...
							if (version != 0) {
								...
								itor.remove();
								if (UnitedServerManager2.logger.isInfoEnabled()) {
									UnitedServerManager2.logger.info("[全部通知底层完成] [移除已经存储的物品] [id:" + id + "]");
								}
							}
						}
					}
	...
	public static boolean 只改名 = true;
	...
	public void uniteServerBegin(UnitedServer[] servers) throws Exception {
		...
		if (configSucc) {
			...
			{
				// 启动释放弱引用
				UnitedServerThread2 releaseReference = new UnitedServerThread2("releaseReference", null, null);
			...
			if (unitedChanagerName) {
				UnitedServerThread2 changeJiazuName = new UnitedServerThread2("changeJiazuName", null, null);
				Thread thread_changeJiazuName = new Thread(changeJiazuName, "合服-changeJiazuName");
				thread_changeJiazuName.start();

				UnitedServerThread2 changeZongpaiName = new UnitedServerThread2("changeZongpaiName", null, null);
				Thread thread_changeZongpaiName = new Thread(changeZongpaiName, "合服-changeZongpaiName");
				thread_changeZongpaiName.start();

				UnitedServerThread2 changePlayerName = new UnitedServerThread2("changePlayerName", null, null);
				Thread thread_changePlayerName = new Thread(changePlayerName, "合服-changePlayerName");
				thread_changePlayerName.start();
				
				UnitedServerThread2 changeZhanDuiName = new UnitedServerThread2("changeZhanDuiName", null, null);
				Thread thread_changeZhanDuiName = new Thread(changeZhanDuiName, "合服-changeZhanDuiName");
				thread_changeZhanDuiName.start();

				while (changeJiazuName.isRunning() || changeZongpaiName.isRunning() || changePlayerName.isRunning()|| changeZhanDuiName.isRunning()) {
					Thread.sleep(20000L);
					logger.error("[合服] [修改家族名字进行中:" + changeJiazuName.isRunning() + "] [宗派改名进行中:" + changeZongpaiName.isRunning() + "] [角色改名进行中:" + changePlayerName.isRunning() + "] [战队改名进行中:" + changeZhanDuiName.isRunning() + "] [cost:" + (System.currentTimeMillis() - startTime) + "] [总耗时:" + UnitedServerManager2.getTotolCost() + "]");
				}
			}
			if (只改名) {
				logger.warn("*******************************************************************************************");
			...
			UnitedServerThread2 getAllArticleEntitys = new UnitedServerThread2("getAllArticleEntitys", null, null);
			...
			logger.error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@[合服流程] [删除数据] [开始]");
			{
				if (removeData) {
					for (Class<?> clazz : clearClasses) {
						UnitedServerThread2 serverThread = new UnitedServerThread2("clear", new Class<?>[] { Class.class }, new Object[] { clazz });
						Thread thread = new Thread(serverThread, "删除数据-" + clazz.getSimpleName());
						thread.start();
					}
				}
			}
			logger.error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@[合服流程] [删除数据] [结束] [总耗时:" + UnitedServerManager2.getTotolCost() + "]");
			...
			logger.error("@@@@@@@@@@@@@@@@@@@@@@@@@@@@[合服流程] [合并数据] [开始]");
			{
				if (unitedData) {
					...
					while (leftUniteClasses.size() > 0) {
						if (currentUnitedThreads.size() < sameTimeRunUnitedClassNum) {
							...
							try {
								if (clazz.equals(ArticleEntity.class)) {
									UnitedServerThread2 getArticleEntitys = new UnitedServerThread2("getArticleEntitys", null, null);
									...
									UnitedServerThread2 serverThread = new UnitedServerThread2("merge", new Class<?>[] { Class.class }, new Object[] { clazz });
						...
						logger.warn("[数据合并中] [暂停20秒] [剩余class" + leftUniteClasses.size() + "] [正在执行线程数:" + currentUnitedThreads.size() + "] [正在执行的class] [" + getCurrentRunThreadInfo() + "] [总耗时:" + UnitedServerManager2.getTotolCost() + "]");
						Thread.sleep(20000L);
						removeDoneThread();
					}
					while (!allThreadOver(currentUnitedThreads)) {
						removeDoneThread(); // 在此线程中清理应该删除 但是未删除的已经运行完毕的线程
						logger.warn("[最后的表合并中] [每分钟检查一次] [" + getCurrentRunThreadInfo() + "] [总耗时:" + UnitedServerManager2.getTotolCost() + "]");
						Thread.sleep(60000);
					}

					//这里开始调用存储过程
					logger.warn("开始调用存储过程");
					String dburl = ((SimpleEntityManagerOracleImpl<ArticleEntity>)mainServer.getArticleManager()).getConnection().getMetaData().getURL();
					
					String password = "aafasdgAEWTWEddsWERr453";
					
					Connection conn = DriverManager.getConnection(dburl, "sys as sysdba",password);
					
					long beginCall = System.currentTimeMillis();
				    ArrayDescriptor descriptor = ArrayDescriptor.createDescriptor(
				    	      "VARRAY_TABLES",conn );
				    List<Class<?>> uniteClasses = new ArrayList<Class<?>>();
				    uniteClasses.addAll(this.uniteClasses);
				    boolean isRemove = uniteClasses.remove(ArticleEntity.class);
				    
				    if(isRemove)
				    {
				    	logger.warn("[不在存储过程中合道具] [从集合中删除它]");
				    }
				    
				    List<String> tableNames = new ArrayList<String>();
				    
					for (Class<?> clazz : uniteClasses) {
						if(clazz.isAssignableFrom(ArticleEntity.class))
						{
							logger.warn("[不在存储过程中合道具] [跳过]");
							continue;
						}
						SimpleEntity annotation = clazz.getAnnotation(SimpleEntity.class);
						if (annotation == null) {
					...
					ARRAY array = new ARRAY(descriptor, conn, tableNames.toArray());
					
					CallableStatement stmt = null;  
					stmt = conn.prepareCall("{call buchang(?)}");  
					stmt.setArray(1, array);
					stmt.execute();
					conn.commit();
					conn.close();
					
					logger.warn("[存储过程执行完毕] [cost:"+(System.currentTimeMillis()-beginCall)+"ms]");
	...
	private void removeDoneThread() {
		Iterator<UnitedServerThread2> itor = currentUnitedThreads.iterator();
		while (itor.hasNext()) {
			UnitedServerThread2 ut = itor.next();
			if (!ut.isRunning()) {
	...
	public String getCurrentRunThreadInfo() {
		...
		try {
			for (UnitedServerThread2 ut : currentUnitedThreads) {
				if (ut == null) {
					continue;
				}
				String name = "";
	...
	public boolean allThreadOver(List<UnitedServerThread2> list) {
		for (UnitedServerThread2 serverThread : list) {
			...
			for (UnitServerRiseActivity ra : riseActivities) {
				if (ra.getCountryId() == countryId && ra.getServerName().equals(serverName)) {
					if (ra.isOpen()) {
						...
						ActivitySubSystem.logger.warn("[合服活动] [崛起奖励] [当前服务器无对应活动] [" + serverName + "]");
					}
				} else {
					ActivitySubSystem.logger.warn("[合服活动] [崛起奖励] [当前服务器无对应活动] [" + serverName + "]");
				}
		...
		for (DailyLoginActivity activity : dailyLoginActivities) {
			if (activity != null) {
				if (activity.timeAndSeverFit(day, serverName)) {
					...
					logger.warn("[登录奖励] [失败] [" + player.getLogString() + "] [奖励数据:" + activity.getLogString() + "] [时间/类型不符] [day:" + day + "(" + activity.getDay().equals(day) + ")] [serverName:" + serverName + "(" + activity.getServers().contains(GameConstants.getInstance().getServerName()) + ")] [platForm:" + (PlatformManager.getInstance().isPlatformOf(activity.getPlatform())) + "]");
				}
			}
		}
	}
