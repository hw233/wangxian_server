...
public final class MultiLanguageTranslateManager {
	...
	public static MultiLanguageTranslateTool languageTranslateTool;
...
//				System.out.println(""+i+"/"+maxRow+"  " + cell.getStringCellValue());
				if (cell != null) {
					...
					if (cell != null) {
						...
						TaskManager.logger.warn("[class中] " + baseName + "--->" + translate);
					} else {
			...
			sheet = workbook.getSheetAt(1);// translate.java中的内容
			...
			for (int i = 0; i < maxRow; i++) {
				...
				if (cell != null) {
					...
					baseName = baseName.replace(separator_n, "\n");
					cell = row.getCell(1);
					if (cell != null) {
						...
						TaskManager.logger.warn("[xml中] " + baseName + "--->" + translate);
					} else {
						logger.error("[本地化] [含有空的value] [key:" + baseName + "]");
					}
				} else {
		...
		// System.out.println("webRoot" + MultiLanguageTranslateTool.webRoot);
		// Translate.allMapDisplayName = MultiLanguageTranslateTool.getSendClientBindata().toArray(new String[0]);
		// for (int i = 0; i < Translate.allMapDisplayName.length; i++) {
		// Translate.translateMapDisplayName[i] = languageTranslate(Translate.allMapDisplayName[i]);
		// }
		System.out.println("[本地化] [执行完毕] [是否需要本地化:" + needTranslate + "] [耗时:" + (System.currentTimeMillis() - start) + "ms]");
					...
					if (field != null) {
						...
						if (value != null) {
							...
							if (object.getClass().isArray()) {// 如果是数组
								logger.warn("[本地化] [数组] [" + object.getClass() + "] [" + field.getName() + "] [orig:--] [to:" + StringUtil.arrayToString(stringToArray(value),",") + "]");
								field.set(translate, stringToArray(value));
							} else {
								logger.warn("[本地化] [非数组] [" + object.getClass() + "] [" + field.getName() + "] [orig:"+(String)object+"] [to:" + value + "]");
