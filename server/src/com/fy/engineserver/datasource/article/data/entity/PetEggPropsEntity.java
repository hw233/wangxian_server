 ...
 */
@SimpleEntity
public class PetEggPropsEntity extends PropsEntity{
	
	
	public final static String[] skillSt = {Translate.技能一,Translate.技能二,Translate.技能三,Translate.技能四,Translate.技能五,Translate.技能六,Translate.技能七,Translate.技能八,Translate.技能九,Translate.技能十,
		Translate.技能11,Translate.技能12,Translate.技能13,Translate.技能14,Translate.技能15,Translate.技能16,Translate.技能17,Translate.技能18,Translate.技能19,Translate.技能20,};
	public static final long serialVersionUID = 23545547344354552L;

	public PetEggPropsEntity(){}
	
	public PetEggPropsEntity(long id){
		super(id);
	}

	// 任务类型1 生成一个特别次的宠物       普通类型0
	private byte eggType;
	@SimpleColumn(name="eggPetId")
...
//			ArticleManager.logger.warn("[查询物品出错][道具]["+this.getArticleName()+"][实体存在物种为空]");
			if(ArticleManager.logger.isWarnEnabled())
				ArticleManager.logger.warn("[查询物品出错][道具][{}][实体存在物种为空]", new Object[]{this.getArticleName()});
			return "";
		}
		if(getPetId()<=0){
			...
			PetFlyState stat = PetManager.getInstance().getPetFlyState(pet.getId(),"宠物蛋实体");
			...
			if(stat != null){
				if(stat.getFlyState() == 1){
					...
					isxian= true;
					lxPoint = stat.getLingXingPoint();
				}
			}
			if(!isxian){
				sb.append("<f color='0xFFFFFF' size='28'>").append(pet.getTrainLevel()).append("</f>");
			}else{
				sb.append("\n<f color='0x00FF00' size='28'>").append(Translate.灵性点数).append("：").append("</f>");
				sb.append("<f color='0xFFFFFF' size='28'>").append(lxPoint).append("</f>");
			}
			{
			...
			if (pet.getOrnaments()[0] > 0) {
				...
				if (aee != null) {
					...
					sb.append("\n");
//					sb.append("\n<f color='0x00FF00' size='28'>").append(Translate.宠物饰品).append(aee.getArticleName()).append("</f>");
					sb.append("\n");
				}
			}
			for(int i = 0; i<5; i++){
			...
			sb.append("\n");
			sb.append("\n<f color='0x00FF00' size='28'>").append(Translate.性格).append("：").append("</f>")
			.append("<f color='0xFFFFFF' size='28'>").append(PetManager.得到宠物性格名(pet.getCharacter())).append("</f>");
			sb.append("\n");
			
			if(isxian){
				if(stat != null && stat.getSkillId() > 0){
					PetFlySkillInfo info = PetManager.getInstance().petFlySkills.get(new Integer(stat.getSkillId()));
					if(info != null && info.getName() != null){
						sb.append("\n<f color='0x00FF00' size='28'>").append(Translate.飞升技能).append("：").append("</f>")
						.append("<f color='0xFFFFFF' size='28'>").append(info.getName()).append("</f>").append("\n");
					}
				}
			}
			...
			if(pet.getSkillIds() != null && pet.getSkillIds().length > 0){
				...
				if(cm != null){
					...
					for(int skillId : pet.getSkillIds()){
						...
						if(skill != null){
							sb.append("\n<f color='0x00FF00' size='28'>").append(Translate.基础+PetEggPropsEntity.skillSt[i]).append("</f>");
							sb.append(skill.getName());
							sb.append(" Lv"+
									(
											(activeSkillLevels == null || i>=activeSkillLevels.length) ? 1 :
										activeSkillLevels[i])
										);
							has = true;
							++i;
