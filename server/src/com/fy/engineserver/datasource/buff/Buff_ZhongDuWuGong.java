 ...
 */
@SimpleEmbeddable
public class Buff_ZhongDuWuGong extends Buff implements AbnormalStateBuff{
		...
		if(heartBeatStartTime - lastExeTime >= LastingTime){
			...
			if(owner instanceof Player){
				...
				if(p.isImmunity()){
					...
					int hp = 0;
					BuffTemplate_ZhongDuWuGong bt = (BuffTemplate_ZhongDuWuGong)this.getTemplate();
					if(bt.modulus != null && bt.modulus.length > getLevel()){
						if(getCauser() instanceof Player){
							...
							hp = (int)(pet.getPhyAttack()*bt.modulus[getLevel()]/100);
						}
					}
