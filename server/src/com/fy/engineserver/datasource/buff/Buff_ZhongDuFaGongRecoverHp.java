 ...
 */
@SimpleEmbeddable
public class Buff_ZhongDuFaGongRecoverHp extends Buff implements AbnormalStateBuff{
		...
		if (heartBeatStartTime - lastExeTime >= LastingTime) {
			...
			if (owner instanceof Player) {
				...
				if (p.isImmunity()) {
					...
					BuffTemplate_ZhongDuFaGongRecoverHp bt = (BuffTemplate_ZhongDuFaGongRecoverHp) this.getTemplate();
					...
					if (this.getCauser() != null) {
						...
						int tempHp = this.getCauser().getMaxHP() - this.getCauser().getHp();
						if (tempHp > 0) {
							tempHp = hp > tempHp ? tempHp : hp;
						}
						if (tempHp > 0) {
							if (!FighterTool.isCanNotIncHp(getCauser())) {
								this.getCauser().setHp(this.getCauser().getHp() + tempHp);
								if (this.getCauser() instanceof Player) {
									NOTIFY_EVENT_REQ req2 = new NOTIFY_EVENT_REQ(GameMessageFactory.nextSequnceNum(), (byte) 0, this.getCauser().getId(), (byte) Event.HP_INCREASED, tempHp);
									((Player) this.getCauser()).addMessageToRightBag(req2);
									p.addMessageToRightBag(req2);
								}
								if (Skill.logger.isDebugEnabled()) Skill.logger.debug("[可以回血状态] [Buff_ZhongDuFaGongRecoverHp] [" + p.getLogString() + "] [" + this.getCauser().getName() + "] [血] [" + tempHp + "]");
							} else {
								if (Skill.logger.isDebugEnabled()) Skill.logger.debug("[无法回血状态] [屏蔽Buff_ZhongDuFaGongRecoverHp] [" + p.getLogString() + "] [血]");
							}
						}
					...
					if (this.getCauser() != null) {
						...
						if (tempHp > 0) {
							if (!FighterTool.isCanNotIncHp(getCauser())) {
								...
								if (Skill.logger.isDebugEnabled()) Skill.logger.debug("[回血状态] [Buff_ZhongDuFaGongRecoverHp] [Sprite] [" + s.getName() + "] [血:" + tempHp + "]");
							} else {
								if (Skill.logger.isDebugEnabled()) Skill.logger.debug("[无法回血状态] [屏蔽Buff_ZhongDuFaGongRecoverHp] [Sprite] [" + s.getName() + "] [血]");
							}
						}
					} else {
						Skill.logger.debug("Buff_ZhongDuFaGong.heartbeat: getCauser is null");
