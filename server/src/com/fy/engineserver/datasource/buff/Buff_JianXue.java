 ...
 */
@SimpleEmbeddable
public class Buff_JianXue extends Buff{
		...
		if(heartBeatStartTime - lastExeTime >= LastingTime){
			...
			if(owner instanceof Player){
				...
				if(p.isImmunity()){
					...
					int hp = 0;
					BuffTemplate_JianXue bt = (BuffTemplate_JianXue)this.getTemplate();
...
//					}
					...
					if(hudun > 0){
						...
						p.causeDamage(getCauser(), hp, 10,Fighter.DAMAGETYPE_SPELL);
