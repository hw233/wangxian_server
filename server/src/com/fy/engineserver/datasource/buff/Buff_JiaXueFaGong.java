 ...
 */
@SimpleEmbeddable
public class Buff_JiaXueFaGong extends Buff{
	...
	public void heartbeat(Fighter owner,long heartBeatStartTime, long interval, Game game){
		...
		if(heartBeatStartTime - lastExeTime >= LastingTime){
			...
			if(owner instanceof Player){
				Player p = (Player)owner;
				if(p.isCanNotIncHp()){
					Skill.logger.debug("[无法回血状态] [屏蔽Buff_JiaXueFaGong] [" + p.getLogString() + "] [血]");
					return;
				}
				p.setCure(true);
				int hp = 0;
				BuffTemplate_JiaXueFaGong bt = (BuffTemplate_JiaXueFaGong)this.getTemplate();
				...
				if(s.isCanNotIncHp()){
					Skill.logger.debug("[无法回血状态] [屏蔽Buff_JiaXueFaGong] [Sprite] [血]");
					return;
				}
				int hp = 0;
				BuffTemplate_JiaXueFaGong bt = (BuffTemplate_JiaXueFaGong)this.getTemplate();
