 ...
 */
@SimpleEmbeddable
public class Buff_JianLanWuGong extends Buff{
		...
		if(heartBeatStartTime - lastExeTime >= LastingTime){
			...
			if(owner instanceof Player){
				...
				if(p.isImmunity()){
					...
					int mp = 0;
					BuffTemplate_JianLanWuGong bt = (BuffTemplate_JianLanWuGong)this.getTemplate();
					if(bt.modulus != null && bt.modulus.length > getLevel()){
						if(getCauser() instanceof Player){
							//除以100是因为在编辑器里编辑的系数为整数，约定除以100为真正的系数
							mp = (int)(((Player)getCauser()).getPhyAttack()*bt.modulus[getLevel()]/100);	
						}else if(getCauser() instanceof Monster){
							Monster m = (Monster)getCauser();
							mp = (int)(m.getPhyAttack()*bt.modulus[getLevel()]/100);
						}else if(getCauser() instanceof NPC){
							NPC n = (NPC)getCauser();
							mp = (int)(n.getPhyAttack()*bt.modulus[getLevel()]/100);
						}
					}
					if(p.getMp() > mp ){
						p.setMp(p.getMp() - mp);
