 ...
 */
@SimpleEmbeddable
public class Buff_ShunJiaXueZhi extends Buff{
	...
	public void start(Fighter owner){
		if(owner instanceof Player){
			...
			if(p.isCanNotIncHp()){
				if(Skill.logger.isDebugEnabled())Skill.logger.debug("[无法回血状态] [屏蔽Buff_ShunJiaXueZhi] [" + p.getLogString() + "] [血]");
				return;
			}
			BuffTemplate_ShunJiaXueZhi bt = (BuffTemplate_ShunJiaXueZhi)this.getTemplate();
			if(bt.hpB != null && bt.hpB.length > getLevel()){
				hp = bt.hpB[getLevel()];
			}
			...
			if(p.isCanNotIncHp()){
				if(Skill.logger.isDebugEnabled())Skill.logger.debug("[无法回血状态] [屏蔽Buff_ShunJiaXueZhi] [Sprite] [血]");
				return;
			}
			BuffTemplate_ShunJiaXueZhi bt = (BuffTemplate_ShunJiaXueZhi)this.getTemplate();
			if(bt.hpB != null && bt.hpB.length > getLevel()){
