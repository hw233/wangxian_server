...
@SimpleEntity
public class FateActivityAbroad extends FateActivity {

	public FateActivityAbroad(){
	
	}
	
	public FateActivityAbroad(long id){
		super(id);
	}
...
//				String keyString = player.getName()+"同意了你的请求,请到凤栖梧桐去找她";
				...
				FateManager.logger.error("[同意活动异常] ["+player.getLogString()+"] ",e);
			}
		}
		return false;
	}
	...
	public void addExp(Player invite,Player invited,boolean addexp1,boolean addexp2){
		...
		if(FateManager.logger.isWarnEnabled()){
			FateManager.logger.warn("[国外仙缘加经验] [邀请人:"+(invite != null ?invite.getLogString() : "")+"] [被邀请人:"+(invited != null ?invited.getLogString():"")+"]");
		}
