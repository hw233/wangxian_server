...
@SimpleEntity
public class BeerActivityAbroad extends FateActivity {

	public BeerActivityAbroad(){
		
	}
	public BeerActivityAbroad(long id){
		super(id);
	}
	
	
//	public String getKeyString(Player invite,Player invited){
////		String keyString = "(国外论道)枭雄四起,八方豪杰，恭喜你找到有缘人"+invited.getName()+",你想送他什么？";
...
//				String keyString = player.getName()+"同意了你的请求,请到凤栖梧桐去找他";
				...
				FateManager.logger.error("[被邀请玩家同意国外论道] [被邀请人:"+player.getLogString()+"] [邀请人id:"+inviteId+"]",e);
			}
		}
		return false;
	}
	...
	public void addExp(Player invite,Player invited,boolean addexp1,boolean addexp2){
		...
		if(FateManager.logger.isWarnEnabled()){
			FateManager.logger.warn("[国外论道加经验] [邀请人:"+(invite != null ?invite.getLogString() : "")+"] [被邀请人:"+(invited != null ?invited.getLogString():"")+"]");
		}
	}
	
	public long getExp(Player player){
		
		Map<Integer, Long> map = FateManager.getInstance().getExp2Map();
