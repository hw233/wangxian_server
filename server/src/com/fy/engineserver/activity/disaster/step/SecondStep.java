	...
	public void refreshNPC(Game game) {
		...
		try {
			...
			for (int i=0; i<module.getFireNPCPoints().size(); i++) {
				...
				NPC npc = MemoryNPCManager.getNPCManager().createNPC(module.getSecondStepNPCId()[0]);
				npc.setX(point[0]);
				npc.setY(point[1]);
				npc.setBornPoint(new Point(npc.getX(), npc.getY()));
				game.addSprite(npc);
			...
			for (int i=0; i<module.getFireNPCOuterPoints().size(); i++) {
				...
				NPC npc = MemoryNPCManager.getNPCManager().createNPC(module.getSecondStepNPCId()[1]);
				npc.setX(point[0]);
				npc.setY(point[1]);
				npc.setBornPoint(new Point(npc.getX(), npc.getY()));
				game.addSprite(npc);
			...
			if (DisasterManager.logger.isInfoEnabled()) {
				DisasterManager.logger.info("[第二阶段刷新NPC] [异常]", e);
			}
		}
	}
