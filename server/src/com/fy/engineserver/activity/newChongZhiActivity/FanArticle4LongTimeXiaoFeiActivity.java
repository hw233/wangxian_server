...
public class FanArticle4LongTimeXiaoFeiActivity extends NewXiaoFeiActivity {
	
	public FanArticle4LongTimeXiaoFeiActivity(int configID, int[] xiaofeiType,
			String name, int platform, String startTime, String endTime,
			String[] serverNames, String[] unServerNames, String mailTitle,
			String mailMsg, String[] parameters) throws Exception {
		super(configID, xiaofeiType, name, platform, startTime, endTime, serverNames,
				unServerNames, mailTitle, mailMsg, parameters);
	}

	public static String DISK_KEY = "XFfanArticle";
	public HashMap<Long, Long> playerMoneys = new HashMap<Long, Long>();	//玩家充值金额
	public static String DISK_KEY1 = "1XFfanArticle";
	public long sendFanLiTime;												//返利时间

	private long needMoney;						//金额限制
	...
	public void loadDiskCache() {
		try{
			...
			logger.error("返物品 loadDiskCache", e);
		}
	}
	...
	public void doActivity(Player player, long yinzi) {
		try{
			...
			diskcache.put(DISK_KEY + getConfigID(), playerMoneys);
			logger.warn("[返物品消费] [{}] [{}] [{}] [{}] [{}]", new Object[]{player.getLogString(), yinzi, oldMoney, getConfigID(), getName()});
		}catch(Exception e) {
			logger.error("返物品消费 heatbeat", e);
		}
	}
						...
						for (Long playerID : playerMoneys.keySet()) {
							try{
								...
								if (money.longValue() >= getNeedMoney()) {
									...
									if (a == null) {
										logger.warn("[消费返物品不存在] [{}] [{}] [{}]", new Object[]{playerID, getConfigID(), getFanArticleNames()[index]});
										return;
									}
									...
									logger.warn("[消费返物品奖] {} [{}] [{}] [{}]", new Object[]{getConfigID() + " " + getName(), playerID, getFanArticleNames()[index], index});
								}
							}catch(Exception e) {
								logger.error("消费返物品奖出错", e);
							}
						}
						logger.warn("[消费返物品一次] {} [{}] [{}]", new Object[]{getLogString(), playerMoneys.size(), index});
					}
				}
	...
	public void creatParameter(String[] parameters) {
		try{
			//需要金额
			needMoney = Long.parseLong(parameters[0]);
			//返利时间
			fanliTimes = parameters[1].split(",");
			...
			//返利间隔
			String[] fanliSpaceString = parameters[2].split(",");
			...
			//返物品名字
			String[] fanArticleNameString = parameters[3].split(",");
			setFanArticleNames(fanArticleNameString);
			//返物品个数
			String[] fanArticleNumString = parameters[4].split(",");
			...
			//返物品绑定
			String[] fanArticleBindString = parameters[5].split(",");
			...
			logger.error("返物品 creatParameter", e);
		}
	}
	...
	public void cleanActivityData() {
		if (playerMoneys.size() > 0 || sendFanLiTime != 0L) {
			logger.warn("[消费返物品数据清除] {} [{}] [{}]", new Object[]{getLogString(), playerMoneys.size(), sendFanLiTime});
			playerMoneys.clear();
			diskcache.put(DISK_KEY + getConfigID(), playerMoneys);
			sendFanLiTime = 0L;
			diskcache.put(DISK_KEY1 + getConfigID(), sendFanLiTime);
