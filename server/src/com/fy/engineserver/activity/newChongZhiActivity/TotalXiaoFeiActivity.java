...
/**
 *	累计消费活动，玩家在活动期间内累计消费满设定金额，得到奖励
 */
public class TotalXiaoFeiActivity extends NewXiaoFeiActivity {
	
	public static String DISK_KEY = "leijiXiaoFei";
	...
	public TotalXiaoFeiActivity(int configID, int[] xt, String name, int platform,
	...
	public void loadDiskCache() {
		try{
			...
			logger.error("累计消费 loadDiskCache", e);
		}
	}
	...
	public void doActivity(Player player, long yinzi) {
		try{
			...
			diskcache.put(DISK_KEY + getConfigID(), playerMoneys);
			if (!isGive && oldMoney.longValue() >= getNeedMoney()) {
				sendReward(player);
			}else {
				logger.warn("[累计消费] {} [{}] [{}] [{}]", new Object[]{getLogString(), player.getLogString(), yinzi, oldMoney});
			}
		}catch(Exception e) {
			logger.error("累计消费 doActivity", e);
		}
	}
	...
	public void sendReward(Player player) {
		try{
			...
			logger.warn("[累计消费奖] {} [{}]", new Object[]{getLogString(), player.getLogString()});
		}catch(Exception e) {
			logger.error("累计消费奖出错", e);
		}
	}
	...
	public void creatParameter(String[] parameters) {
		try{
			...
			logger.error("累计消费 creatParameter", e);
		}
	}
