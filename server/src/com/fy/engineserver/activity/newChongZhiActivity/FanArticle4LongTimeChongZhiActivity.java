...
public class FanArticle4LongTimeChongZhiActivity extends NewMoneyActivity {
	
	public static String DISK_KEY = "fanArticle4LT";
	public HashMap<Long, Long> playerMoneys = new HashMap<Long, Long>();	//玩家充值金额
	public static String DISK_KEY1 = "1fanArticle4LT";
	...
	public FanArticle4LongTimeChongZhiActivity(int configID, String name,
	...
	public void doActivity(Player player, long yinzi) {
		try{
			...
			diskcache.put(DISK_KEY + getConfigID(), playerMoneys);
			logger.warn("[返物品充] [{}] [{}] [{}] [{}] [{}]", new Object[]{player.getLogString(), yinzi, oldMoney, getConfigID(), getName()});
		}catch(Exception e) {
			logger.error("返物品 heatbeat", e);
		}
	}
						...
						for (Long playerID : playerMoneys.keySet()) {
							try{
								...
								if (money.longValue() >= getNeedMoney()) {
									...
									if (a == null) {
										logger.warn("[返物品不存在] [{}] [{}] [{}]", new Object[]{playerID, getConfigID(), getFanArticleNames()[index]});
										return;
									}
									...
									logger.warn("[返物品奖] {} [{}] [{}] [{}]", new Object[]{getConfigID() + " " + getName(), playerID, getFanArticleNames()[index], index});
								}
							}catch(Exception e) {
								logger.error("返物品奖出错", e);
							}
						}
						logger.warn("[返物品一次] {} [{}] [{}]", new Object[]{getLogString(), playerMoneys.size(), index});
					}
				}
	...
	public void creatParameter(String[] parameters) {
		try{
			...
			needMoney = Long.parseLong(fanliTimes[0]);
			fanliDatas = new long[fanliTimes.length-1];
			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			for (int i = 0; i < fanliDatas.length; i++) {
				fanliDatas[i] = format.parse(fanliTimes[i+1]).getTime();
			}
			//返利间隔
	...
	public void cleanActivityData() {
		if (playerMoneys.size() > 0 || sendFanLiTime != 0L) {
			logger.warn("[返物品数据清除] {} [{}] [{}]", new Object[]{getLogString(), playerMoneys.size(), sendFanLiTime});
			playerMoneys.clear();
			diskcache.put(DISK_KEY + getConfigID(), playerMoneys);
			sendFanLiTime = 0L;
			diskcache.put(DISK_KEY1 + getConfigID(), sendFanLiTime);
