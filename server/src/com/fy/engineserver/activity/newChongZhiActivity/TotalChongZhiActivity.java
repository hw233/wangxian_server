...
/**
 *	累计充值活动，玩家在活动期间内累计充值满设定金额，得到奖励
 */
public class TotalChongZhiActivity extends NewMoneyActivity {
	
	public static String DISK_KEY = "leiji";
	...
	public TotalChongZhiActivity(int configID, String name, int platform,
	...
	public void doActivity(Player player, long yinzi) {
		try{
			...
			if (!isGive && oldMoney.longValue() >= getNeedMoney()) {
				...
				logger.warn("[累计充] {} [{}] [{}] [{}]", new Object[]{getLogString(), player.getLogString(), yinzi, oldMoney});
			}
		}catch(Exception e) {
	...
	public void sendReward(Player player) {
		try{
			...
			logger.warn("[累计奖] {} [{}]", new Object[]{getLogString(), player.getLogString()});
		}catch(Exception e) {
			logger.error("累计奖出错", e);
		}
	}
