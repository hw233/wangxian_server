 ...
 * <tr bgcolor="#00FFFF" align="center"><td>请求消息</td><td>类型</td><td>响应消息</td><td>类型</td><td>说明</td></tr> * <tr bgcolor="#FFFFFF" align="center"><td>ICON_REQ</td><td>0x00000001</td><td>ICON_RES</td><td>0x80000001</td><td>请求技能图标数据</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>BUFF_REQ</td><td>0x00000002</td><td>BUFF_RES</td><td>0x80000002</td><td>请求BUFF数据</td></tr>
 * </table>
 */
public class CareerAndSkillFactory extends AbstractMessageFactory {
	...
	protected static CareerAndSkillFactory self;

	public static CareerAndSkillFactory getInstance(){
		if(self != null){
			return self;
		}
		synchronized(CareerAndSkillFactory.class){
			if(self != null) return self;
			self = new CareerAndSkillFactory();
		}
		return self;
	}
	...
	public Message newMessage(byte[] messageContent,int offset,int size)
		...
		try{
			if(type == 0x00000001L){
				try {
					return new ICON_REQ(sn,messageContent,offset,end - offset);
				} catch (Exception e) {
					throw new MessageFormatErrorException("construct ICON_REQ error:" + e.getClass() + ":" + e.getMessage());
				}
			}else if(type == 0x80000001L){
				try {
					return new ICON_RES(sn,messageContent,offset,end - offset);
				} catch (Exception e) {
					throw new MessageFormatErrorException("construct ICON_RES error:" + e.getClass() + ":" + e.getMessage());
				}
