 ...
 * 
 */
public class ZhiFuBaoWapChargeMode extends UrlArgsReturnChargeMode {

	public ZhiFuBaoWapChargeMode(String modeName) {
		super(modeName);
	}
	...
	public synchronized String doCharge(Player player, Passport passport, String clientChannel, String os, ChargeMoneyConfigure chargeMoneyConfigure, String... parms) {
		...
		// String res = "";
		// try {
		// res = bossClientService.savingForChannelUser(player.getUsername(), 1, getModeName(), parms[0], parms[1], (int) chargeMoneyConfigure.getDenomination(),
		// gameConstants.getServerName(), clientChannel, os);
		// } catch (Exception e) {
		// notice = "提交异常,请联系GM!";
		// ChargeManager.logger.error(player.getLogString() + "[充值异常] [doCharge] [卡类型:" + getModeName() + "] [充值面额ID:" + chargeMoneyConfigure.getId() + "] [渠道:" + clientChannel +
		// "] [平台:" + os + "] [parms:" + Arrays.toString(parms) + "]", e);
		// }
		// if (ChargeManager.logger.isWarnEnabled()) {
		// ChargeManager.logger.warn(player.getLogString() + "[充值提交] [doCharge] [卡类型:" + getModeName() + "] [充值面额ID:" + chargeMoneyConfigure.getId() + "] [渠道:" + clientChannel +
		// "] [平台:" + os + "] [parms:" + Arrays.toString(parms) + "] [结果:" + notice + "]");
		// }
		//
