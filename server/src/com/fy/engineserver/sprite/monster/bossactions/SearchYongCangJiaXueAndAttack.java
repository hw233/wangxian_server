...
/**
 * 检测前方多少范围内是否有咏唱加血且没有无敌BUFF的玩家，
 * 有就进行技能攻击
 *  
 * 
 *
 */
public class SearchYongCangJiaXueAndAttack implements BossAction{
			...
			if(fs[i] instanceof Player){
				...
				if(e != null && e.getStatus() == ActiveSkillEntity.STATUS_INTONATE){
					if( (boss.getDirection() == LivingObject.LEFT && p.getX() <= boss.getX())
						...
						if( e.getSkill() instanceof SkillWithoutTraceAndOnTeamMember){
							SkillWithoutTraceAndOnTeamMember sk = (SkillWithoutTraceAndOnTeamMember)e.getSkill();
							if(sk.getRangeType() == 0 || sk.getRangeType() == 1 || sk.getRangeType() == 3){
