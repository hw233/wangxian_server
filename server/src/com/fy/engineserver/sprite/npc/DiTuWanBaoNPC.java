 ...
 *
 */
public class DiTuWanBaoNPC extends NPC implements Cloneable{
	...
	public void heartbeat(long heartBeatStartTime, long interval, Game game){
		...
		if(heartBeatStartTime > lastHeartbeatTime + 2000){
			...
			if(game.gi.时间地图类型 != GameMap.TIME_MAP_TYPE_WANBAO){
				game.gi.时间地图类型 = GameMap.TIME_MAP_TYPE_WANBAO;
			}
			...
			if(los != null){
				for(LivingObject lo : los){
					if(lo instanceof Player){
						...
						if(heartBeatStartTime >= p.getLastInWanBaoDiTuTime() + 加时间的间隔分钟时长*60000){
							p.updateLastInWanBaoDiTuTime(heartBeatStartTime, 加时间的间隔分钟时长);
						}

						//没有进入时间或等级不够踢出
						if(!p.isValidInWanBaoDiTuTime() || p.getMainSoul().getGrade() < MIN_LEVEL){
	 ...
	 */
	public Object clone() {
		DiTuWanBaoNPC p = new DiTuWanBaoNPC();
