 ...
 *
 */
public class NPCFightingAgent {

	static java.util.Random random = new java.util.Random();
	
	public NPCFightingAgent(FightableNPC f){
		fighter = f;
	}
	 ...
	 * 攻击者
	 */
	FightableNPC fighter;
	
	int level = 0;
	
	private int lastHP = -1;
	private int sameHPCount = 0;
	private long holdingTime = -1;
	private int sameTargetPathFindingTimes = 0;
	...
	public FightableNPC getFighter() {
		return fighter;
	}
	...
	public void heartbeat(long heartBeatStartTime,Game g) {
		...
		if( target != null && fighter != null && (fighter.getX() < activePoint.x - fighter.getPursueWidth()/2 || fighter.getX() > activePoint.x + fighter.getPursueWidth()/2
				|| fighter.getY() < activePoint.y - fighter.getPursueHeight()/2 || fighter.getY() > activePoint.y + fighter.getPursueHeight()/2)){
			if (!(fighter instanceof AppointedAttackNPC)) {
				targetDisappear = true;
			}
		}
	...
	protected void fight(long heartBeatStartTime,Game g){
		...
		ActiveSkillAgent agent = fighter.getActiveSkillAgent();
		...
		if(agent.getExecuting() == null && agent.getWaitting() == null
			...
			if(result == ActiveSkill.TARGET_TOO_FAR){
				if(((LivingObject)fighter).getMoveTrace() == null && fighter.isStun() == false && fighter.isHold() == false){
					pathFinding(g,target.getX(),target.getY());
					holdingTime = heartBeatStartTime + 1000;
				}
			}else if(result == 0){
				if (hitCount < 5) {
					if(fighter instanceof LivingObject){
						agent.usingSkill(skill,level,target, (int)target.getX(),(int) target.getY(),new byte[]{target.getClassType()}, new long[]{target.getId()},((LivingObject)fighter).getDirection());
					}else{
						agent.usingSkill(skill,level,target, (int)target.getX(),(int) target.getY(),new byte[]{target.getClassType()}, new long[]{target.getId()},(byte)0);
					}
	...
	protected void pathFinding(Game de,int dx,int dy){
		...
		if(L > distance && de.getGameInfo().navigator.isVisiable(s.x,s.y,e.x,e.y)){
			...
			if(fighter.getSpeed() > 0){
				...
				MoveTrace path = new MoveTrace(roadLen,ps,(long)(com.fy.engineserver.gametime.SystemTime.currentTimeMillis() + (long)L1*1000/1));
				fighter.setMoveTrace(path);
			}
			...
			if(fighter.getSpeed() > 0){
				...
				MoveTrace path = new MoveTrace(roadLen,ps,com.fy.engineserver.gametime.SystemTime.currentTimeMillis() + totalLen*1000/1);
				fighter.setMoveTrace(path);
			}
		}
	}
