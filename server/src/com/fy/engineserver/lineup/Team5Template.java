 ...
 *
 */
public class Team5Template implements TeamTemplate {
	
	public static final int ROLE_NUM_NEED_奶妈 = 1;
	
	public static final int ROLE_NUM_NEED_坦克 = 1;
	
	public static final int ROLE_NUM_NEED_输出 = 3;
...
//				LineupManager.log.debug("[尝试组合] [5人组合] ["+seq_id+"] [目前组合:"+str+"] ["+luplayer.player.getId()+"] ["+luplayer.player.getName()+"] ["+TeamRole.getRoleDesp(luplayer.teamrole)+"] ["+teamplayers.size()+"]");
					if (LineupManager.log.isDebugEnabled()) LineupManager.log.debug("[尝试组合] [5人组合] [{}] [目前组合:{}] [{}] [{}] [{}] [{}]", new Object[]{seq_id,str,luplayer.player.getId(),luplayer.player.getName(),TeamRole.getRoleDesp(luplayer.teamrole),teamplayers.size()});
			}
			return true;
		}
		return false;
