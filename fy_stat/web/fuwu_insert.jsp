...
<%@ page
  ...
  <%
    ...
    if(flag!=null){
    BufferedReader bufferedreader=new BufferedReader(new InputStreamReader(new  FileInputStream("D:\\qq.txt"),"UTF-8")); 
	      long now=System.currentTimeMillis();
    	  Connection con = null;
           con = DataSourceManager.getInstance().getConnection();
           con.setAutoCommit(false);
           PreparedStatement ps          = con.prepareStatement("insert into fuwu_linshi v (id,name,remark) values(?,?,?)");
		   try {
		    int i =-1;
		    long num=1L;
         ...
         while((instring=bufferedreader.readLine())!=null){
			ps.setLong(1, num++);
			ps.setString(2, instring.replaceAll("\t","").replaceAll("\r","").trim());
			ps.setString(3, "");
