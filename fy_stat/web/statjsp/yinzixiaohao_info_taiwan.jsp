	...
	**/
	          ...
	          Transaction_SpecialManagerImpl transaction_SpecialManager = Transaction_SpecialManagerImpl.getInstance();
               ...
               String sql_YinDingChanChu="select t.createdate,'dd' fenqu,'银锭产出' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t,dt_reasontype r "
                            ...
                            +"'邮件获取后台发送-系统奖励！','邮件获取后台发送-系统邮件','金钱道具使用碎銀子','金钱道具使用銀兩','金钱道具使用銀塊',"
	           ...
	           +",'JIEYI_COST加入结义消耗','创建宗派创建宗派','购买家族徽章购买家族徽章','封印任务封印任务','结婚婚礼规模白首同心-5-一心一意對戒(男)~一心一意對戒(女)~'"
	           +",'结婚婚礼规模琴瑟和弦-10-三生缱绻对戒(男)~三生缱绻对戒(女)~','结婚婚礼规模福禄鸳鸯-20-比翼双飞对戒(男)~比翼双飞对戒(女)~'"
	           +",'结婚婚礼规模天造地設-50-三生相守對戒(男)~三生相守對戒(女)~','结婚婚礼规模龙凤呈祥-100-山盟海誓对戒(男)~山盟海誓对戒(女)~'"
	           ...
	           +"when r.name in ('交付镖车交付镖车','金钱道具使用鏢玉(白)','金钱道具使用鏢玉(綠)','金钱道具使用鏢玉(藍)','金钱道具使用鏢玉(紫)','金钱道具使用鏢玉(橙)','金钱道具使用镖银(白)','金钱道具使用镖银(绿)','金钱道具使用鏢銀(藍)','金钱道具使用镖银(紫)','金钱道具使用镖银(橙) ','击杀奸细击杀奸细','金钱道具使用鏢金(白)','金钱道具使用鏢金(綠)') then -t.money "
	           ...
	           +"when r.name in ('交付镖车交付镖车','金钱道具使用鏢玉(白)','金钱道具使用鏢玉(綠)','金钱道具使用鏢玉(藍)','金钱道具使用鏢玉(紫)','金钱道具使用鏢玉(橙)','金钱道具使用镖银(白)','金钱道具使用镖银(绿)','金钱道具使用镖银(蓝)','金钱道具使用镖银(紫)','金钱道具使用镖银(橙)','击杀奸细击杀奸细','金钱道具使用鏢金(白)','金钱道具使用鏢金(綠)') then -t.money "
	           +"when r.name in ('竞标竞标','追加资金追加资金') then t.money*0.03 "
	           +"when r.name in ('交易交易给他人','摆摊在摆摊上购买物品') then round(t.money/21) "
	           +"when r.name in ('拍卖竞拍费拍卖竞拍费') then round(t.money*0.2) "
	           +"when r.name in ('求购求购预存费用和手续费') then round(t.money/11)  end) money " 
         ...
         if("1".equals(displayType)){
			...
			for( int g=0;g< ls_YinDingXiaoHao.size();g++){
			   ...
			   if(k<dayList.size()-1){
			            ...
			            if(k>0){
			            if(channel_regday_nums[5][k]==0||channel_regday_nums[5][k]==0){
			            tuxing[0][k]=0;
			            }else{
			            tuxing[0][k]=(gg>50000||gg< -50000)   ? tuxing[0][k-1] : gg;
			            }
			            }else{
			            tuxing[0][k]=0;
			            }
