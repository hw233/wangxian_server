	...
	ArrayList<String[]> realFenQuList = new ArrayList<String []>();
     ...
     for (int k = 0; k < realFenQuList.size(); k++) {
  			...
  			Date fuQustartDay = DateUtil.parseDate(dfenqu[2], "yyyy-MM-dd");
  			  ...
  			  Date day_datenew=new Date(fuQustartDay.getTime() + Integer.parseInt(startDay)*24*3600000+24*3600000);//开服第startDay天的日期.
  				   String day = DateUtil.formatDate(day_datenew, "yyyy-MM-dd");
  			       if(day.compareTo(today)>=0){ break;}//如果超过了当天,则退出
  			     ...
  			     if(daynum>6){
  			      for(int weeks=0;weeks<weekNum;weeks++){
  			       Date weekstartDay_date= new Date(day_datenew.getTime()+(weeks*7)*24*3600000L); 
  			       String weekstartDay= DateUtil.formatDate(weekstartDay_date, "yyyy-MM-dd");
  			       if(weekstartDay.compareTo(today)>=0){ break;}//如果超过了当天,则退出
  			      
  			     Date weekendDay_date= new Date(day_datenew.getTime()+(weeks*7+6)*24*3600000L);  //开服第daycount天的7日的日期.
  				String  weekendDay = DateUtil.formatDate(weekendDay_date, "yyyy-MM-dd");
  				    
  			      Date weekLiuCunStartDay_date= new Date(day_datenew.getTime()+(weeks*7+7)*24*3600000L);  //开服第daycount天的当周的日期次周第一天.
  				  String  weekLiuCunStartDay = DateUtil.formatDate(weekLiuCunStartDay_date, "yyyy-MM-dd");
  			       if(weekLiuCunStartDay.compareTo(today)>=0){ break;}//如果次周超过了当天，则退出
  			     
  			      
  			      Date weekLiuCunEndDay_date= new Date(day_datenew.getTime()+(weeks*7+13)*24*3600000L);  //开服第daycount天的当周的日期次周的最后一天.
  				  String  weekLiuCunEndDay = DateUtil.formatDate(weekLiuCunEndDay_date, "yyyy-MM-dd");
  			      //TODO 调用方法。
  			      //String weekloginUumber=playGameManager.getLiuCun(day,weekendDay,day,weekendDay,fenqu,qudao,jixing);
  			      String weekloginUumber=playGameManager.getMonthLoginUsercout(weekstartDay,weekendDay,fenqu,qudao,jixing);//当周登录人数
  			     // out.println("weekstartDay:"+weekstartDay+"weekendDay:"+weekendDay+"weekLiuCunStartDay:"+weekLiuCunStartDay+"weekLiuCunEndDay:"+weekLiuCunEndDay);
  			     // out.println("jie");
  			      
  			      String weekliucun=playGameManager.getLiuCun(weekstartDay,weekendDay,weekLiuCunStartDay,weekLiuCunEndDay,fenqu,qudao,jixing);
  			      if(weekloginUumber!=null&&!"0".equals(weekloginUumber)){
  			      fenQu_weekLiuCun_nums[0][k][weeks]=String.valueOf(Float.parseFloat(weekliucun)/Float.parseFloat(weekloginUumber));
  			      }else{fenQu_weekLiuCun_nums[0][k][weeks]="0";}
  			      	 }
  			     }
  			     
  				/**月留存，月留存   start*/
  				if(daynum>29){
  				//int monthNum=(daynum+1)/30;
  				 //fenQu_monthLiuCun_nums = new String[2][realFenQuList.size()][monthNum]; //各个分区开服后N天周留存
  				 
  				 for(int mons=0;mons<monthNum;mons++){
  				 
  				 
  				 Date monstartDay_date= new Date(day_datenew.getTime()+(mons*30)*24*3600000L); 
  			       String monstartDay= DateUtil.formatDate(monstartDay_date, "yyyy-MM-dd");
  			       if(monstartDay.compareTo(today)>=0){ break;}//如果超过了当天,则退出
  				 
  				 
  			     Date monendDay_date= new Date(day_datenew.getTime()+(mons*30+29)*24*3600000L);  //开服第daycount天的7日的日期.
  				 String  monendDay = DateUtil.formatDate(monendDay_date, "yyyy-MM-dd");
  				  String monloginUumber=playGameManager.getMonthLoginUsercout(monstartDay,monendDay,fenqu,qudao,jixing);//当月登录人数
  				  fenQu_monthLiuCun_nums[1][k][mons]=monloginUumber;//当月登录人数
  				  for(int num=0;num<daynum;num++ ){
  				  if(monloginUumber!=null&&!"0".equals(monloginUumber)){
  				  dauMaU_nums[0][k][num]= String.valueOf((float)channel_regday_nums[3][k][num]/Float.parseFloat(monloginUumber));
  				  }else{dauMaU_nums[0][k][num]="0";}
  				  }
  				    
  			      Date monLiuCunStartDay_date= new Date(day_datenew.getTime()+(mons*30+29)*24*3600000L);  //开服第daycount天的当周的日期次周第一天.
  				  String  monLiuCunStartDay = DateUtil.formatDate(monLiuCunStartDay_date, "yyyy-MM-dd");
  			       if(monLiuCunStartDay.compareTo(today)>=0){ break;}//如果次周超过了当天，则退出
  			     
  			      Date monLiuCunEndDay_date= new Date(day_datenew.getTime()+(mons*30+59)*24*3600000L);  //开服第daycount天的当周的日期次周的最后一天.
  				  String  monLiuCunEndDay = DateUtil.formatDate(monLiuCunEndDay_date, "yyyy-MM-dd");
  			      
  			      //TODO 调用方法。
  			      String monliucun=playGameManager.getLiuCun(monstartDay,monendDay,monLiuCunStartDay,monLiuCunEndDay,fenqu,qudao,jixing);//次月留存人数
  			      if(monloginUumber!=null&&!"0".equals(monloginUumber)){
  			      fenQu_monthLiuCun_nums[0][k][mons]=String.valueOf(Float.parseFloat(monliucun)/Float.parseFloat(monloginUumber));//次月留存率
  			      }else{fenQu_monthLiuCun_nums[0][k][mons]="0";}
  			      	 }
  				}
...
var mycars=[<%for (int i = 0; i < daynum; i++) {
 ...
 <script language="JavaScript">
  ...
  <script language="JavaScript">
      ...
      <script language="JavaScript">
         var mycarsweek=[<%for (int i = 0; i < weekNum; i++) {
				out.print("\"" + (i+1) +"周" + "\",");
			}%>"E"];
       function drawWeekRetainUserFlotr(){
		    var f = Flotr.draw(
		$('weekRetainUserContainer'), [
		<%for (int j = 0; j <  realFenQuList.size(); j++) {
				StringBuffer sb2 = new StringBuffer();
				sb2.append("[");
				for (int kk = 0; kk < fenQu_weekLiuCun_nums[0][j].length; kk++) {
					sb2.append("[" + kk + "," + fenQu_weekLiuCun_nums[0][j][kk] + "]");
					if (kk < fenQu_weekLiuCun_nums[0][j].length - 1) sb2.append(",");
				}
				sb2.append("]");
				out.println("{");
				out.println("data:" + sb2.toString() + ",");
			...
			xaxis:{
				noTicks: <%=weekNum%>,	// Display 7 ticks.	
				tickFormatter: function(n){ return mycarsweek[Math.floor(n)];}, // =>
				min: 0,
				max: <%=weekNum%>
			},
			yaxis:{
       ...
       <script language="JavaScript">
         var mycarsmonth=[<%for (int i = 0; i < monthNum; i++) {
				out.print("\"" + (i+1) +"月" + "\",");
			}%>"E"];
       function drawMonthRetainUserFlotr(){
		    var f = Flotr.draw(
		$('monthRetainUserContainer'), [
		<%for (int j = 0; j <  realFenQuList.size(); j++) {
				StringBuffer sb2 = new StringBuffer();
				sb2.append("[");
				for (int kk = 0; kk < fenQu_monthLiuCun_nums[0][j].length; kk++) {
					sb2.append("[" + kk + "," + fenQu_monthLiuCun_nums[0][j][kk] + "]");
					if (kk < fenQu_monthLiuCun_nums[0][j].length - 1) sb2.append(",");
				}
				sb2.append("]");
				out.println("{");
				out.println("data:" + sb2.toString() + ",");
			...
			xaxis:{
				noTicks: <%=monthNum%>,	// Display 7 ticks.	
				tickFormatter: function(n){ return mycarsmonth[Math.floor(n)];}, // =>
				min: 0,
				max: <%=monthNum%>
			},
			yaxis:{
       ...
       <script language="JavaScript">
         var mycarsmonthlogin=[<%for (int i = 0; i < monthNum; i++) {
				out.print("\"" + (i+1) +"月" + "\",");
			}%>"E"];
       function drawloginRetainUserFlotr(){
		    var f = Flotr.draw(
		$('monthloginUserContainer'), [
		<%for (int j = 0; j <  realFenQuList.size(); j++) {
				StringBuffer sb2 = new StringBuffer();
				sb2.append("[");
				for (int kk = 0; kk < fenQu_monthLiuCun_nums[1][j].length; kk++) {
					sb2.append("[" + kk + "," + fenQu_monthLiuCun_nums[1][j][kk] + "]");
					if (kk < fenQu_monthLiuCun_nums[1][j].length - 1) sb2.append(",");
				}
				sb2.append("]");
				out.println("{");
				out.println("data:" + sb2.toString() + ",");
			...
			xaxis:{
				noTicks: <%=monthNum%>,	// Display 7 ticks.	
				tickFormatter: function(n){ return mycarsmonthlogin[Math.floor(n)];}, // =>
				min: 0,
				max: <%=monthNum%>
			},
			yaxis:{
		    	...
		    	<h3>DAU(日活跃用户数)</h3>
		          ...
		          <h3>ACU（活跃用户的ARPU） ：付费总额/DAU</h3>
		           ...
		           <h3>ARPU（付费用户的ARPU） ：付费总额/付费用户数</h3>
		            ...
		            <h3>次周留存</h3>
		    		<div id="weekRetainUserContainer" style="width:100%;height:400px;display:block;"></div>
		            <script>
		            drawWeekRetainUserFlotr();
		            </script> 
		            
		            <h3>次月留存留存</h3>
		    		<div id="monthRetainUserContainer" style="width:100%;height:400px;display:block;"></div>
		            <script>
		            drawMonthRetainUserFlotr();
		            </script> 
		            
		            
		             <h3>月登录用户数</h3>
		    		<div id="monthloginUserContainer" style="width:100%;height:400px;display:block;"></div>
		            <script>
		            drawloginRetainUserFlotr();
		            </script> 
