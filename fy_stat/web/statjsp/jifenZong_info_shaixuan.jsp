	...
	Object lock = new Object(){};
     ...
     <%
	          ...
	          String endDay = request.getParameter("endDay");
	           ...
	           if("0".equals(money)||money==null){money="1000000";}
...
<html>
	<head>
	 ...
	 <body>
		<center>
		<h2 style="font-weight:bold;">筛选超标服务器</h2>
        ...
        +" group by to_char(y.createdate,'YYYY-MM-DD')) mm on nn.d1=mm.d2 order by nn.d1 ";
		 ...
		 for(int k=0;k<ls.size()-1;k++)
				 ...
				 long tt2= (tuxing[0][k]>0?tuxing[0][k]:-tuxing[0][k])-(tuxing[1][k]>0?tuxing[1][k]: -tuxing[1][k]);
				 if(tt2>=Long.parseLong(money)){
				 display=true; break;
				 }
				 }
				if(display){
				out.print( fenqu+"   超过了"+money+"指标，数据如下:");
				out.println("<table id='test2' align='center' width='90%' cellpadding='1' bgcolor='#000000' cellspacing='1' border='0'>");
		    		out.print("<tr bgcolor='#EEEEBB'><td>日期</td>");
		    		 for(int k = 0 ; k < dayList.size() ; k++){
			            ...
			            out.print("</tr>");
		    		out.print("<tr bgcolor='#FFFFFF'> <td>实际库存变化</td>");
		    		 for(int k = 0 ; k < dayList.size() ; k++){
			            out.print("<td>"+tuxing[0][k]+"</td>");
			            }
			            out.print("</tr>");
			            out.print("<tr  bgcolor='#FFFFFF'> <td>预计库存变化</td>");
			          for(int k = 0 ; k < dayList.size() ; k++){
