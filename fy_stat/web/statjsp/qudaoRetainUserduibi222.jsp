...
<%@ page
  ...
  <%
  	...
  	String dateStr[] = "1,3,7".split(",");
  	String fenQu_regday_nums[][][] = new String[dateStr.length][realChannelList.size()][dayList.size()];
  	String fenQu_regday_nums_tu[][][] = new String[dateStr.length][realChannelList.size()][dayList.size()];
  	...
  	for (int tianshu = 0; tianshu < dateStr.length; tianshu++) {
  		...
  		BigDecimal dayNum=new BigDecimal("0");//指定日期留存人数
  			...
  			if (dateStr2.compareTo(DateUtil.formatDate(new Date(), "yyyy-MM-dd")) < 0) {
  				qudaoshuList_1 = playGameManager.getFenQuRetainUserCount(_day, _day,qudao);//注册当日留存
  				qudaoshuList_2 = playGameManager.getFenQuRetainUserCount(_day, dateStr2,qudao);//指定日期留存
  				for (int j = 0; j < realChannelList.size(); j++) {
  					...
  					for (int l = 0; l < qudaoshuList.size(); l++) {
  						...
  						if (ss[0].equals(_channel)) {
  							if (ss[1] != null) {
  								fenQu_regday_nums[tianshu][j][k] = ss[1];
  							}
  						}
  					}
  					...
  					for (int l = 0; l < qudaoshuList_tu.size(); l++) {
  						...
  						if (ss_tu[0].equals(_channel)) {
  							if (ss_tu[1] != null) {
  								fenQu_regday_nums_tu[tianshu][j][k] = ss_tu[1];
  							}
  						}
  					}
  				}
...
<%for (int tianshu_ = 0; tianshu_ < dateStr.length; tianshu_++) {%>
 function drawRegUserFlotr_<%=tianshu_%>(){
		...
		<%for (int j = 0; j < realChannelList.size(); j++) {
					...
					sb2.append("[");
					for (int k = 0; k < fenQu_regday_nums_tu[tianshu_][j].length; k++) {
						sb2.append("[" + k + "," + fenQu_regday_nums_tu[tianshu_][j][k] + "]");
						if (k < fenQu_regday_nums_tu[tianshu_][j].length - 1) sb2.append(",");
					}
					sb2.append("]");
					out.println("{");
	...
	<body>
		...
		-- 结束日期：<input type='text' name='endDay' value='<%=endDay%>'>(格式：2012-02-09) &nbsp;&nbsp;
		    ...
		    渠道：<select name="qudao">
                       ...
                       String _qudao = channelList.get(i);
                       %>
                        <option value="<%=_qudao %>" 
                        <%
		     ...
		     <table width="1000"><tr>
		    		<%
		    			for (int i = 0; i < fenQuList.size(); i++) {
		    				...
		    				String[] _channel = fenQuList.get(i);
		    				if (realChannelList.contains(_channel[1])) {
		    					out.print("<td align='left'><input type='checkbox' name='filterChannel' value='" + _channel[1] + "' checked >" + _channel[1]
		    							+ "&nbsp;</td>");
		    				} else {
		    					out.print("<td  align='left'><input type='checkbox' name='filterChannel' value='" + _channel[1] + "'>" + _channel[1] + "&nbsp;</td>");
		    				}
		    			}
		    		%>
		    		</tr>
		    		...
		    		<%
		    			out.println("<table id='test2' align='center' width='90%' cellpadding='1' bgcolor='#000000' cellspacing='1' border='0'>");
		    				...
		    				for (int i = 0; i < dayList.size(); i++) {
		    					...
		    					for (int c = 0; c < realChannelList.size(); c++) {
		    						out.print("<td>" + fenQu_regday_nums[tianshu_][c][i] + "</td>");
		    					}
		    					out.println("</tr>");
		    				}
