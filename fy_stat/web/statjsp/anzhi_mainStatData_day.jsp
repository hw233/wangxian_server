...
<%@ page
  ...
  String flag = null;
	...
	if(qudao==null||"-1".equals(qudao)){ qudao="anzhi_MIESHI','ANZHISDK_MIESHI"; tqudao="-1";   }
	...
	realChannelList.add("游戏币库存");
		...
		if ("true".equals(flag)) {//如果是刚打开页面，不查询
         for(int k = 0 ; k < dayList.size() ; k++){
				...
				String key="";
				if("null".equals(qudao)||"-1".equals(tqudao)){key=_day+"anzhi"+jixing;}
				else{
		...
		<br/><br/>
		       渠道：<select name="qudao" style="width:150px" >
                       <option  value="-1">全部</option>
                        <option value="anzhi_MIESHI" 
                        <%  if("anzhi_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >anzhi_MIESHI</option>
                        
                        <option value="ANZHISDK_MIESHI" 
                        <%  if("ANZHISDK_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >ANZHISDK_MIESHI</option>
