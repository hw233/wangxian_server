...
synchronized(lock){
	...
	//某个渠道，注册日期
       ...
       for(int k = 0 ; k < dayList.size() ; k++){
				...
				qudaoshuList =userManager.getQudaoRegistUserCount(_day);
				}
 ...
 function drawRegUserFlotr(){
	...
	<body>
		...
		<h2 style="font-weight:bold;">
			<%=startDay %>各渠道每天的注册用户数
		</h2>
		<form method="post">起始日期：<input type='text' name='day' value='<%=startDay %>'>
		                 -- 终止日期：<input type='text' name='endDay' value='<%=endDay %>'>(格式：2012-02-09) 
		<br/>
		    过滤渠道：
		     <table width="500">
		        <tr>
		    		...
		    		if(i>0&&i%9==0){out.print("</tr><tr>");}
		    			...
		    			if(realChannelList.contains(_channel)){
		    				out.print("<td align='left'><input type='checkbox' name='filterChannel' value='"+_channel+"' checked >"+_channel+"&nbsp;</td>");
		    			}else{
					...
					out.print("<tr bgcolor='#EEEEBB'><td>日期</td><td>总注册用户数</td>");
