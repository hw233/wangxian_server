...
<%@ page
  ...
  <%
	...
	if(qudao==null||"-1".equals(qudao)){ qudao="SINAWEI_MIESHI"; tqudao="-1";   }
	...
	realChannelList.add("游戏币库存");
		...
		if ("true".equals(flag)) {//如果是刚打开页面，不查询
         for(int k = 0 ; k < dayList.size() ; k++){
				...
				String key="";
				if("null".equals(qudao)||"-1".equals(tqudao)){key=_day+"SINAWEI"+jixing;}
				else{
		...
		<br/><br/>
		       渠道：<select name="qudao" style="width:150px" >
                       <option  value="-1">全部</option>
                        <option value="SINAWEI_MIESHI" 
                        <%  if("SINAWEI_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >SINAWEI_MIESHI</option>
                 </select> &nbsp;&nbsp;
