...
<%@ page
  ...
  <%
	...
	if(qudao==null||"-1".equals(qudao)){ qudao="BAIDUYY_MIESHI','duokuApi_MIESHI"; tqudao="-1";   }
	...
	if ("true".equals(flag)) {//如果是刚打开页面，不查询
       for(int k = 0 ; k < dayList.size() ; k++){
				...
				String key="";
				if("null".equals(qudao)||"-1".equals(tqudao)){key=_day+"duoku用户"+null+jixing;}
				else{
				key=_day+qudao+null+jixing;
				}
		...
		<br/><br/>
		       渠道：<select name="qudao" style="width:150px" >
                       <option  value="-1">全部</option>
                        ...
                        <option value="BAIDUYY_MIESHI" 
                        <%  if("BAIDUYY_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >BAIDUYY_MIESHI</option>
                        
                         <option value="duokuApi_MIESHI" 
                        <%  if("duokuApi_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >duokuApi_MIESHI</option>
                 </select> &nbsp;&nbsp;
