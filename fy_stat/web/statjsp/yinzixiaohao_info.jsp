	...
	**/
	          ...
	          Transaction_SpecialManagerImpl transaction_SpecialManager = Transaction_SpecialManagerImpl.getInstance();
	           ...
	           }else
	           {
	            ...
	            ysubSql+=" and  y.fenqu not in ('春暖花开','桃花仙岛','天下无双','海纳百川','琼楼金阙','飘渺仙道','万里苍穹','盛世欢歌','修罗转生')";
	           }
	           
	            List<Transaction_yinzi> ls_SuiHouLiuTong=null;
	            List<Transaction_yinzi> ls_YinDingChanChu=null;
	             List<Transaction_yinzi> ls_ChongZhiBuDan=null;
               ...
               String sql_YinDingChanChu="select t.createdate,'dd' fenqu,'银锭产出' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t,dt_reasontype r "
                                +" where  r.name in ('邮件获取充值返利','邮件获取充值活动','充值活动奖励累计充值奖励返利',"
                            ...
                            +"'10级奖励银子80级奖励银子','10级奖励银子90级奖励银子')  and  t.createdate between '"+startDay+"' and '"+endDay_kucun+"' "+subSql+"  and to_char(t.reasontype)=to_char(r.id) group by t.createdate order by t.createdate";
	                         ...
	                         String sql_ChongZhiBuDan="select t.createdate,'dd' fenqu,'银锭产出' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t,dt_reasontype r "
                                +" where  r.name in ('邮件获取系统补单','邮件获取GM工具补发')  and  t.createdate between '"+startDay+"' and '"+endDay_kucun+"' "+subSql+"  and to_char(t.reasontype)=to_char(r.id) group by t.createdate order by t.createdate";
	                         ls_ChongZhiBuDan = transaction_SpecialManager.getTransaction_yinziBySql(sql_ChongZhiBuDan);
	           ...
	           +",'结婚求婚送花或糖1','结婚求婚送花或糖2','结婚强制离婚','刷新任务','宠物炼化宠物炼化','JIEYI_COST结义消耗','天尊令使用天尊令'"
	           ...
	           +"'大师技能加点skMasterExPoint','副本结束转盘副本转盘抽奖','仙府种植仙府种植-1级百宝树','仙府种植仙府种植-2级百宝树','仙府种植仙府种植-3级百宝树',"
	                        ...
	                        +"'绑银不足翅膀强化用绑银强化翅膀','家族祈福家族上香','猎命猎命单抽消耗','猎命猎命十连抽消耗','极地寻宝抽奖极地寻宝抽奖','银票不够极地寻宝抽奖极地寻宝抽奖') then t.money "
               ...
               String sql_YinDingChanChu="select 'dd' createdate,t.fenqu,'银锭产出' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t,dt_reasontype r "
                            ...
                            +"'10级奖励银子80级奖励银子','10级奖励银子90级奖励银子')  and  t.createdate between '"+startDay+"' and '"+endDay_kucun+"' "+subSql+"   and to_char(t.reasontype)=to_char(r.id)  group by t.fenqu order by t.fenqu";
	                         ...
	                         String sql_ChongZhiBuDan="select 'dd' createdate,t.fenqu,'银锭产出' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t,dt_reasontype r "
                                +" where r.name in  ('邮件获取系统补单','邮件获取GM工具补发')  and  t.createdate between '"+startDay+"' and '"+endDay_kucun+"' "+subSql+"   and to_char(t.reasontype)=to_char(r.id)  group by t.fenqu order by t.fenqu";
	           ...
	           +",'活动金剪刀活动','活动银剪刀活动','兑换活动兑换活动扣除银子','进入宠物迷城进入宠物迷城','接取任务挖宝','大师技能加点skMasterAddPoint',"
	           +"'大师技能加点skMasterExPoint','副本结束转盘副本转盘抽奖','仙府种植仙府种植-1级百宝树','仙府种植仙府种植-2级百宝树','仙府种植仙府种植-3级百宝树',"
	                        +"'仙府种植仙府种植-4级百宝树','坐骑阶培养新坐骑系统','银票不够副本结束转盘副本转盘抽奖','银票不够仙府种植仙府种植-1级百宝树','银票不够仙府种植仙府种植-2级百宝树',"
	                        +"'银票不够仙府种植仙府种植-3级百宝树','银票不够仙府种植仙府种植-4级百宝树','银票不够坐骑阶培养新坐骑系统',"
	                        +"'家族祈福家族修炼','家族祈福家族上香','家族祈福刷新任务','创建战队创建战队花费','绑银不足翅膀强化用绑银强化翅膀',"
	                        +"'家族祈福家族上香','猎命猎命单抽消耗','猎命猎命十连抽消耗','极地寻宝抽奖极地寻宝抽奖','银票不够极地寻宝抽奖极地寻宝抽奖') then t.money "
	           
	           +"when r.name in ('交付镖车交付镖车','金钱道具使用镖玉(白)','金钱道具使用镖玉(绿)','金钱道具使用镖玉(蓝)','金钱道具使用镖玉(紫)','金钱道具使用镖玉(橙)'"
	           +",'金钱道具使用镖银(白)','金钱道具使用镖银(绿)','金钱道具使用镖银(蓝)','金钱道具使用镖银(紫)','金钱道具使用镖银(橙)','金钱道具使用镖金(白)'"
	           +",'金钱道具使用镖金(绿)','金钱道具使用镖金(蓝)','金钱道具使用镖金(紫)','金钱道具使用镖金(橙)','击杀奸细击杀奸细') then -t.money "
	           +"when r.name in ('竞标竞标','追加资金追加资金') then t.money*0.03 "
	           +"when r.name in ('交易交易给他人','摆摊在摆摊上购买物品') then round(t.money/21) "
	           +"when r.name in ('拍卖竞拍费拍卖竞拍费') then round(t.money*0.2) "
	           +"when r.name in ('求购求购预存费用和手续费') then round(t.money/11)  end) money " 
         ...
         if("1".equals(displayType)){
			...
			for( int g=0;g< ls_YinDingChanChu.size();g++){
			       ...
			       channel_regday_nums[2][k]=transaction_yinzi.getMoney()/1000000;
			  		...
			  		for( int g=0;g< ls_ChongZhiBuDan.size();g++){
			       Transaction_yinzi transaction_yinzi=(Transaction_yinzi)ls_ChongZhiBuDan.get(g);
			        for(int k = 0 ; k < dayList.size() ; k++){
		            	String _day = dayList.get(k);
		            if(_day.equals(transaction_yinzi.getCreateDate())){
			       channel_regday_nums[3][k]=transaction_yinzi.getMoney()/1000000;
			       }
			       }
			       }     
			...
			for( int g=0;g< ls_YinDingChongZhi.size();g++){
			       Transaction_yinzi transaction_yinzi=(Transaction_yinzi)ls_YinDingChongZhi.get(g);
			          for(int k = 0 ; k < dayList.size() ; k++){
		            	String _day = dayList.get(k);
		            if(_day.equals(transaction_yinzi.getCreateDate())){
			       channel_regday_nums[4][k]=transaction_yinzi.getMoney()/2000;
			       }
			       }
			       }
			for( int g=0;g< ls_YinDingXiaoHao.size();g++){
			    ...
			    for( int g=0;g< ls_yinziKuCun.size();g++){
			       Transaction_yinzi transaction_yinzi=(Transaction_yinzi)ls_yinziKuCun.get(g);
			         for(int k = 0 ; k < dayList.size() ; k++){
		            	String _day = dayList.get(k);
		            if(_day.equals(transaction_yinzi.getCreateDate())){
			         channel_regday_nums[6][k]=transaction_yinzi.getMoney()/1000000;
			   ...
			   for(int k = 0 ; k < dayList.size() ; k++){
			          if(k < dayList.size()-1&&k!=0){
			            ...
			            Long gg=channel_regday_nums[6][k+1] - channel_regday_nums[6][k];
			            
			            if(channel_regday_nums[6][k]==0||channel_regday_nums[6][k+1]==0){
			            tuxing[0][k]=0;
			            }else{
			           ...
			           Long temp=channel_regday_nums[2][k]+channel_regday_nums[3][k] + channel_regday_nums[4][k] - channel_regday_nums[5][k];
		  ...
		  抹平幅度<input type='text' name='money' value='<%=money %>'>（如果振幅超过这个值，就用前一天的值替带）
		    <br/><br/>
		    	...
		    	<a href="yinzixiaohao_info_qufenqu.jsp">剔除库存断档的服</a>  
		    		   ...
		    		   |<a href="yinziJianKongxiaohao_info.jsp">银子消耗</a>   
		    		...
		    		out.print("<tr><td>日期</td>");
		    		 for(int k = 0 ; k < dayList.size() ; k++){
			            ...
			            out.print("</tr>");
		    		out.print("<tr> <td>实际库存变化</td>");
		    		 for(int k = 0 ; k < dayList.size() ; k++){
			            out.print("<td>"+tuxing[0][k]+"</td>");
			            }
			            out.print("</tr>");
			            out.print("<tr><td>预计库存变化</td>");
			          for(int k = 0 ; k < dayList.size() ; k++){
		    		...
		    		<font style="font-weight:bold;color='red'">预计库存变化=银锭产出+系统补单+银锭充值-系统消耗;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		    		“系统消耗”已经包含“流通税收”了。亲！不要再在上面的公式里重复的减一遍了。ok？
		    		<br/>另外，这是银子监控的汇总统计功能，用以监控银子总体水平，监测银子是否被刷。预计库存变化和实际的库存变化会一些误差，只要在我们允许的误差范围内，是可以理解的。</font>
		    		<br/><br/><br/>
