	...
	**/
	...
	realChannelList.add("平均付费次数");
		...
		String channel_regday_nums[] = new String[realChannelList.size()];
				...
				String userCount=playGameManager.active_userCount_wliucun(startDay,endDay,fenqu,qudao);
				if(userCount!=null){
				channel_regday_nums[0] =userCount;//人数
				}
				
				  String loginUserCount=playGameManager.active_userLoginTimes_wliucun(startDay,endDay,fenqu,qudao);
                  if(loginUserCount!=null){
				   channel_regday_nums[1] =loginUserCount;//平均独立登录次数
				   }
				   
				   String avgOnLineTime=playGameManager.active_userAVGOnLineTime_wliucun(startDay,endDay,fenqu,qudao);
				  if(avgOnLineTime!=null){
				   channel_regday_nums[2] =avgOnLineTime;//平均在线时长
				    }
				    
				   String pay_userCount=playGameManager.active_pay_userCount_wliucun(startDay,endDay,fenqu,qudao);//独立付费人数
				  if(pay_userCount!=null){
				   channel_regday_nums[3] =pay_userCount;//独立付费人数
				    }
				    
				  String pay_Money=playGameManager.active_pay_Money_wliucun(startDay,endDay,fenqu,qudao);
				  if(pay_Money!=null){
				   channel_regday_nums[4] =pay_Money;//付费总金额(元)
				}
  
                
				  String pay_AVGTimes=playGameManager.active_pay_AVGTimes_wliucun(startDay,endDay,fenqu,qudao);
...
<html>
	...
	<body>
		...
		<i>周留存定义：上周达到活跃标准，本周依然达到活跃标准的用户</i>
