...
<html>
	<head>
	 ...
	 <body>
		<center>
		<h2 style="font-weight:bold;">各个服务器数据</h2>
		    ...
		    <br/><br/>
		    	...
		    	<br/>
		    	   ...
		    	   <a href="jifenZong_info_qufenqu.jsp">剔除库存断档的服</a>|
					...
					out.print("<tr bgcolor='#EEEEBB'><td>分区</td><td>日期</td><td>充值积分</td><td>老用户回馈</td><td>积分消耗</td><td>积分库存剩余</td><td>预计库存变化</td><td>实际库存变化</td></tr>");
					
					
					 for(int num=0;num<fenQuList.size();num++){
                     String [] lst=fenQuList.get(num);
                     fenqu=lst[1];
        ...
        +" group by to_char(y.createdate,'YYYY-MM-DD')) mm on nn.d1=mm.d2 order by nn.d1 ";
		 ...
		 for(int k=0;k<ls.size()-1;k++)
					...
					String[] strS2=ls.get(k+1);
			            ...
			            Long temp=Long.parseLong(strS[1]==null?"0":strS[1])+Long.parseLong(strS[2]==null?"0":strS[2]) - Long.parseLong(strS[3]==null?"0":strS[3]);
					...
					out.print("<tr bgcolor='#FFFFFF'><td>"+fenqu+"</td><td>"+strS[0]+"</td><td>"+strS[1]+"</td><td>"+strS[2]+"</td><td>"+strS[3]+"</td><td>"+strS[4]+"</td><td bgcolor='#EEEEBB'>"+tuxing[1][i]+"</td><td bgcolor='#EEEEBB'>"+tuxing[0][i]+"</td></tr>");
