...
<%@ page
  ...
  <%
   String flag = null;
	...
	if(qudao==null||"-1".equals(qudao)){ qudao="PPZHUSHOU_MIESHI','PPZHUSHOULINSHI_MIESHI"; tqudao="-1";   }
	...
	realChannelList.add("游戏币库存");
	 ...
	 if ("true".equals(flag)) {//如果是刚打开页面，不查询
         for(int k = 0 ; k < dayList.size() ; k++){
				...
				String key="";
				if("null".equals(qudao)||"-1".equals(tqudao)){key=_day+"PPZHUSHOU"+jixing;}
				else{
		...
		<br/><br/>
		       渠道：<select name="qudao" style="width:150px" >
                       <option  value="-1">全部</option>
                        ...
                        <option value="PPZHUSHOU_MIESHI" 
                        <%  if("PPZHUSHOU_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >PPZHUSHOU_MIESHI</option>
                        
                        <option value="PPZHUSHOULINSHI_MIESHI" 
                        <%  if("PPZHUSHOULINSHI_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >PPZHUSHOULINSHI_MIESHI</option>
