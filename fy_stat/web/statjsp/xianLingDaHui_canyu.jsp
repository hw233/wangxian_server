...
<%@ page
  ...
  <%
	...
	if(endDay == null) endDay = today;
             ...
             String sql="select pdd,npccount,pcount,round(npccount*100/pcount,3) ratio from "+
             ...
             +" and to_date('"+endDay+" 23:59:59','YYYY-MM-DD hh24:mi:ss') and p.maxlevel>151 group by to_char(p.enterdate,'YYYY-MM-DD') ) nn "
             +" left join "
             ...
             +" group by to_char(t.createdate,'YYYY-MM-DD')) mm  on nn.pdd=mm.npcdd ";
             ...
             String sql_zong="select npccount,pcount,round(npccount*100/pcount,3) ratio from "+
             "( select count(distinct(p.username||p.fenqu)) pcount "
             +" from stat_playgame p where  p.enterdate between to_date('"+startDay+" 00:00:00','YYYY-MM-DD hh24:mi:ss') "
             +" and to_date('"+endDay+" 23:59:59','YYYY-MM-DD hh24:mi:ss') and p.maxlevel>151 ) nn "
             +" left join "
		...
		<form  name="form1" id="form1" method="post">
		  ...
		  结束日期： <input type="text" name="endDay" id="endDay" class="nqz_input" style="width:80px;" readonly="readonly" onfocus="new Calendar(false,'yyyy-MM-dd').showTimePicker(this);" value="<%=endDay %>"/>
		   ...
		   <input type="hidden" name="today" id="today"  value="<%=today %>"/>
					...
					out.print("<tr bgcolor='#EEEEBB'><td>日期</td><td>进入玩家数</td><td>DAU（151级以上）</td><td>进入玩家占比</td></tr>");
