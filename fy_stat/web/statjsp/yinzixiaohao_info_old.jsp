...
<%@ page
	...
	**/
	          ...
	          Transaction_SpecialManagerImpl transaction_SpecialManager = Transaction_SpecialManagerImpl.getInstance();
	           ...
	           String sql_SuiHouLiuTong="select t.createdate,'dd' fenqu,'税后流通' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	                       ...
	                       String sql_Sui1="select t.createdate,'dd' fenqu,'税后流通' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	                            +" where  t.reasontype in ('摆摊在摆摊出售了东西','交易获取从他人那交易来') "
	                            +" and  t.createdate between '"+startDay+"' and '"+endDay+"' "+subSql+" group by t.createdate order by t.createdate";
	                        // ls_SuiHouLiuTong=transaction_SpecialManager.getTransaction_yinziBySql(sql_SuiHouLiuTong);
	                         
	                       String sql_Sui2="select t.createdate,'dd' fenqu,'税后流通' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
               ...
               String sql_YinDingChanChu="select t.createdate,'dd' fenqu,'银锭产出' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	           ...
	           //String sql_YinDingChongZhi="select t.createdate,'dd' fenqu,'银锭充值' REASONTYPE,'0' ACTION,sum(t.money) money "
	             //               +" from stat_gamechongzhi_yinzi t where t.reasontype like '充值%' and  "
	           ...
	           String sql_YinDingXiaoHao="select t.createdate,'dd' fenqu,'银锭消耗' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	           ...
	           String sql_SuiHouLiuTong="select 'dd' createdate,t.fenqu,'税后流通' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	                            +" where  t.reasontype in ('摆摊在摆摊出售了东西','交易获取从他人那交易来','求购求购出售物品','邮件获取给卖家发钱') "
	                            +" and  t.createdate between '"+startDay+"' and '"+endDay+"' "+subSql+" group by t.fenqu order by t.fenqu";
	                           ls_SuiHouLiuTong=transaction_SpecialManager.getTransaction_yinziBySql(sql_SuiHouLiuTong);
	       
	                      String sql_Sui1="select 'dd' createdate,t.fenqu,'税后流通' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	                            +" where  t.reasontype in ('摆摊在摆摊出售了东西','交易获取从他人那交易来') "
	                            +" and  t.createdate between '"+startDay+"' and '"+endDay+"' "+subSql+" group by t.fenqu";
	                         //ls_SuiHouLiuTong=transaction_SpecialManager.getTransaction_yinziBySql(sql_SuiHouLiuTong);
	                         
	                      String sql_Sui2="select 'dd' createdate,t.fenqu,'税后流通' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	                            +" where  t.reasontype in ('求购求购出售物品') "
	                            +" and  t.createdate between '"+startDay+"' and '"+endDay+"' "+subSql+" group by t.fenqu ";
	                         //ls_SuiHouLiuTong=transaction_SpecialManager.getTransaction_yinziBySql(sql_SuiHouLiuTong);
               ...
               String sql_YinDingChanChu="select 'dd' createdate,t.fenqu,'银锭产出' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
                                +" where t.reasontype in ('金钱道具使用银块','10级奖励银子10级奖励银子','占领城市给的钱占领城市给的钱','占领矿给的钱占领矿给的钱'"
	           ...
	           String sql_YinDingXiaoHao="select 'dd' createdate,t.fenqu,'银锭消耗' REASONTYPE,'0' ACTION,sum(t.money) money from stat_gamechongzhi_yinzi t "
	                            +" where t.reasontype in ('祈福祈福','商店购买','宠物炼化宠物炼化','进入付费地图进入付费地图','宠物合体宠物合体','扩展背包扩展背包'"
	                            +",'配方合成用银子合成装备','结婚婚礼规模龙凤呈祥-100-山盟海誓对戒(男)~山盟海誓对戒(女)~','家族捐献家族捐献','原地复活原地复活'"
	                            +",'千层塔刷新千层塔刷新','装备升级用银子强化装备','结婚婚礼规模天造地设-50-三生相守对戒(男)~三生相守对戒(女)~','庄园开地','洗练洗练'"
	                            +",'修理修理','刷新任务','炼器炼器','拍卖拍卖手续费','创建宗派创建宗派','JIEYI_COST结义消耗','天尊令使用天尊令','宠物炼化宠物强化'"
		    ...
		    <br/><br/>
               ...
               <!-- 
		    	...
		    	<br/>
		    	  ...
		    	  <a href="yinzixiaohao_info.jsp">银子消耗</a>   |<a href="yinzixiaohao_info_old.jsp">银子消耗(旧版)</a>|<a href="yinpiaoxiaohao_info.jsp">银票消耗</a>|  <a href="yinpiaoxiaohao_info_old.jsp">银票消耗(老版)</a>|
