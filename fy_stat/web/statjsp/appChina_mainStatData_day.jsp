...
<%@ page
  ...
  String flag = null;
	...
	if(qudao==null||"-1".equals(qudao)){ qudao="appchina";
	                                    // tqudao="-1";  
	                                      }
	...
	realChannelList.add("网页部分付费金额（分）");
	realChannelList.add("游戏币库存");
       ...
       for(int k = 0 ; k < dayList.size() ; k++){
				...
				String key="";
				if("null".equals(qudao)){key=_day+"内侧用户"+fenqu;}
			//	else if ("-1".equals(tqudao)){key=_day+tqudao+fenqu;}
				else{
				key=_day+qudao+fenqu+null;
				}
				if(!today.equals(_day)){
				...
				if(registUserCount!=null){
				  ...
				  Long reg_LoginUserCount=playGameManager.getReg_LoginUserCount(_day,_day,qudao,fenqu,null);//当天注册，在分区道登陆的用户
				  ...
				  Long youXiaoUserCount=userManager.getYouXiaoUerCount(_day,_day,qudao,fenqu,null);
				...
				if(registUserCount!=null&&reg_LoginUserCount_all!=null){
			     zhuCeNOCreatPlayer= registUserCount -reg_LoginUserCount_all;
				  ...
				  Long enterGameUserCount=playGameManager.getEnterGameUserCount(_day,_day,qudao,fenqu,null);
				  ...
				  Long avgOnlineUserCount=onLineUsersCountManager.getAvgOnlineUserCount(_day,_day,qudao,fenqu,null);
				  ...
				  Long maxOnlineUserCount=onLineUsersCountManager.getMaxOnlineUserCount(_day,_day,qudao,fenqu,null);
				 ...
				 Long avgOnLineTime=playGameManager.getAvgOnLineTime(_day,_day,qudao,fenqu,null);
				 ...
				 Long chongZhiUserCount=chongZhiManager.getChongZhiUserCount(_day,_day,qudao,fenqu,null);
				 ...
				 Long unAheadChongZhiUserCount=chongZhiManager.getUnAheadChongZhiUserCount(_day,_day,qudao,fenqu,null);
				 ...
				 Long youXiaoUerAVGOnLineTime=userManager.getYouXiaoUerAVGOnLineTime(_day,_day,qudao,fenqu,null);
				 ...
				 Long chongZhiCount=chongZhiManager.getChongZhiCount(_day,_day,qudao,fenqu,null);//分区充值金额
				...
				//Long leftYouXiBiCount=playGameManager.getAppChina_null_pay(_day,qudao,fenqu,null);
				
				 Long chongZhiAppChia_null=chongZhiManager.getAppChina_null_pay(_day,_day,qudao,null);//appchina注册用户在 null渠道充值钱数
			    if(chongZhiAppChia_null!=null){
				  stat_common[15]=chongZhiAppChia_null.toString();
				  channel_regday_nums[14][k] =chongZhiAppChia_null;
				...
				channel_regday_nums[15][k] =leftYuanBaoCount;//游戏币库存
				  ...
				  */
				  if(!today.equals(_day)){ userManager.addstat_common(_day,stat_common);}
				  }
		  }
		}
%>
		...
		-- 结束日期：<input type='text' name='endDay' value='<%=endDay %>'>(格式：2012-02-09) -->
		 ...
		 开始日期： <input type="text" name="day" id="day" class="nqz_input" style="width:100px;" readonly="readonly" onfocus="new Calendar(false,'yyyy-MM-dd').showTimePicker(this);" value="<%=startDay %>"/>&nbsp;到
		  结束日期： <input type="text" name="endDay" id="endDay" class="nqz_input" style="width:100px;" readonly="readonly" onfocus="new Calendar(false,'yyyy-MM-dd').showTimePicker(this);" value="<%=endDay %>"/>
		    		...
		    		if(c!=1&&c!=2&&c!=5&&c!=6&&c!=7&&c!=9&&c!=10&&c!=15){
