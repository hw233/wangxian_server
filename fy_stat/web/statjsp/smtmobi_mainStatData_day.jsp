...
<%@ page
  ...
  <%
   String flag = null;
	...
	if(qudao==null||"-1".equals(qudao)){ qudao="smtmobi01_mieshi','smtmobi01_MIESHI','smtmobi02_MIEHSI','smtmobi03_MIEHSI','SMTMOBI_MIESHI"; tqudao="-1";   }
	...
	if ("true".equals(flag)) {//如果是刚打开页面，不查询
       for(int k = 0 ; k < dayList.size() ; k++){
				...
				String key="";
				if("null".equals(qudao)||"-1".equals(tqudao)){key=_day+"smtmobi用户";}
				else{
				key=_day+qudao;
				}
				...
				stat_common[0]=key;
				Long registUserCount=userManager.getRegistUerCount(_day,_day,qudao,null,null);
				if(registUserCount!=null){
				  ...
				  Long reg_LoginUserCount=playGameManager.getReg_LoginUserCount(_day,_day,qudao,null,null);//当天注册，在分区道登陆的用户
				  ...
				  Long youXiaoUserCount=userManager.getYouXiaoUerCount(_day,_day,qudao,null,null);
				if(youXiaoUserCount!=null){
				 ...
				 Long zhuCeNOCreatPlayer=0L;
				  Long reg_LoginUserCount_all=playGameManager.getReg_LoginUserCount(_day,_day,qudao,null,null);//注册并登陆的用户，不区分区
				if(registUserCount!=null&&reg_LoginUserCount_all!=null){
			     zhuCeNOCreatPlayer= registUserCount -reg_LoginUserCount_all;
				  ...
				  Long enterGameUserCount=playGameManager.getEnterGameUserCount(_day,_day,qudao,null,null);
				  ...
				  Long avgOnlineUserCount=onLineUsersCountManager.getAvgOnlineUserCount(_day,_day,qudao,null,null);
				  ...
				  Long maxOnlineUserCount=onLineUsersCountManager.getMaxOnlineUserCount(_day,_day,qudao,null,null);
				 ...
				 Long avgOnLineTime=playGameManager.getAvgOnLineTime(_day,_day,qudao,null,null);
				 ...
				 Long chongZhiUserCount=chongZhiManager.getChongZhiUserCount(_day,_day,qudao,null,null);
				 ...
				 Long unAheadChongZhiUserCount=chongZhiManager.getUnAheadChongZhiUserCount(_day,_day,qudao,null,null);
				 ...
				 Long youXiaoUerAVGOnLineTime=userManager.getYouXiaoUerAVGOnLineTime(_day,_day,qudao,null,null);
				 ...
				 Long chongZhiCount=chongZhiManager.getChongZhiCount(_day,_day,qudao,null,null);//分区充值金额
				 ...
				 Long chongZhiCount_all=chongZhiManager.getChongZhiCount(_day,_day,qudao,null,null);//全部充值金额，不分区
		...
		<br/><br/>
		       渠道：<select name="qudao" style="width:150px" >
                       <option  value="-1">全部</option>
                        ...
                        <option value="smtmobi01_mieshi" 
                        <%  if("smtmobi01_mieshi".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >smtmobi01_mieshi</option>
                        <option value="SMTMOBI_MIESHI" 
                        <%  if("SMTMOBI_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >SMTMOBI_MIESHI</option>
               
                        <option value="smtmobi01_MIESHI" 
                        <%  if("smtmobi01_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >smtmobi01_MIESHI</option>
                       
                       <option value="smtmobi02_MIEHSI" 
                        <%  if("smtmobi02_MIEHSI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >smtmobi02_MIEHSI</option>
                        
                       <option value="smtmobi03_MIEHSI" 
                        <%  if("smtmobi03_MIEHSI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >smtmobi03_MIEHSI</option>
