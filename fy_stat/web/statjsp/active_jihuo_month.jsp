	...
	**/
	...
	realChannelList.add("游戏币库存");
		...
		String channel_regday_nums[] = new String[realChannelList.size()];
				...
				String userCount=playGameManager.active_userCount_wjihuo(startDay,endDay,fenqu,qudao);
				if(userCount!=null){
				channel_regday_nums[0] =userCount;//人数
				}
				
				  String loginUserCount=playGameManager.active_userLoginTimes_wjihuo(startDay,endDay,fenqu,qudao);
                  if(loginUserCount!=null){
				   channel_regday_nums[1] =loginUserCount;//平均独立登录次数
				   }
				   
				   String avgOnLineTime=playGameManager.active_userAVGOnLineTime_wjihuo(startDay,endDay,fenqu,qudao);
				  if(avgOnLineTime!=null){
				   channel_regday_nums[2] =avgOnLineTime;//平均在线时长
				    }
				    
				   String pay_userCount=playGameManager.active_pay_userCount_wjihuo(startDay,endDay,fenqu,qudao);//独立付费人数
				  if(pay_userCount!=null){
				   channel_regday_nums[3] =pay_userCount;//独立付费人数
				    }
				    
				  String pay_Money=playGameManager.active_pay_Money_wjihuo(startDay,endDay,fenqu,qudao);
				  if(pay_Money!=null){
				   channel_regday_nums[4] =pay_Money;//付费总金额(元)
				}
  
                
				  String pay_AVGTimes=playGameManager.active_pay_AVGTimes_wjihuo(startDay,endDay,fenqu,qudao);
				  if(pay_AVGTimes!=null){
				   channel_regday_nums[5] =pay_AVGTimes;//平均付费次数
				  }
				
				  //String spend_userCount=playGameManager.active_spend_userCount_wjihuo(startDay,endDay,fenqu,qudao);
				   String spend_userCount=null;
				  if(spend_userCount!=null){
				   channel_regday_nums[6] =spend_userCount;//独立消费人数
				  }
				 //String spend_money=playGameManager.active_spend_money_wjihuo(startDay,endDay,fenqu,qudao);
				 String spend_money=null;
				 if(spend_money!=null){
				   channel_regday_nums[7] =spend_money;//消费金额
				}
				
				 //String left_yuanbao=playGameManager.active_left_yuanbao_wjihuo(startDay,endDay,fenqu,qudao);
				  String left_yuanbao=null;
				 if(left_yuanbao!=null){
				channel_regday_nums[8] =left_yuanbao;//元宝库存
				}
				
				 //String left_youxibi=playGameManager.active_left_youxibi_wjihuo(startDay,endDay,fenqu,qudao);
...
<html>
	...
	<body>
		...
		<i>月留存定义：M0活跃，M1不活跃，M2活跃，则视为M2的回流活跃用户</i>
