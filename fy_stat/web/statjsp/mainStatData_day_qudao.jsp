	...
	long channel_regday_nums[][] = new long[realChannelList.size()][channelList.size()];
     if ("true".equals(flag)) {//如果是刚打开页面，不查询
       for(int k = 0 ; k < channelList.size() ; k++){
		...
		//request.setAttribute("channel_regday_nums",channel_regday_nums);
		}
%>
...
<html>
	<head>
        ...
        function downloads(){
         ...
         $('form1').action="mainStatData_day_qudao_order.jsp";
	     $('form1').submit();
         }
        </script>
	</head>
		    	...
		    	<input type='submit' value='提交'></form>
		    		...
		    		for(int c = 0 ; c < realChannelList.size() ; c++){
		    			//out.print("<td>"+realChannelList.get(c)+"</td>");
		    				out.print("<td> <a href=javascript:downloads()>"+realChannelList.get(c)+"</a></td>");
		    		}
					out.println("</tr>");
