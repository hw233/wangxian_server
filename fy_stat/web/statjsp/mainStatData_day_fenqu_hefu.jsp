...
<%@ page
  ...
  <%
	...
	if(startDay == null) {// startDay = today;
	         ...
	         //if(fenQuList==null)
	         //  {
		         fenQuList= userManager.getFenQuByStatus("1");//获得现有的分区信息
	...
	realChannelList.add("当天进入ARPU(不分区)");
	realChannelList.add("付费ARPU");
	realChannelList.add("付费渗透率(千分之)");
  	realChannelList.add("独立进入(分区排重)");

	long channel_regday_nums[][] = new long[realChannelList.size()][fenQuList.size()];
     if ("true".equals(flag)) {//如果是刚打开页面，不查询
       for(int k = 0 ; k < fenQuList.size() ; k++){
			    ...
			    Long shenTouLv=(chongZhiUserCount*1000)/enterGameUserCount;
				  ...
				  channel_regday_nums[14][k] =shenTouLv;//     付费渗透率=付费人数/独立进入
...
var mycars=[<% for(int i = 0 ; i < fenQuList.size() ; i++){ out.print("\""+fenQuList.get(i)[1]+"\",");} %>"E"];
       function drawRegUserFlotr(){
		...
		%>
			...
			xaxis:{
				noTicks: <%=fenQuList.size()%>,	// Display 7 ticks.	
	...
	<body>
		...
		<form  method="post">
		 ...
		 开始日期： <input type="text" name="day" id="day" class="nqz_input" style="width:80px;" readonly="readonly" onfocus="new Calendar(false,'yyyy-MM-dd').showTimePicker(this);" value="<%=startDay %>"/>
		       ...
		       渠道：<select name="qudao">
		    	...
		    	<input type='submit' value='提交'></form>
					...
					for(int i = 0 ; i < fenQuList.size() ; i++){
						out.print("<tr bgcolor='#FFFFFF'><td>"+fenQuList.get(i)[1]+"</td>");
						for(int c = 0 ; c < realChannelList.size() ; c++){
					...
					huizong[5]=huizong[5]/fenQuList.size();
					huizong[7]=huizong[7]/fenQuList.size();
					huizong[10]=huizong[10]/fenQuList.size();
