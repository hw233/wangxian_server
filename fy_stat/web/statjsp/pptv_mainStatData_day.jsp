...
<%@ page
  ...
  <%
   String flag = null;
	...
	if(qudao==null||"-1".equals(qudao)){ qudao="pptv_MIESHI"; tqudao="-1";   }
	...
	realChannelList.add("游戏币库存");
      ...
      synchronized(lock){
       ...
       for(int k = 0 ; k < dayList.size() ; k++){
				...
				String key="";
				if("null".equals(qudao)||"-1".equals(tqudao)){key=_day+"pptv_MIESHI用户"+jixing;}
				else{
		...
		<br/><br/>
		       渠道：<select name="qudao" style="width:150px" >
                       <option  value="-1">全部</option>
                        <option value="pptv_MIESHI" 
                        <%  if("pptv_MIESHI".equals(qudao)){ out.print(" selected=\"selected\"");  } %>
                        >pptv_MIESHI</option>
                 </select> &nbsp;&nbsp;
