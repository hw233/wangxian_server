...
<%@ page
	...
	com.xuanzhi.tools.text.*,com.sqage.stat.commonstat.entity.*,com.xuanzhi.tools.text.*,
    ...
    <%
     ...
     if ("true".equals(flag)) {  //如果是刚打开页面，不产生下载文件
      ...
      ///////////////////////下载文件 ////////////////////////////////
        String spNumber=startDay+"JiFenMingXi";
        String basePath =getServletContext().getRealPath("/")+"excel";
         ...
         String sql=" select t.id,t.username,t.time,t.game,t.fenqu,t.gamelevel,t.money,"
         +" t.action, t.currencytype,r.name reasontype,t.qudao,t.jixing  from stat_gamechongzhi_jifen t,dt_reasontype r "
         +"where t.currencytype=31 and to_char(t.time,'YYYY-MM-DD')='"+startDay+"' and to_char(t.reasontype)=to_char(r.id)";
         
          List<GameChongZhi> ls=gameChongZhiManager.getBySql(sql);
             
              for(int j = 0; j < ls.size();j++){
                  GameChongZhi gameChongZhi=(GameChongZhi)ls.get(j);
                  
                  StringBuffer contentStr=new StringBuffer(gameChongZhi.getUserName()+"\t"+gameChongZhi.getTime()+"\t"+gameChongZhi.getFenQu()+"\t"+gameChongZhi.getGameLevel()+"级\t");
                  if(gameChongZhi.getAction()==0){
                  contentStr.append("获取\t");
                  }else if (gameChongZhi.getAction()==1){
                  contentStr.append("消耗\t");
                  }else if (gameChongZhi.getAction()==2){
                  contentStr.append("回馈\t");
                  }else{
                  contentStr.append("其他\t");
                  }
                  contentStr.append(gameChongZhi.getReasonType()+"\t"+gameChongZhi.getMoney()+"\t"+gameChongZhi.getQuDao()+"\r\n");
                
                  writing(contentStr.toString(),f);
              }
              }
...
<html>
	...
	<body>
		...
		<input type='hidden' name='flag' value='true'/>
		   ...
		   下载统计日期： <input type="text" name="day" id="day" class="nqz_input" style="width:100px;" readonly="readonly" onfocus="new Calendar(false,'yyyy-MM-dd').showTimePicker(this);" value="<%=startDay %>"/>&nbsp;
