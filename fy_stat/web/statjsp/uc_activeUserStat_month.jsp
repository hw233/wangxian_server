	...
	**/
	...
	realChannelList.add("游戏币库存");
		...
		String channel_regday_nums[] = new String[realChannelList.size()];
				...
				String userCount=playGameManager.active_MuserCount(startDay,endDay,fenqu,qudao);
				if(userCount!=null){
				channel_regday_nums[0] =userCount;//人数
				}
				
				  String loginUserCount=playGameManager.active_MuserLoginTimes(startDay,endDay,fenqu,qudao);
                  if(loginUserCount!=null){
				   channel_regday_nums[1] =loginUserCount;//平均独立登录次数
				   }
				   
				   String avgOnLineTime=playGameManager.active_MuserAVGOnLineTime(startDay,endDay,fenqu,qudao);
				  if(avgOnLineTime!=null){
				   channel_regday_nums[2] =avgOnLineTime;//平均在线时长
				    }
				    
				   String pay_userCount=playGameManager.active_pay_MuserCount(startDay,endDay,fenqu,qudao);//独立付费人数
				  if(pay_userCount!=null){
				   channel_regday_nums[3] =pay_userCount;//独立付费人数
				    }
				    
				  String pay_Money=playGameManager.active_pay_MMoney(startDay,endDay,fenqu,qudao);
				  if(pay_Money!=null){
				   channel_regday_nums[4] =pay_Money;//付费总金额(元)
				}
  
                
				  String pay_AVGTimes=playGameManager.active_pay_MAVGTimes(startDay,endDay,fenqu,qudao);
				  if(pay_AVGTimes!=null){
				   channel_regday_nums[5] =pay_AVGTimes;//平均付费次数
				  }
				
				  //String spend_userCount=playGameManager.active_spend_MuserCount(startDay,endDay,fenqu,qudao);
				  String spend_userCount=null;
				  if(spend_userCount!=null){
				   channel_regday_nums[6] =spend_userCount;//独立消费人数
				  }
				 //String spend_money=playGameManager.active_spend_Mmoney(startDay,endDay,fenqu,qudao);
				 String spend_money=null;
				 if(spend_money!=null){
				   channel_regday_nums[7] =spend_money;//消费金额
				}
				
				 //String left_yuanbao=playGameManager.active_left_Myuanbao(startDay,endDay,fenqu,qudao);
				 String left_yuanbao=null;
				 if(left_yuanbao!=null){
				channel_regday_nums[8] =left_yuanbao;//元宝库存
				}
				
				 //String left_youxibi=playGameManager.active_left_Myouxibi(startDay,endDay,fenqu,qudao);
...
<html>
	...
	<body>
		...
		<br>
		<i>月活跃用户定义：1月内独立登录12次或以上，或累计在线时长达到24小时或以上</i>
		<br>
	</body>
</html>
