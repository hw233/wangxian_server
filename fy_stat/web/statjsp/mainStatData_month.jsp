...
synchronized(lock){
     List<Date[]> zhouList=DateUtil.splitTime(s,t,2);//把时间段时间分周
	...
	realChannelList.add("游戏币库存");
		...
		//long channel_regday_nums[][] = long [0][0];
       ...
       for(int k = 0 ; k < dayList.size() ; k++){
				...
				channel_regday_nums[1][k] =youXiaoUserCount;//有效注册人数
				  ...
				  //Long zhuCeNOCreatPlayer=userManager.getZhuCeNOCreatPlayer(day[0],day[1],qudao,null);
				  
				  
				   Long zhuCeNOCreatPlayer=0L;
				   Long reg_LoginUserCount_all=playGameManager.getReg_LoginUserCount(day[0],day[1],qudao,null,null);//注册并登陆的用户，不分区
				  if(zhuCeNOCreatPlayer!=null&&reg_LoginUserCount_all!=null){
				...
				channel_regday_nums[6][k] =maxOnlineUserCount;//每分钟最高在线
				 ...
				 Long avgOnLineTime=playGameManager.getAvgOnLineTime(day[0],day[1],qudao,fenqu,null);
				 if(avgOnLineTime!=null){
				 Long avgOnline=avgOnLineTime/60000;
				channel_regday_nums[7][k] =avgOnline;//平均在线时长
				}
				...
				channel_regday_nums[9][k] =unAheadChongZhiUserCount;//新付费人数
				 ...
				 out.print(youXiaoUerAVGOnLineTime);
				 channel_regday_nums[10][k] =fenzheng;//有效用户平均在线时长
				   }
				
				
				 Long chongZhiCount=chongZhiManager.getChongZhiCount(day[0],day[1],qudao,fenqu,null);//分区充值的金额
				 if(chongZhiCount!=null){
				channel_regday_nums[11][k] =chongZhiCount;//充值金额
				}
				
				 Long chongZhiCount_all=chongZhiManager.getChongZhiCount(day[0],day[1],qudao,null,null);//全部充值金额
