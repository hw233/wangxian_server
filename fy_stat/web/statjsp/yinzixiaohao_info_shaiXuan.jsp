			...
			for( int g=0;g< ls_YinDingXiaoHao.size();g++){
			   ...
			   for(int k = 0 ; k < dayList.size() ; k++){
				...
				boolean display=false;
				 ...
				 for(int k = 0 ; k < dayList.size() ; k++){
				 long tt= (tuxing[0][k]>0?tuxing[0][k]:-tuxing[0][k])-(tuxing[1][k]>0?tuxing[1][k]: -tuxing[1][k]);
				 if(tt>=Long.parseLong(money)){
				 display=true; break;
				 }
				 }
				if(display){
				...
				out.println("<table id='test2' align='center' width='90%' cellpadding='1' bgcolor='#000000' cellspacing='1' border='0'>");
		    		out.print("<tr bgcolor='#EEEEBB'><td>日期</td>");
		    		 for(int k = 0 ; k < dayList.size() ; k++){
			            ...
			            out.print("</tr>");
		    		out.print("<tr bgcolor='#FFFFFF'> <td>库存增加</td>");
		    		 for(int k = 0 ; k < dayList.size() ; k++){
			            out.print("<td>"+tuxing[0][k]+"</td>");
			            }
			            out.print("</tr>");
			            out.print("<tr  bgcolor='#FFFFFF'> <td>存储变化</td>");
			          for(int k = 0 ; k < dayList.size() ; k++){
