...
-->
  ...
  window.location.replace("leftframe.jsp?exit=true");
	...
	//gm的id
	out.print("管理员：" + session.getAttribute("username")
			+ "你好!");
	...
	for (Authority a : roleids) {
		...
		for (String mid : mids) {
		    if(!usermodules.contains(mtmanager.getModule(mid)))
	...
	//out.print("<br/>文件夹的长度为："+directoryMap.size());
	 
	    if("admin".equals(username)){
			ArrayList<String[]> directory = new ArrayList<String[]>();
			directoryMap.put("后台用户系统",directory);
			directory.add(new String[]{"功能页面列表","types_manager.jsp"});
			directory.add(new String[]{"角色管理","trole_list.jsp"});
			directory.add(new String[]{"账号管理","gmuser_list.jsp"});
		}	
%>
		<script type=text/javascript>
		<!--
...
<%String pageUrl = request.getRequestURL().toString();
		...
		<span style="padding: 20px 20px 20px 15px">
		 <a href="javascript:window.parent.parent.close()">退出系统</a>
		  </span>
