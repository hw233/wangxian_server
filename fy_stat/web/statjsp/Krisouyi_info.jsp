...
<%@ page
	...
	com.xuanzhi.tools.text.*,com.sqage.stat.commonstat.entity.*,com.sqage.stat.commonstat.dao.UserDao,
	com.sqage.stat.commonstat.manager.UserManager,com.sqage.stat.commonstat.manager.Impl.*,com.sqage.stat.service.*,
	com.sqage.stat.model.Channel,java.text.SimpleDateFormat"
	%>
  ...
  <%
  	...
  	ArrayList<String> typeList = chongZhiManager.getTypes(null);//获得现有的充值类型
  	...
  	for (int days = 0; days < 100; days++)///展示日期数
  	{
  		daysList.add(new Integer(days).toString());
  	}
  	   long channel_regday_nums[][] = new long[realChannelList.size()][daysList.size()];
  	   long result[][] = new long[daysList.size()][2];//  记录充值钱数，和充值人数的和     result[][0] 充值金额， result[][1]充值人数
       ...
       for (int regday=0;regday<regdayList.size();regday++) {//注册日期列表
            String regdayStr=regdayList.get(regday);
  		    ...
  		    Long reg_LoginUserCount=playGameManager.getReg_LoginUserCount(regdayStr,regdayStr,qudao,null,null);//当天注册，在分区道登陆的用户
  		     ...
  		     map.put(regdayStr,reg_LoginUserCount);
				  ...
				  long daynum=0L;
				    ...
				    if(reg_LoginUserCount!=0){
				         ...
				         channel_regday_nums[regday][(int)daynum]=daymoney;
				         result[(int)daynum][0]+=daymoney;
				         result[(int)daynum][1]+=map.get(regdayStr);
				     }
				      } 
					}
  	               }
  		                for (int regdaynum = 0; regdaynum < realChannelList.size(); regdaynum++) {
  		                    ...
  		                    String regdayStr = realChannelList.get(regdaynum);//注册日期串

                            Date date1 = new SimpleDateFormat("yyyy-MM-dd").parse(regdayStr); //注册日期
	                        Date date2 = new SimpleDateFormat("yyyy-MM-dd").parse(today); 
	                      //注册日期到今天的天数 ;日期相减得到相差的日期 
	                        long regTotodaydaynum = (date1.getTime()-date2.getTime())/(24*60*60*1000)>0 ? (date1.getTime()-date2.getTime())/(24*60*60*1000):
	                            (date2.getTime()-date1.getTime())/(24*60*60*1000);   		                   

		    				for (int daynum = 0; daynum < daysList.size(); daynum++) {
		    				if(daynum!=0&&channel_regday_nums[regdaynum][daynum]==0&&daynum<=regTotodaydaynum){
		    		...
		    		<%
		    			...
		    			for (int regdaynum = 0; regdaynum < realChannelList.size(); regdaynum++) {
		    				...
		    				for (int daynum = 0; daynum < daysList.size(); daynum++) {
		    				 ...
		    				 if(channel_regday_nums[regdaynum][daynum] ==0){
		    				 out.print("<td>&nbsp;</td>");
		    				 }else{
		    			...
		    			out.print("<tr bgcolor='#EEEEBB'><td>加权平均</td>");
		    			for (int daynum = 0; daynum < daysList.size(); daynum++) {
		    			if(result[daynum][1]!=0){
		    				out.print("<td>" + result[daynum][0]/result[daynum][1] + "</td>");}
		    				else{
		    				out.print("<td>&nbsp;</td>");}
		    				}
		    			out.print("</tr>");
