...
<%@ page
  ...
  <%
	...
	realChannelList.add("独立进入（分区排重）");
		...
		//long channel_regday_nums[][] = new long[0][0];
	  synchronized(lock){
       ...
       for(int k = 0 ; k < dayList.size() ; k++){
				...
				if(registUserCount!=null&&reg_LoginUserCount_all!=null){
			     zhuCeNOCreatPlayer= registUserCount -reg_LoginUserCount_all;
				  ...
				  channel_regday_nums[3][k] =enterGameUserCount;//独立登陆(分区不排重)
				  ...
				  channel_regday_nums[6][k] =maxOnlineUserCount;//最高在线人数
				 ...
				 if(avgOnLineTime!=null){
				  ...
				  channel_regday_nums[7][k] =fenzhong;//平均在线时间
                       ...
                       <option value="IOS" 
                        ...
                        if("IOS".equals(jixing)){ out.print(" selected=\"selected\"");    }
                         %>>IOS</option>
                           ...
                           <option value="web" 
                        <%
                        if("web".equals(jixing)){ out.print(" selected=\"selected\"");    }
                         %>>web</option>
                  </select>
		        <br/>
