...
<%@ page
  ...
  <%
	...
	if(endDay == null) endDay = today;
            ...
            +" ( select to_char(t.createdate,'yyyyiw') as week,t.fenqu,t.username,sum(t.award) num   from stat_npcinfo t "
            +" where t.npcname='武神之巅'  and t.tasktype like '%战斗%' and t.createdate between to_date('"+startDay+" 00:00:00','YYYY-MM-DD hh24:mi:ss') "
            +" and to_date('"+endDay+" 23:59:59','YYYY-MM-DD hh24:mi:ss') "
            +" group by to_char(t.createdate,'yyyyiw'),t.fenqu,t.username "
            +" order by sum(t.award) desc  ) where  rownum<101 ";
		...
		<form  name="form1" id="form1" method="post">
		  ...
		  结束日期： <input type="text" name="endDay" id="endDay" class="nqz_input" style="width:80px;" readonly="readonly" onfocus="new Calendar(false,'yyyy-MM-dd').showTimePicker(this);" value="<%=endDay %>"/>
		   ...
		   <input type="hidden" name="today" id="today"  value="<%=today %>"/>
					...
					out.print("<tr bgcolor='#EEEEBB'><td>年周</td><td>服务器名</td><td>角色名</td><td>获得积分数</td></tr>");
