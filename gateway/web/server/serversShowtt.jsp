...
function startchecked(tag){
	if(tag){
		if(tag.checked==true){
			id=self.setInterval("somefunction()",200000);
		}else{
...
<body style="font-size: 12px; background-color: #c8edcc">
	...
	out.print("startcheck:"+startcheck);
...
%>
		...
		<table border="0" cellpadding="0" cellspacing="1" width="80%"
			...
			<tr bgcolor="#00FFFF" align="center">
				...
				<td>服务器类型</td>
				<td>真实名称</td>
				<td>单服地址</td><td>启动检查</td><td>关闭检查</td><td><input type="checkbox" onclick='startchecked(this)'></td>
			</tr>
			<%
		Map<String,String> map = new TreeMap<String,String>();
		String serverurl = "http://116.213.192.200:8110/game_boss/getServerUrl.jsp";
		...
		for(MieshiServerInfo s : showServers){
			...
			<tr align="center"
				...
				<td style="background-color:" id='<%=s.getRealname() %>stop'></td>
			</tr>
			<%}%>
