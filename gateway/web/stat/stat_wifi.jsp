...
<%@ page
	...
	public String getSql(String startDay,String endDay,String SPV,String channel){
		String sql = "select R.aaa,R.ccc,T.ccc from ( "+
				"select aaa,count(1) ccc from( "+
				"		select NTOFC aaa from client c where C.TIMEOFFIRSTCONNECTED >= to_date('"+startDay+"','yyyy-mm-dd') "+ 
				"		and C.TIMEOFFIRSTCONNECTED < to_date('"+endDay+"','yyyy-mm-dd')  and CPVOLC='"+SPV+"' "+
				"		and (C.HASLOGIN='T' or C.HASREGISTER='T') "
				+ (channel==null?"":(" and channel='"+channel+"' "))
				+" ) group by aaa order by ccc desc) R "+
				"		left join ( "+

				"		select aaa,count(1) ccc from( "+
				"		select NTOLC aaa from client c where C.TIMEOFFIRSTCONNECTED >= to_date('"+startDay+"','yyyy-mm-dd') "+ 
				"		and C.TIMEOFFIRSTCONNECTED < to_date('"+endDay+"','yyyy-mm-dd')  and CPVOLC='"+SPV+"' "+
				"		and (C.HASLOGIN<>'T' and C.HASREGISTER<>'T') "
				+ (channel==null?"":(" and channel='"+channel+"' "))
				+" ) group by aaa order by ccc desc) T "+
				"		on R.aaa = T.aaa ";
	...
	public String getSqlForMysql(String startDay,String endDay,String SPV,String channel){
		String sql = "select R.aaa,R.ccc,T.ccc from ( "+
				"select aaa,count(1) ccc from( "+
				"		select NTOFC aaa from CLIENT_S1 C where C.TIMEOFFIRSTCONNECTED >= str_to_date('"+startDay+"','%Y-%m-%d') "+ 
				"		and C.TIMEOFFIRSTCONNECTED < str_to_date('"+endDay+"','%Y-%m-%d')  and CPVOLC='"+SPV+"' "+
				"		and (C.HASLOGIN='T' or C.HASREGISTER='T') "
				+ (channel==null?"":(" and channel='"+channel+"' "))
				+" ) DD group by aaa order by ccc desc) R "+
				"		left join ( "+

				"		select aaa,count(1) ccc from( "+
				"		select NTOLC aaa from CLIENT_S1 C where C.TIMEOFFIRSTCONNECTED >= str_to_date('"+startDay+"','%Y-%m-%d') "+ 
				"		and C.TIMEOFFIRSTCONNECTED < str_to_date('"+endDay+"','%Y-%m-%d')  and CPVOLC='"+SPV+"' "+
				"		and (C.HASLOGIN<>'T' and C.HASREGISTER<>'T') "
				+ (channel==null?"":(" and channel='"+channel+"' "))
				+" ) AA group by aaa order by ccc desc) T "+
				"		on R.aaa = T.aaa ";
...
<html>
	...
	<body>
		...
		<tr bgcolor='#EEEEBB'>
			<td>联网类型</td><td>客户端总数</td><td>成功注册数量</td><td>没有注册数量</td><td>成功注册率</td>
		</tr>
		<%
