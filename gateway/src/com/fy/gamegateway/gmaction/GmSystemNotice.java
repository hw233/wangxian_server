...
public class GmSystemNotice {
	...
	public void sendNotice(String content,int type) {
		...
		try {
					...
					for(int k=0;k<this.getSendnum();k++){
						if(this.getUrlnames()!=null&&this.getUrlnames().length>0){
							...
							for(int j=0;j<urls.length;j++){
								...
								try {
									byte [] b = HttpUtils.webPost(new URL(urls[j]+"admin/"+"system_chatmessage.jsp"), args.getBytes(), new HashMap(), 60000, 60000);
			...
			e.printStackTrace();
			GmSystemNoticeManager.logger.warn("[发送公告] [异常] [type:"+type+"]",e);
		}
	}
	...
	public boolean isvalid(long now) {
		...
		if(days!=null && days.length>0){
			try{
				String maxDay = days[days.length-1]+" "+limitEndTime;
				GmSystemNoticeManager.logger.warn("[是否有效] [id:"+id+"] [记录人:"+recorder+"] [记录时间:"+sdf.format(creattime)+"] [结束时间:"+maxDay+"] [下次发送:"+sdf.format(lastSendTime+messLength)+"]");
	...
	public String toString() {
		return " [cons=" + Arrays.toString(cons) + ", creattime="
				+ creattime + ", days=" + Arrays.toString(days) + ", id=" + id
				+ ", lastSendTime=" + lastSendTime + ", limitBeginTime="
				+ limitBeginTime + ", limitEndTime=" + limitEndTime
				+ ", messLength=" + messLength + ", messesLength="
				+ messesLength + ", recorder=" + recorder + ", sendnum="
				+ sendnum + ", sentsystemnotice=" + sentsystemnotice
				+ ", state=" + state + ", timeType=" + timeType + ", urlnames="
				+ Arrays.toString(urlnames) + "]";
	}
