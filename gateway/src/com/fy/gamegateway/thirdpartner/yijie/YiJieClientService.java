package com.fy.gamegateway.thirdpartner.yijie;
...
import com.fy.gamegateway.thirdpartner.yijie.sdk.HTTPHelper;

public class YiJieClientService {

	// 应用在易接服务获取的同步密钥
	private final String PRIVATE_KEY = "SIA23ACRAH7HB09FWACQKLA791EMISGK";
	private final int LOGIN_RESULT_SUCCESS = 0;
	private String checkUrl = "http://sync.1sdk.cn/login/check.html";
	Logger logger = NewLoginHandler.logger;
	private static YiJieClientService self;
	public static YiJieClientService getInstance(){
		if(self == null){
			self = new YiJieClientService();
		}
		return self;
	}
	...
	public static final String [] keys = {"F52F35C5A04A1876","11B5148EC73B3613","FC9729AE50FCAD69","FB589C6DEBE9A197","F64D801F9C555C7C","5FE465A070528702","F7373E8EBC2700D7","B4447B49BC295EFE","E7FDED8015C8FD56",
		...
		"8B5495BDFB0B9465","C7DD907515B158C6","EA968F0B6789F4D6","82342231B0181D53","6C50D0EFE64CE1C9","B99FC5AD0452CB37","8CB8805B292F4383","6FF74F1743CF5222","4A5B65B6A8DC61EB","1A48430570669AFC"};
	...
	public static final String [] values = {"UC","斯凯科技（冒泡客）","木蚂蚁","当乐网","腾讯应用宝","电信爱游戏","联通沃商店","豌豆荚","360","金立    ","移动游戏基地","华为","丫丫玩   ","小米    ","安智市场    ","4399手机游戏","机锋网",
		...
		"炫玩（越狱）","清流游戏(越狱)","乐读科技（越狱）","自由网络","乐嗨嗨游戏(B)"};
	...
	public boolean checkLogin(String username,String pwd){
		try {
			String app = pwd.split("###@@@")[2];
			String sdk = pwd.split("###@@@")[1];
			String sess = pwd.split("###@@@")[0];
			String uin = username.split("YIJIEUSER_")[1];
			
			StringBuilder getUrl = new StringBuilder();
			getUrl.append(checkUrl);
			getUrl.append("?app=");
			getUrl.append(app);
			getUrl.append("&sdk=");
			getUrl.append(sdk);
			...
			if (ret.code != 200) {
				logger.warn("[用户登录易接] [失败:code不是200] [username:"+username+"] ["+pwd+"]");
				return false;
			}
			if (ret.data == null || ret.data.length == 0) {
				logger.warn("[用户登录易接] [失败:data不存在] [username:"+username+"] ["+pwd+"]");
				return false;
			} else {
				String r = new String (ret.data);
				Integer i = new Integer(r);
				if (i == LOGIN_RESULT_SUCCESS) {
					logger.warn("[用户登录易接] [成功] [username:"+username+"] ["+pwd+"] ["+getChannelStr(sdk)+"]");
					return true;
				} else {
					logger.warn("[用户登录易接] [失败:data不存在] [username:"+username+"] ["+pwd+"]");
				}
			...
			e.printStackTrace();
			logger.warn("[用户登录易接] [异常] [username:"+username+"] ["+pwd+"]",e);
		}
		return false;
	}
	...
	public boolean checkLogin2(String username,String pwd){
		try {
			...
			String sdk = pwd.split("###@@@")[1];
			String sess = pwd.split("###@@@")[0];
			String uin = username.split("YIJIEUSER2_")[1];
	...
	public boolean checkLogin3(String username,String pwd){
		try {
			...
			String sdk = pwd.split("###@@@")[1];
			String sess = pwd.split("###@@@")[0];
			String uin = username.split("YIJIEUSER3_")[1];
