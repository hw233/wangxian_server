 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>addresses.length</td><td>int</td><td>4</td><td>数组长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>addresses[0].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>addresses[0]</td><td>String</td><td>addresses[0].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>addresses[1].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>addresses[1]</td><td>String</td><td>addresses[1].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>addresses[2].length</td><td>int</td><td>2</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FAFAFA" align="center"><td>addresses[2]</td><td>String</td><td>addresses[2].length</td><td>字符串对应的byte数组</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td colspan=4>......... 重复</td></tr>
 ...
 * </table>
 */
public class GET_SERVER_RES implements ResponseMessage{
	...
	public GET_SERVER_RES(long seqNum,String[] names,int[] playerCount,byte[] status,String[] addresses,byte[] clientIDs,byte[] serverIDs){
		this.seqNum = seqNum;
		this.names = names;
	...
	public GET_SERVER_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += 4;
		if(len < 0 || len > 4096) throw new Exception("array length ["+len+"] big than the max length [4096]");
		addresses = new String[len];
		for(int i = 0 ; i < addresses.length ; i++){
			len = (int)mf.byteArrayToNumber(content,offset,2);
			offset += 2;
			if(len < 0 || len > 16384) throw new Exception("string length ["+len+"] big than the max length [16384]");
			addresses[i] = new String(content,offset,len);
		offset += len;
		}
		len = (int)mf.byteArrayToNumber(content,offset,4);
		offset += 4;
	...
	public int getLength() {
		...
		for(int i = 0 ; i < addresses.length; i++){
			len += 2;
			len += addresses[i].getBytes().length;
		}
		len += 4;
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			for(int i = 0 ; i < addresses.length; i++){
				byte[] tmpBytes2 ;
				tmpBytes2 = addresses[i].getBytes();
				buffer.putShort((short)tmpBytes2.length);
				buffer.put(tmpBytes2);
			}
