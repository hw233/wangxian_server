 ...
 * </table>
 */
public class ANNOUNCEMENT_RES implements ResponseMessage{
	...
	public ANNOUNCEMENT_RES(){
	}

	public ANNOUNCEMENT_RES(long seqNum,int announcetype,String[] contents){
		this.seqNum = seqNum;
		this.announcetype = announcetype;
		this.contents = contents;
	}

	public ANNOUNCEMENT_RES(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		...
		for(int i = 0 ; i < contents.length ; i++){
			...
			contents[i] = new String(content,offset,len);
		offset += len;
		}
	}
	...
	public int getLength() {
		...
		for(int i = 0 ; i < contents.length; i++){
			len += 2;
			len += contents[i].getBytes().length;
		}
		packet_length = len;
		return len;
	}
