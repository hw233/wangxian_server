...
<%
	...
	for(int i = 0 ; i < users.length ; i++){
		...
		for(int j = 0; j < roles.length ; j++){
			...
			if(!roles[j].getId().equals("modify_password") 
					&& !roles[j].getId().equals("wangxian_gm_captain") 
					&& !roles[j].getId().equals("wangxian_gm_manager") 
					&& !roles[j].getId().equals("gm_tools") 
					&& !roles[j].getId().equals("game_monitor")
					&& !roles[j].getId().equals("role_gm_common")
					&& !roles[j].getId().equals("role_gm_captain")) {
				hasOther = true;
			}
		}
		...
		out.print("<td>"+sb+"</td>");
		out.print("<td><a href='./gmpassword.jsp?action=modify&name="+u.getName()+"'>重置密码</a></td>");
		out.print("<td><a href='./gmuser.jsp?action=remove&name="+u.getName()+"'>删除</a></td>");
		out.print("</tr>\n");
	}
%></table><br>
<a href='./gmuser.jsp?action=add'>添加新的用户</a>
