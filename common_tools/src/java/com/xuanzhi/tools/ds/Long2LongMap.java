...
/**
 * 实现一个long2long HashMap
 * 可以节省HashMap产生的大量的小对象
 * 
 * 其内部实现是采用了两个long数组来实现的。
 * 第一个数组为entryArray，存储(key,value)对，长度为2的幂
 * 第二个数组为conflictEntry，存储产生冲突的(key,value)对,长度为实际冲突的个数。
 * 
 * 这个实现采用了long数组，替代了HashMap中Map.Entry数组，
 * 在存储大量long对时，会节省不少内存。统计采用数组实现，也提高了查询效率。
 * 
 ...
 *
 */
public class Long2LongMap implements java.io.Serializable{

	private static final long serialVersionUID = 1546346464356436L;

	private boolean debug = false;
	
	private long reservedKey;
	...
	//i*3+1 value
	//i*3+2 nextentry
	private long entryArray[];
	private int size;
	
	//i*3+0 key
	//i*3+1 value
	//i*3+2 nextentry
	private long conflictEntry[];
	...
	public Long2LongMap(long reservedKey,long reservedValue){
		this(reservedKey,reservedValue,16);
	}
	
	public Long2LongMap(long reservedKey,long reservedValue,int initialCapacity){
		this(reservedKey,reservedValue,initialCapacity,false);
	}
	
	public Long2LongMap(long reservedKey,long reservedValue,int initialCapacity,boolean debug){
	...
	public boolean check(){
		for(int i = 0 ; i < conflictSize ; i++){
			...
			int next2 = -1;
			int curr2 = -1;
			curr2 = (int)entryArray[3*k2+2];
			if(curr2 == -1){
				return false;
			}
			next2 = (int)conflictEntry[3*curr2+2];
			while(curr2 != i){
				curr2 = next2;
				if(curr2 == -1){
					return false;
	...
	public void clear(){
		conflictSize = 0;
		conflictEntry = new long[(initialCapacity/8<8?8:initialCapacity/8)*3];

		size = 0;
		entryArray = new long[initialCapacity*3];
	...
	private void rehash(){
		long []oldEntryArray = entryArray;
		long []oldConflictEntry = conflictEntry;
		int oldConflictSize = conflictSize;
		
		entryArray = new long[(entryArray.length+entryArray.length)];
		int n = entryArray.length/3;
		for(int i = 0 ; i < n; i++){
			entryArray[3*i] = reservedKey;
			entryArray[3*i+1] = reservedValue;
		...
		conflictSize = 0;
		conflictEntry = new long[(oldConflictSize<8?8:oldConflictSize)*3];
		n = conflictEntry.length/3;
		for(int i = 0 ; i < n ; i++){
			conflictEntry[3*i] = reservedKey;
			conflictEntry[3*i+1] = reservedValue;
	...
	public void put(long key,long value){
		...
		if(entryArray[3*k] == reservedKey){
			...
			if(conflictSize == conflictEntry.length/3){
				long []oldConflictEntry = conflictEntry;
				conflictEntry = new long[oldConflictEntry.length+oldConflictEntry.length/2];
				System.arraycopy(oldConflictEntry, 0, conflictEntry, 0, oldConflictEntry.length);
			}
			
			int curr = -1;
			int next = (int)entryArray[3*k+2];
			while(next != -1){
				...
				curr = next;
				next = (int)conflictEntry[3*next+2];
			}
			if(curr == -1){
	...
	public void remove(long key){
		...
		if(entryArray[3*k] == reservedKey){
			...
			int curr_prev = -1;
			int curr = (int)entryArray[3*k+2];
			int next = -1;
			if(curr != -1) next = (int)conflictEntry[3*curr+2];
	...
	private int hash(long h){
		...
		return (int)(h & (entryArray.length/3 -1));
	}
}
