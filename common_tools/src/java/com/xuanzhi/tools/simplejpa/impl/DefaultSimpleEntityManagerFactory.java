 ...
 * 此类需要一个配置文件simpleEMF.xml,此文件必须手动指定
 * 
 * 下面是此xml结构
 * 
 ...
 *
 */
public class DefaultSimpleEntityManagerFactory {
	
	static Logger logger = LoggerFactory.getLogger(DefaultSimpleEntityManagerFactory.class);
	
	boolean initialized = false;
	boolean initialize_success = false;
	HashMap<String,SimpleEntityManager<?>> emMap = new HashMap<String,SimpleEntityManager<?>>();
	ConnectionPool pool = null;
	int serverId;
	String serverName;
	ArrayList<EntityManagerInfo> eiList = new 	ArrayList<EntityManagerInfo>();
	String userName;
	
	private  Map<Integer,String> cidInfo = new HashMap<Integer, String>();
	
	//private static Map<Integer,String> classInfo = new HashMap<Integer, String>();
	
	String configFilePath = null;
	
	public DefaultSimpleEntityManagerFactory(String configFileFullPath){
	...
	static class EntityManagerInfo{
		Class<?> cl;
		ArrayList<Class<?>> subClass = new ArrayList<Class<?>>();
		int checkIntervalInSeconds;
		int maxRowPerTableForMysql = 5000000;
		Map<Class<?>,Integer> classInfo = new HashMap<Class<?>,Integer>();
	}
	
	public <T> SimpleEntityManager<T> getSimpleEntityManager(Class<T> cl){
		if(initialized == false){
			synchronized(DefaultSimpleEntityManagerFactory.class){
				if(initialized ==false){
					try{
						init();
	 ...
	 * @return
	 */
	private <T> SimpleEntityManager<T> createMysqlSimpleEntityManager(Class<T> cl,EntityManagerInfo info){
	...
	private <T> SimpleEntityManager<T> createSimpleEntityManager(Class<T> cl,EntityManagerInfo info){
	...
	private void init() throws Exception{
		...
		//checkIntervalInSeconds = XmlUtil.getValueAsInteger(XmlUtil.getChildByName(root, "checkIntervalInSeconds"), 30);
		...
		for(int i = 0 ; i < eles.length ; i++){
			...
			ei.maxRowPerTableForMysql = XmlUtil.getAttributeAsInteger(eles[i], "maxRowPerTableForMysql", 5000000);
