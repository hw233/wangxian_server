...
public class QueueSelectorThroughPutTestCase2 extends TestCase{

	public QueueSelectorThroughPutTestCase2(){
		super();
	}
	int queueNum = 1000;
	int beatheartNum = 50;
	...
	public static void main(String args[]) throws Exception{
		QueueSelectorThroughPutTestCase2 qqq = new QueueSelectorThroughPutTestCase2();
		
		for(int i = 0 ; i < args.length ; i++){
			if(args[i].equals("-h")){
				System.out.println("Usage  : java -cp . QueueSelectorThroughPutTestCase2 -q queuenum -b beatheartNum -s selectThreadNum -n messageNum -m queueMaxSize -c handleMessageCost");
	...
	public void testThroughPut() throws Exception{
		...
		final XXXQueueSelector selector = new XXXQueueSelector(queueNum);
		final XXXSelectableQueue queues[] = new XXXSelectableQueue[queueNum];
		for(int i = 0 ; i < queueNum ; i++){
			queues[i] = new XXXSelectableQueue(queueMaxSize);
		...
		System.out.println("Usage  : java -cp . QueueSelectorThroughPutTestCase2 -q queuenum -b beatheartNum -s selectThreadNum -n messageNum -m queueMaxSize");
		System.out.println("Author : myzdf");
		System.out.println("Version: 0.1");
	...
	static class MySelectThread implements Runnable{
		...
		XXXQueueSelector selector ;
		int handleMessageCost;
		
		public MySelectThread(XXXQueueSelector selector,int handleMessageCost){
		...
		public void run() {
			while(Thread.currentThread().isInterrupted() == false){
				XXXSelectableQueue queue = selector.select(1000);
				if(queue != null){
					Message m = null;
	...
	static class MySendThread implements Runnable{
		...
		int num;
		int messageNum;
		XXXSelectableQueue queues[] = null;
		
		public MySendThread(int index,int offset,int num,int messageNum,XXXSelectableQueue[] queues){
			this.index = index;
			this.offset = offset;
			this.num = num;
		...
		public void run() {
			...
			while(Thread.currentThread().isInterrupted() == false){
				...
				for(int i = offset ; i < offset+num ; i++){
					XXXSelectableQueue dq = queues[i];
