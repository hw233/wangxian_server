 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>oldpasswd.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>oldpasswd</td><td>String</td><td>oldpasswd.length</td><td>字符串对应的byte数组</td></tr>
 ...
 * </table>
 */
public class MODIFY_PASSWD_REQ implements RequestMessage{
	...
	public MODIFY_PASSWD_REQ(long seqNum,String username,String oldpasswd,String newpasswd,String question,String answer){
		this.seqNum = seqNum;
	...
	public MODIFY_PASSWD_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += 2;
		if(len < 0 || len > 10240000) throw new Exception("string length ["+len+"] big than the max length [10240000]");
		oldpasswd = new String(content,offset,len);
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 10240000) throw new Exception("string length ["+len+"] big than the max length [10240000]");
		newpasswd = new String(content,offset,len);
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 10240000) throw new Exception("string length ["+len+"] big than the max length [10240000]");
	...
	public int getLength() {
		...
		len += 2;
		len +=oldpasswd.getBytes().length;
		len += 2;
		len +=newpasswd.getBytes().length;
		len += 2;
		try{
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			tmpBytes1 = oldpasswd.getBytes();
			buffer.putShort((short)tmpBytes1.length);
			buffer.put(tmpBytes1);
			tmpBytes1 = newpasswd.getBytes();
			buffer.putShort((short)tmpBytes1.length);
			buffer.put(tmpBytes1);
				try{
