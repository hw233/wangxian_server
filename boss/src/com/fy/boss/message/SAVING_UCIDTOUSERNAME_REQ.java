 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>ucid.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>ucid</td><td>String</td><td>ucid.length</td><td>字符串对应的byte数组</td></tr>
 ...
 * </table>
 */
public class SAVING_UCIDTOUSERNAME_REQ implements RequestMessage{
	...
	public SAVING_UCIDTOUSERNAME_REQ(){
	}

	public SAVING_UCIDTOUSERNAME_REQ(long seqNum,String ucid,String username,String channel){
		this.seqNum = seqNum;
		this.ucid = ucid;
	...
	public SAVING_UCIDTOUSERNAME_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 10240000) throw new Exception("string length ["+len+"] big than the max length [10240000]");
		ucid = new String(content,offset,len,"UTF-8");
		offset += len;
		len = (int)mf.byteArrayToNumber(content,offset,2);
		offset += 2;
		if(len < 0 || len > 10240000) throw new Exception("string length ["+len+"] big than the max length [10240000]");
	...
	public String getTypeDescription() {
		return "SAVING_UCIDTOUSERNAME_REQ";
	}
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=ucid.getBytes("UTF-8").length;
