...
import com.xuanzhi.tools.text.DateUtil;
	...
	public class AppStoreMaLaiSavingManager {
	static AppStoreMaLaiSavingManager self;
	...
	public void initialize(){
		AppStoreMaLaiSavingManager.self=this;
		System.out.println("["+this.getClass().getName()+"] [initialized]");
		PlatformSavingCenter.logger.info("["+this.getClass().getName()+"] [initialized]");	
	}
	
	static {//单位人民币分
		map.put("525_0_1_250", 250L);
		map.put("525_0_2_750", 750L);
		map.put("525_0_3_1250", 1250L);
		map.put("525_0_4_5000", 5000L);
		map.put("525_0_5_7500", 7500L);
		map.put("525_0_6_12500", 12500L);
		map.put("525_0_7_25000", 25000L);
	}
	...
	public String appStoreSaving(Passport passport, String channel, String servername, String receipt, String deviceCode){
		...
		order.setSavingPlatform("MalaiAppstore");
		order.setSavingMedium("MalaiAppstore");
					...
					if(receiptMap.get("transaction_id") != null) {
						...
						if(exists != null) {
							...
							if(addAmount != null && addAmount > 0) {
								order.setPayMoney(addAmount * MaLaiSavingCallBackServlet.factor);
								order.setOrderId(orderID);
								order.setHandleResult(OrderForm.HANDLE_RESULT_SUCC);
								order.setResponseResult(OrderForm.RESPONSE_SUCC);
								OrderFormManager.getInstance().update(order);
								PlatformSavingCenter.logger.warn("[应用商店充值"+(isSandBox?"沙盒":"" )+"订单生成] [成功] [id:"+order.getId()+"] [orderID："+orderID+"] [充值类型：AppStore] [支付平台:AppStore] [paymoney:"+addAmount+"] [用户:"+passport.getUserName()+"] [服务器:"+servername+"] [渠道:"+channel+"] [信息："+result+"] [url:"+RECHARGE_URL+"] [receipt:"+receipt+"] [乘以系数:"+MaLaiSavingCallBackServlet.factor+"] ["+(System.currentTimeMillis()-t)+"ms]");
							} else {
								order.setHandleResult(OrderForm.HANDLE_RESULT_FAILED);
								OrderFormManager.getInstance().update(order);
								PlatformSavingCenter.logger.warn("[应用商店充值"+(isSandBox?"沙盒":"" )+"订单生成] [失败：此交易返回的数据有误（充值额<0）] [id:"+order.getId()+"] [orderID："+orderID+"] [充值类型：AppStore] [支付平台:AppStore] [paymoney:"+addAmount+"] [用户:"+passport.getUserName()+"] [服务器:"+servername+"] [渠道:"+channel+"] [信息："+result+"] [receipt:"+receipt+"] [乘以系数:"+MaLaiSavingCallBackServlet.factor+"] ["+(System.currentTimeMillis()-t)+"ms]");
							} 
						}
					} else {
						order.setHandleResult(OrderForm.HANDLE_RESULT_FAILED);
	...
	public static AppStoreMaLaiSavingManager getInstance(){
		if(self == null) {
			self = new AppStoreMaLaiSavingManager();
		}
		return self;
	}
}
