 ...
 * <tr bgcolor="#FAFAFA" align="center"><td>verifyStr.length</td><td>short</td><td>2个字节</td><td>字符串实际长度</td></tr>
 * <tr bgcolor="#FFFFFF" align="center"><td>verifyStr</td><td>String</td><td>verifyStr.length</td><td>字符串对应的byte数组</td></tr>
 * </table>
 */
public class APPSTORE_SAVING_VERIFY_REQ implements RequestMessage{
	...
	public APPSTORE_SAVING_VERIFY_REQ(){
	}

	public APPSTORE_SAVING_VERIFY_REQ(long seqNum,String username,String channel,String verifyStr){
		this.seqNum = seqNum;
	...
	public APPSTORE_SAVING_VERIFY_REQ(long seqNum,byte[] content,int offset,int size) throws Exception{
		this.seqNum = seqNum;
		int len = 0;
		...
		offset += 2;
		if(len < 0 || len > 10240000) throw new Exception("string length ["+len+"] big than the max length [10240000]");
		verifyStr = new String(content,offset,len,"UTF-8");
		offset += len;
	}
	...
	public String getTypeDescription() {
		return "APPSTORE_SAVING_VERIFY_REQ";
	}
	...
	public int getLength() {
		...
		len += 2;
		try{
			len +=verifyStr.getBytes("UTF-8").length;
		}catch(java.io.UnsupportedEncodingException e){
		 e.printStackTrace();
			throw new RuntimeException("unsupported encoding [UTF-8]",e);
		}
	...
	public int writeTo(ByteBuffer buffer) {
		...
		try{
			...
			buffer.put(tmpBytes1);
				try{
			tmpBytes1 = verifyStr.getBytes("UTF-8");
				}catch(java.io.UnsupportedEncodingException e){
			 e.printStackTrace();
					throw new RuntimeException("unsupported encoding [UTF-8]",e);
				}
