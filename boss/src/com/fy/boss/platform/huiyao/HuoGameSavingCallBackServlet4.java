...
public class HuoGameSavingCallBackServlet4 extends HttpServlet{

	public static final String app_key = "6833a245ca5098f2d8ae9d844bb6ff2e";
...
//		app_id=1&cp_order_id=20161028111&ext=kjsdkjdskjdsdkjs&mem_id=12325&order_id=14794504894304304120001&order_status=2&pay_time=1479450489&product_id=1&product_name=%E5%85%83%E5%AE%9D&product_price=1&app_key=f875364690581668449d4cf0aeb60560
			...
			if(!order_status.equals("2")){
				...
				PlatformSavingCenter.logger.warn("[充值回调] [充值平台：一剑诛仙SDK] [失败:充值失败] [orderid:"+cp_order_id+"] [amount:"+product_price+"] [order_id:"+order_id+"] [order_status:"+order_status+"] [costs:"+(System.currentTimeMillis()-now)+"]");
				return;
			}
			if(sign.equals(mysign)){
				OrderForm order = OrderFormManager.getInstance().getOrderForm(cp_order_id);
				if(order == null){
					PlatformSavingCenter.logger.warn("[充值回调] [充值平台：一剑诛仙SDK] [核验订单状态结果:没找到订单] [orderid:"+cp_order_id+"] [amount:"+product_price+"] [order_id:"+order_id+"] [order_status:"+order_status+"] [costs:"+(System.currentTimeMillis()-now)+"]");
					res.getWriter().write("FAILURE");
					return;
				}
				...
				synchronized(order) {
					if(order.getResponseResult() != OrderForm.RESPONSE_NOBACK) {
						PlatformSavingCenter.logger.error("[充值回调] [充值平台：一剑诛仙SDK] [失败:此订单已经回调过了] [orderid:"+cp_order_id+"] [order_id:"+order_id+"] [amount:"+product_price+"] [gamename:"+order.getServerName()+"] [costs:"+(System.currentTimeMillis()-now)+"]");
						res.getWriter().write("FAILURE");
						return;
					}
					...
					try {
						...
						PlatformSavingCenter.logger.error("[充值回调] [充值平台：一剑诛仙SDK] [更新订单信息失败] [id:"+order.getId()+"] [orderid:"+order.getOrderId()+"] gamename:"+order.getServerName()+"] [数据库中记录金额:"+order.getPayMoney()+"] [costs:"+(System.currentTimeMillis()-now)+"]",e);
						res.getWriter().write("FAILURE");
						return;
					}
				}
				PlatformSavingCenter.logger.error("[充值回调] [成功] [充值平台：一剑诛仙SDK] [订单:"+cp_order_id+"] [order_id:"+order_id+"] [amount:"+product_price+"] [oldPayMoney:"+oldPayMoney+"] [costs:"+(System.currentTimeMillis()-now)+"]");
			}else{
				PlatformSavingCenter.logger.error("[充值回调] [失败:签名失败]  [充值平台：一剑诛仙SDK] [sign:"+sign+"] [mysign:"+mysign+"] [signStr:"+signStr+"] [costs:"+(System.currentTimeMillis()-now)+"]");
				res.getWriter().write("FAILURE");
				return;
			}
		}catch (Exception e) {
			e.printStackTrace();
			PlatformSavingCenter.logger.info("[充值回调] [解析登录返回数据失败] [充值平台：一剑诛仙SDK] ["+(System.currentTimeMillis()-now)+"ms]", e);
			res.getWriter().write("FAILURE");
			return;
		}
	}
