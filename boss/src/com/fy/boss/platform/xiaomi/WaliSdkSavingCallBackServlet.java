...
public class WaliSdkSavingCallBackServlet extends HttpServlet {
			...
			throws ServletException, IOException 
	{
		...
		try
		{
			String appKey = "6feeea8b-d27d-e05f-2c12-516bb33784fa";//正式
			...
			String my_sign = HmacSHA1Encryption.hmacSHA1Encrypt(returnString,appKey);
			int errcode = 200;
			//去掉sign标记 得到所有参数
			//验证sign
				...
				if(orderForm != null)
				{
					synchronized(orderForm) {
						if(orderForm.getResponseResult() != OrderForm.RESPONSE_NOBACK) {
							PlatformSavingCenter.logger.error("[充值回调] [充值平台：瓦力SDK] [失败：订单已经回调过了] [XiaomiSDK订单id:"+channelOrderId+"] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [交易金额:"+payFee+"] [实际金额:"+payFee+"] [订单中原金额:"+orderForm.getPayMoney()+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
							errcode = 1005;
					...
					orderForm.setResponseDesp("瓦力SDK回调充值接口成功");
					if(Long.parseLong(payFee) != orderForm.getPayMoney())
					{
						PlatformSavingCenter.logger.warn("[充值回调] [充值平台：瓦力SDK] [警告：传回的充值金额和订单存储的金额不一致] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [充值金额:"+payFee+"] [传入实际金额:"+payFee+"] [订单中原金额:"+orderForm.getPayMoney()+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
					}
					...
					else
					{
						...
						orderForm.setResponseDesp("瓦力SDK交易失败");
						orderForm.setChannelOrderId(channelOrderId);
					}
					try {
						orderFormManager.update(orderForm);
						if(PlatformSavingCenter.logger.isInfoEnabled())
							PlatformSavingCenter.logger.info("[充值回调] [充值平台：瓦力SDK] [成功] [充值"+("TRADE_SUCCESS".equals(orderStatus) ? "成功" : "失败")+"] [传入瓦力SDKORDERID:"+channelOrderId+"] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [充值金额:"+payFee+"] [扣完手续费传入实际金额:"+payFee+"] [更新入订单的金额:"+Math.round(Double.valueOf(payFee) * 100)+"] [订单中原金额:"+orderForm.getPayMoney()+"] [瓦力SDK用户:"+uid+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
					} catch (Exception e) {
						errcode = 1001;
						PlatformSavingCenter.logger.error("[充值回调] [充值平台：瓦力SDK] [失败：更新订单出错] [传入瓦力SDKORDERID:"+channelOrderId+"] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [充值金额:"+payFee+"] [扣完手续费传入实际金额:"+payFee+"] [更新入订单的金额:"+Math.round(Double.valueOf(payFee) * 100)+"] [订单中原金额:"+orderForm.getPayMoney()+"] [瓦力SDK用户:"+uid+"] [返回结果:"+orderStatus+"] [错误信息:"+orderStatus+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
					}
				}
				else
				{
					PlatformSavingCenter.logger.warn("[充值回调] [充值平台：瓦力SDK] [失败] [查找订单失败] [传入瓦力SDKORDERID:"+channelOrderId+"] [传入orderId:"+orderId+"] [充值金额:"+payFee+"] [扣完手续费传入实际金额:"+payFee+"] [更新入订单的金额:"+payFee+"] [瓦力SDK用户:"+uid+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
				}
			}
			else
			{
				PlatformSavingCenter.logger.error("[充值回调] [充值平台：瓦力SDK] [失败：签名验证失败] [appkey:"+appKey+"] [准备用于签名的字符串:"+returnString+"] [传入的sign:"+sign+"] [my sign:"+my_sign+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
			}
		...
		catch(Exception e)
		{
			...
			try {
				...
				PlatformSavingCenter.logger.error("[充值回调] [充值平台：瓦力SDK] [失败：出现异常] [准备用于签名的字符串:--] [传入的sign:--] [my sign:--] [异常:"+e+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e1);
			}
			res.getWriter().write(json);
			PlatformSavingCenter.logger.error("[充值回调] [充值平台：瓦力SDK] [失败：出现异常] [准备用于签名的字符串:--] [传入的sign:--] [my sign:--] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
			return;
		}
