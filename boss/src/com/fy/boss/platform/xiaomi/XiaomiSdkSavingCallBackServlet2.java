...
public class XiaomiSdkSavingCallBackServlet2 extends HttpServlet {
...
//			String appKey = "512e79c0-a81e-c2af-38b7-505af006f12f";//正式
			String appKey = "5271784710798";//正式
			String AppSecret = "JdBHg+XmZ7ZnONdVFReFKg==";//正式
				...
				if(orderForm != null)
				{
					synchronized(orderForm) {
						if(orderForm.getResponseResult() != OrderForm.RESPONSE_NOBACK) {
							PlatformSavingCenter.logger.error("[充值回调] [充值平台：小米游戏2] [失败：订单已经回调过了] [XiaomiSDK订单id:"+channelOrderId+"] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [交易金额:"+payFee+"] [实际金额:"+payFee+"] [订单中原金额:"+orderForm.getPayMoney()+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
							errcode = 1005;
					...
					{
						PlatformSavingCenter.logger.warn("[充值回调] [充值平台：小米游戏2] [警告：传回的充值金额和订单存储的金额不一致] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [充值金额:"+payFee+"] [传入实际金额:"+payFee+"] [订单中原金额:"+orderForm.getPayMoney()+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
					}
					...
					try {
						...
						if(PlatformSavingCenter.logger.isInfoEnabled())
							PlatformSavingCenter.logger.info("[充值回调] [充值平台：小米游戏2] [成功] [充值"+("TRADE_SUCCESS".equals(orderStatus) ? "成功" : "失败")+"] [传入小米游戏ORDERID:"+channelOrderId+"] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [充值金额:"+payFee+"] [扣完手续费传入实际金额:"+payFee+"] [更新入订单的金额:"+Math.round(Double.valueOf(payFee))+"] [订单中原金额:"+orderForm.getPayMoney()+"] [小米游戏用户:"+uid+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
					} catch (Exception e) {
						errcode = 1001;
						PlatformSavingCenter.logger.error("[充值回调] [充值平台：小米游戏2] [失败：更新订单出错] [传入小米游戏ORDERID:"+channelOrderId+"] [order id:"+orderForm.getId()+"] [my orderId:"+orderForm.getOrderId()+"] [充值金额:"+payFee+"] [扣完手续费传入实际金额:"+payFee+"] [更新入订单的金额:"+Math.round(Double.valueOf(payFee))+"] [订单中原金额:"+orderForm.getPayMoney()+"] [小米游戏用户:"+uid+"] [返回结果:"+orderStatus+"] [错误信息:"+orderStatus+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
					}
				}
				else
				{
					PlatformSavingCenter.logger.warn("[充值回调] [充值平台：小米游戏2] [失败] [查找订单失败] [传入小米游戏ORDERID:"+channelOrderId+"] [传入orderId:"+orderId+"] [充值金额:"+payFee+"] [扣完手续费传入实际金额:"+payFee+"] [更新入订单的金额:"+payFee+"] [小米游戏用户:"+uid+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
				}
			}
			else
			{
				PlatformSavingCenter.logger.error("[充值回调] [充值平台：小米游戏2] [失败：签名验证失败] [appkey:"+appKey+"] [准备用于签名的字符串:"+returnString+"] [传入的sign:"+sign+"] [my sign:"+my_sign+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
			}
			...
			try {
				...
				PlatformSavingCenter.logger.error("[充值回调] [充值平台：小米游戏2] [失败：出现异常] [准备用于签名的字符串:--] [传入的sign:--] [my sign:--] [异常:"+e+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e1);
			}
			res.getWriter().write(json);
			PlatformSavingCenter.logger.error("[充值回调] [充值平台：小米游戏2] [失败：出现异常] [准备用于签名的字符串:--] [传入的sign:--] [my sign:--] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
			return;
		}
	}
