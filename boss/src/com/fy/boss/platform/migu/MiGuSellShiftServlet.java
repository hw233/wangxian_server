...
public class MiGuSellShiftServlet extends HttpServlet {
//TODO 需要限制ip
	//更新相关的订单状态
...
//		}
		...
		if(mysign.equalsIgnoreCase(sign))
		{
					...
					&& !StringUtils.isEmpty(servername) && !StringUtils.isEmpty(platform) && !StringUtils.isEmpty(channel))
			{
				...
				try {
					...
					res.getWriter().write(jacksonManager.toJsonString(map));
					logger.warn("[米谷通讯] [商品上架] [失败] [连接游戏服出现异常] ["+url+"] ["+username+"] ["+jsons+"] ["+servername+"] ["+channel+"] ["+roleid+"]",e);
					return;
				}
				...
				if(!StringUtils.isEmpty(returnString))
				{
					try
					{
						...
						if(logger.isInfoEnabled())
							logger.info("[米谷通讯] [商品上架] [成功] [ok] ["+url+"] ["+username+"] ["+jsons+"] ["+servername+"] ["+channel+"] ["+roleid+"]");
					}
					catch(Exception e)
					{
						logger.warn("[米谷通讯] [商品上架] [失败] [出现未知异常] ["+url+"] ["+username+"] ["+jsons+"] ["+servername+"] ["+channel+"] ["+roleid+"]",e);
					}
				}
				else
				{
					...
					map.put("type", -1);
					map.put("recordid", "");
					logger.warn("[米谷通讯] [商品上架] [失败] [游戏服返回失败] ["+url+"] ["+username+"] ["+jsons+"] ["+servername+"] ["+channel+"] ["+roleid+"]");
				}
			...
			else
			{
				...
				map.put("recordid", "");
				res.getWriter().write(jacksonManager.toJsonString(map));
				logger.warn("[米谷通讯] [商品上架] [失败] [出现不合法的请求] ["+username+"] ["+servername+"] ["+channel+"] ["+roleid+"]");
				return;
			}
		}
		else
		{
			...
			map.put("recordid", "");
			res.getWriter().write(jacksonManager.toJsonString(map));
			logger.warn("[米谷通讯] [商品上架] [失败] [出现不合法的签名] ["+username+"] ["+servername+"] ["+channel+"] ["+roleid+"] ["+sign+"] ["+mysign+"]");
			return;
		}
	}
