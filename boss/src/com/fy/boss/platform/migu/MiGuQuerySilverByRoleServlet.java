...
public class MiGuQuerySilverByRoleServlet extends HttpServlet {
...
//TODO 需要限制ip
			...
			throws ServletException, IOException 
	{
		...
		if(mysign.equalsIgnoreCase(sign))
		{
			...
			String channel = req.getParameter("channel");
				...
				try {
					...
					money = new String(bytes,encoding);
				} catch (Exception e) {
					map.put("result", 4);
					res.getWriter().write(jacksonManager.toJsonString(map));
					logger.warn("[米谷通讯] [获取角色银两] [失败] [fail] [出现异常] ["+url+"] ["+username+"] ["+map+"] ["+servername+"] ["+channel+"] ["+roleid+"]",e);
					return;
				}
				...
				if(logger.isInfoEnabled())
					logger.info("[米谷通讯] [获取角色银两] [成功] [ok] ["+url+"] ["+username+"] ["+money+"] ["+servername+"] ["+channel+"] ["+roleid+"]");
			...
			else
			{
				map.put("result", 3);
				res.getWriter().write(jacksonManager.toJsonString(map));
				logger.warn("[米谷通讯] [获取角色银两] [失败] [fail] [参数不全] ["+username+"] ["+map+"] ["+servername+"] ["+channel+"] ["+roleid+"] ["+req.getParameterMap()+"]");
				return;
			}
		}
		else
		{
			map.put("result", 2);
			res.getWriter().write(jacksonManager.toJsonString(map));
			logger.warn("[米谷通讯] [获取角色银两] [失败] [fail] [签名验证失败] ["+username+"] ["+map+"] ["+servername+"] ["+channel+"] ["+roleid+"] ["+sign+"] ["+mysign+"] ["+req.getParameterMap()+"]");
			return;
		}
	}
}
