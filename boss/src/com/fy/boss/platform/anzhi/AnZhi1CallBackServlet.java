...
public class AnZhi1CallBackServlet extends HttpServlet {
	...
	String appSecret = "E7S3KH3P2ptTzXpa9b8JHgbE";
	...
	protected void service(HttpServletRequest req, HttpServletResponse res)
		...
		try
		{
			...
			if(isSign)
			{
				...
				{
					PlatformSavingCenter.logger.error("[充值回调] [充值平台：安智1] [失败:数据库中无此订单] [安智1 orderId:"+liushuihaoParam+"] [orderId:"+myOrderId+"] [充值金额:"+moneyParam+"] [账号:--] [payResult:"+payResult+"] [appId:"+appidParam+"] [money:"+moneyParam+"] [myOrderid:"+myOrderId+"] [payResult:"+payResult+"] [payType:"+payType+"] [costs:"+(System.currentTimeMillis()-startTime)+"ms]");
					res.getWriter().write("success");
					return;
				}
			}
		...
		{
			PlatformSavingCenter.logger.error("[充值回调] [充值平台：安智1] [失败:出现异常] [安智1 orderId:--] [cid:--] [payResult:--] [appId:--] [appidParam:--] [money:--] [myOrderid:--] [payResult:--] [orderId:--] [充值金额:--] [账号:--]",e);
			res.getWriter().write("success");
			return;
		}
			}
