...
public class DuokuYYAlipaySavingManager {
	static DuokuYYAlipaySavingManager self;
	...
	public void initialize(){
		DuokuYYAlipaySavingManager.self=this;
	...
	public String cardSaving(Passport passport, String cardtype, int payAmount, String cardno, String cardpass,
		...
		//设置是否通知游戏服状态 设为false
			...
			//设置订单号
			order.setOrderId("BAIDUYY_WANGXIAN_"+DateUtil.formatDate(new Date(), "yyyyMMdd") + "-"+ order.getId());
			try
			{
			...
			{
				PlatformSavingCenter.logger.error("[百度多酷影音充值订单生成] [失败] [更新订单信息] [失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [订单的ID:"+order.getId()+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
				return "";
			}
...
access_token值+client_id值+card_num值+ card_pwd 值+ card_type值+ pay_num 值+session_secret值(计算时无+号)
		...
		params.put("appkey", BAIDUYYSavingManager.client_id);
...
//		params.put("orderid", order.getOrderId());
		...
		{
			PlatformSavingCenter.logger.debug("[百度多酷影音充值订单生成] [构造参数] [参数串:"+content+"] [clientId:"+BAIDUYYSavingManager.client_id+"] [gameid:"+BAIDUYYSavingManager.game_id+"] [sessionScret:"+BAIDUYYSavingManager.session_secret+"] [chargeUrl:"+chargeUrl+"]");
		}
		...
		try
			...
			catch(Exception e)
			{
				...
				PlatformSavingCenter.logger.error("[百度多酷影音充值] [调用接口] [失败] [用户id:"+passport.getId()+"] [用户名:"+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [orderForm ID:"+order.getId()+"] [orderId:"+order.getOrderId()+"] [content:"+content+"] [clientId:"+BAIDUYYSavingManager.client_id+"] [gameid:"+BAIDUYYSavingManager.game_id+"] [sessionScret:"+BAIDUYYSavingManager.session_secret+"] [chargeUrl:"+chargeUrl+"] [costs:"+(System.currentTimeMillis()-startTime)+"ms]",e);
				return "";//infoMap.get(6010000);
			}
		}
		catch(Exception e)
		{
			PlatformSavingCenter.logger.error("[百度多酷影音充值] [调用接口] [失败:出现未知异常] [用户id:"+passport.getId()+"] [用户名:"+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [clientId:"+BAIDUYYSavingManager.client_id+"] [gameid:"+BAIDUYYSavingManager.game_id+"] [sessionScret:"+BAIDUYYSavingManager.session_secret+"] [chargeUrl:"+chargeUrl+"]",e);
			return "";
		}
	}
	...
	public static String sign(LinkedHashMap<String,String> params){
		...
		String md5Str = sb.toString()+BAIDUYYSavingManager.session_secret;
		String sign = StringUtil.hash(md5Str).toLowerCase();
		
		if(PlatformSavingCenter.logger.isDebugEnabled())
		{
			PlatformSavingCenter.logger.debug("[百度多酷影音充值] [生成签名串] [签名前:"+md5Str+"] [签名后:"+sign+"] [session_secret:"+BAIDUYYSavingManager.session_secret+"]");
		}
		
		return sign;
	}
			
	public static DuokuYYAlipaySavingManager getInstance(){
		if(self == null)
		{
			self = new DuokuYYAlipaySavingManager();
		}
		return self;
	}
		...
		if(others == null || others.length <= 0)
		{
			...
			if(order != null)
			{
				...
				try {
					...
					// TODO Auto-generated catch block
					PlatformSavingCenter.logger.error("[百度多酷影音充值] [失败] [更新订单] [失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [角色名:"+order.getMemo1()+"] [costs:"+(System.currentTimeMillis()-t)+"]",e);
				}
				
				PlatformSavingCenter.logger.warn("[百度多酷影音充值] [成功] [无角色id传入] [id:"+order.getId()+"] [orderID："+orderId+"] [充值类型："+cardtype+"] [paymoney:"+0+"] [用户:"+passport.getUserName()+"] [角色:"+order.getMemo1()+"] ["+(System.currentTimeMillis()-t)+"ms]");
			}
			else
			{
				PlatformSavingCenter.logger.warn("[百度多酷影音充值] [失败] [根据订单id没有查到订单] [无角色id传入] [orderid:"+orderId+"]");
			}
			return orderId;
		}
		else if(!StringUtils.isEmpty(orderId))
		{
			...
			if(order != null)
			{
				...
				try {
					...
					// TODO Auto-generated catch block
					PlatformSavingCenter.logger.error("[百度多酷影音充值] [失败：更新订单出现异常] [id:"+order.getId()+"] [orderID："+orderId+"] [充值类型："+cardtype+"] [paymoney:"+0+"] [用户:"+passport.getUserName()+"] [角色:"+order.getMemo1()+"] ["+(System.currentTimeMillis()-t)+"ms]", e);
				}
