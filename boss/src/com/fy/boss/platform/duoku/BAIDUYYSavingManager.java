...
public class BAIDUYYSavingManager {
	static BAIDUYYSavingManager self;
	
	
	public static String client_id = "e3941454403d6a9b8754dc1bf13baidu";
	public static String game_id = "136";
	public static String session_secret = "c95b5b9ccfa1c38d022340a1233baidu";
	public static Map<String, String> cardNo = new HashMap<String,String>();
//	public static String chargeUrl = "http://api.m.duoku.com/openapi/client/duokoo_card_recharge";
	public static String chargeUrl =  DuokuSavingManager.chargeUrl;
...
17	亿卡通
	...
	public void initialize(){
		BAIDUYYSavingManager.self=this;
	...
	public String cardSaving(Passport passport, String cardtype, int payAmount, String cardno, String cardpass,
		...
		try
		{
			...
			//设置充值平台
			order.setSavingPlatform("百度多酷影音");
			//设置充值介质
		...
		//设置是否通知游戏服状态 设为false
			...
			{
				PlatformSavingCenter.logger.error("[百度多酷影音充值订单生成] [失败] [更新订单信息] [失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [订单的ID:"+order.getId()+"]  [订单id:"+order.getOrderId()+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
				return "";
			}
...
access_token值+client_id值+card_num值+ card_pwd 值+ card_type值+ pay_num 值+session_secret值(计算时无+号)
		...
		try {
			...
			PlatformSavingCenter.logger.error("[百度多酷影音充值订单生成] [调用接口失败] [失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [url:"+chargeUrl+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
			return "";
		}
		...
		{
			PlatformSavingCenter.logger.debug("[百度多酷影音充值订单生成] [构造参数] [参数串:"+content+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"]");
		}
...
client_secret	orderid+ session_secret的MD5码（字母小写）	session_secre预先分配合作方
		        ...
		        if(!StringUtils.isEmpty(resMap.get("order_id")))
		        {
					...
					order.setHandleResultDesp("调用百度多酷影音充值接口成功");
					order.setChannelOrderId(resMap.get("order_id"));
					orderFormManager.update(order);
					if(PlatformSavingCenter.logger.isInfoEnabled())
						PlatformSavingCenter.logger.info("[百度多酷影音充值] [调用接口] [成功] [用户id:"+passport.getId()+"] [用户名:"+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [orderForm ID:"+order.getId()+"] [orderId:"+order.getOrderId()+"] [content:"+content+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"] [costs:"+(System.currentTimeMillis()-startTime)+"ms]");
				...
				else//下单不成功
				{
					...
					PlatformSavingCenter.logger.error("[百度多酷影音充值订单生成] [失败：返回调用失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [订单的ID:"+order.getId()+"] [订单id:"+order.getOrderId()+"] [content:"+content+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"] [res:"+resMap+"] [rescontent:"+content+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
					return "";
				}
			...
			{
				order.setHandleResult(OrderForm.HANDLE_RESULT_FAILED);
				order.setHandleResultDesp("调用百度多酷影音充值接口异常");
				try {
					orderFormManager.update(order);
				} catch (Exception e1) {
					PlatformSavingCenter.logger.error(e1);
				}
				PlatformSavingCenter.logger.error("[百度多酷影音充值] [调用接口] [失败] [用户id:"+passport.getId()+"] [用户名:"+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [orderForm ID:"+order.getId()+"] [orderId:"+order.getOrderId()+"] [content:"+content+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"] [costs:"+(System.currentTimeMillis()-startTime)+"ms]",e);
				return "";//infoMap.get(6010000);
			}
		}
		catch(Exception e)
		{
			PlatformSavingCenter.logger.error("[百度多酷影音充值] [调用接口] [失败:出现未知异常] [用户id:"+passport.getId()+"] [用户名:"+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"]",e);
			return "";
		}
	}
	...
	public String cardSaving(Passport passport, String cardtype, int payAmount, String cardno, String cardpass,
		...
		//设置是否通知游戏服状态 设为false
			...
			{
				PlatformSavingCenter.logger.error("[百度多酷影音充值订单生成] [失败] [更新订单信息] [失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [订单的ID:"+order.getId()+"] [角色id:"+others[0]+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
				return "";
			}
...
access_token值+client_id值+card_num值+ card_pwd 值+ card_type值+ pay_num 值+session_secret值(计算时无+号)
		...
		try {
			...
			PlatformSavingCenter.logger.error("[百度多酷影音充值订单生成] [调用接口失败] [失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [url:"+chargeUrl+"] [角色id:"+others[0]+"] [costs:"+(System.currentTimeMillis()-startTime)+"]",e);
			return "";
		}
		Map headers = new HashMap();
		String content = buildParams(params);
		if(PlatformSavingCenter.logger.isDebugEnabled())
		{
			PlatformSavingCenter.logger.debug("[百度多酷影音充值订单生成] [构造参数] [参数串:"+content+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"] [角色id:"+others[0]+"]");
		}
...
client_secret	orderid+ session_secret的MD5码（字母小写）	session_secre预先分配合作方
				...
				else//下单不成功
				{
					...
					PlatformSavingCenter.logger.error("[百度多酷影音充值订单生成] [失败：返回调用失败] [用户id:"+passport.getId()+"] [用户名："+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [订单的ID:"+order.getId()+"] [content:"+content+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"] [res:"+resMap+"] [rescontent:"+content+"] [角色id:"+others[0]+"] [costs:"+(System.currentTimeMillis()-startTime)+"]");
					return "";
				}
		...
		{
			PlatformSavingCenter.logger.error("[百度多酷影音充值] [调用接口] [失败:出现未知异常] [用户id:"+passport.getId()+"] [用户名:"+passport.getUserName()+"] [充值类型:"+cardtype+"] [充值金额:"+payAmount+"] [充值卡号:"+cardno+"] [充值卡密码:"+cardpass+"] [游戏服务器名称:"+server+"] [手机平台:"+mobileOs+"] [clientId:"+client_id+"] [gameid:"+game_id+"] [sessionScret:"+session_secret+"] [chargeUrl:"+chargeUrl+"] [角色id:"+others[0]+"]",e);
			return "";
		}
	}
	...
	public static String buildParams(Map<String,String> params)
	{
		...
		if(PlatformSavingCenter.logger.isDebugEnabled())
		{
			PlatformSavingCenter.logger.debug("[百度多酷影音充值] [生成参数串] [参数:"+sb.toString().substring(0, sb.length() - 1)+"] [session_secret:"+session_secret+"] [chargeUrl:"+chargeUrl+"]");
		}
	...
	public static String sign(LinkedHashMap<String,String> params){
		...
		if(PlatformSavingCenter.logger.isDebugEnabled())
		{
			PlatformSavingCenter.logger.debug("[百度多酷影音充值] [生成签名串] [签名前:"+md5Str+"] [签名后:"+sign+"] [session_secret:"+session_secret+"]");
		}
		
		return sign;
	}
			
	public static BAIDUYYSavingManager getInstance(){
		if(self == null)
		{
			self = new BAIDUYYSavingManager();
		}
		return self;
	}
