...
public class GameServerYeepaySavingManager {
	protected static GameServerYeepaySavingManager m_self = null;
    			
	private String yeepayReturnUrl = "http://116.213.192.216:9110/mieshi_game_boss/yeepayresult";
	private String szxUrl = "https://www.yeepay.com/app-merchant-proxy/command.action";
	private String merId = "10011836107";
	private String privateKey = "2u5ug6X67oH58gn3754c051np0yw7w8kU1Wird0aT39W0S5242P763ExN44i";
	...
	public String cardSaving(Passport passport, String cardtype, int mianzhi, String cardno, String cardpass, String otherDesp,
		...
		try {
			...
			PlatformSavingCenter.logger.warn("[自有易宝充值调用] [生成订单失败] [更新订单失败] [账号:"+passport.getUserName()+"] [orderId:"+order.getOrderId()+"] [username:"+passport.getUserName()+"] [channel:"+channel+"] [server:"+server+"] ["+(System.currentTimeMillis()-now)+"ms]");
		}
    	...
    	try {
    		...
    		for (int t = 0; t < responseStr.length; t++) {
    			...
    			if (i >= 0) {
    				...
    				if (sKey.equals("r0_Cmd")) {
    					...
    					PlatformSavingCenter.logger.error("[充值调用] [错误:调用返回的结果解析错误("+currentResult+")] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [url:"+yeepayUrl+"] \n" + result);
    				}
    			} else {
					PlatformSavingCenter.logger.error("[充值调用] [错误:调用返回的结果解析错误("+currentResult+")] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [url:"+yeepayUrl+"]\n" + result);
    			}
    		...
    		if (r1code != 1) {
				PlatformSavingCenter.logger.error("[充值调用] [错误:调用失败{errorCode:" + rs.getR1_Code() + ";errorMessage:"
	    		...
	    		if (!newHmac.equals(rs.getHmac())) {
					PlatformSavingCenter.logger.error("[充值调用] [错误:调用返回验证不合法] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [url:"+yeepayUrl+"]");
					order.setHandleResult(OrderForm.HANDLE_RESULT_FAILED);	
					willUpdateFieldNames.add("handleResult");
	    		} else {
	    			order.setHandleResult(OrderForm.HANDLE_RESULT_SUCC);
	    			willUpdateFieldNames.add("handleResult");
	    			PlatformSavingCenter.logger.info("[充值调用] [成功] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:"+rs.getR1_Code()+"/"+yeepayResultCodeDespMap.get(rs.getR1_Code())+"] [url:"+yeepayUrl+"]");
	    		}
    		}
    		...
    		e.printStackTrace();
    		PlatformSavingCenter.logger.error("[充值调用] [异常] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [url:"+yeepayUrl+"]", e);
    		order.setHandleResult(OrderForm.HANDLE_RESULT_FAILED);
    		willUpdateFieldNames.add("handleResult");
    	...
    	try {
    		...
    		e.printStackTrace();
    		PlatformSavingCenter.logger.error("[充值调用] [异常:更新订单异常] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [-----]", e);
    	}
    	return orderId;
    }
	...
	public String cardSaving(Passport passport, String cardtype, int mianzhi, String cardno, String cardpass, String otherDesp,
   	...
   	String cardinfo = mianzhi + "@" + cardno + "@" + cardpass;
   	if(badsaving) {
   		PlatformSavingCenter.logger.error("[充值调用] [失败:账号频繁提交坏的订单] [支付平台:易宝] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [用户:"+passport.getUserName()+"] [角色id:"+others[0]+"] ["+(System.currentTimeMillis()-now)+"ms]");
   		return null;
   	}
   	//先生成一个订单
   	java.util.Date cdate = new java.util.Date();
	...
	if(others.length > 2){
   	...
   	//调用扣费接口
		...
		try {
			...
			// TODO Auto-generated catch block
			PlatformSavingCenter.logger.warn("[自有易宝充值调用] [生成订单失败] [更新订单失败] [账号:"+passport.getUserName()+"] [orderId:"+order.getOrderId()+"] [username:"+passport.getUserName()+"] [channel:"+channel+"] [server:"+server+"] [角色id:"+others[0]+"] ["+(System.currentTimeMillis()-now)+"ms]");
		}
		HashMap<String, String> reqParams = new HashMap<String, String>();
		reqParams.put("p0_Cmd", "ChargeCardDirect");
		reqParams.put("p1_MerId", merId);
   	...
   	try {
   		...
   		for (int t = 0; t < responseStr.length; t++) {
   			...
   			if (i >= 0) {
   				...
   				if (sKey.equals("r0_Cmd")) {
   					...
   					PlatformSavingCenter.logger.error("[充值调用] [错误:调用返回的结果解析错误("+currentResult+")] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [角色id:"+others[0]+"] [url:"+yeepayUrl+"] \n" + result);
   				}
   			} else {
					PlatformSavingCenter.logger.error("[充值调用] [错误:调用返回的结果解析错误("+currentResult+")] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [角色id:"+others[0]+"] [url:"+yeepayUrl+"]\n" + result);
   			}
   		...
   		if (r1code != 1) {
   			...
   			PlatformSavingCenter.getInstance().addBadSaving(passport.getId());
	    		...
	    		// hmac不一致则抛出异常
	    		if (!newHmac.equals(rs.getHmac())) {
					PlatformSavingCenter.logger.error("[充值调用] [错误:调用返回验证不合法] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [url:"+yeepayUrl+"] [角色id:"+others[0]+"]");
					order.setHandleResult(OrderForm.HANDLE_RESULT_FAILED);	
					willUpdateFieldNames.add("handleResult");
	    		} else {
	    			order.setHandleResult(OrderForm.HANDLE_RESULT_SUCC);
	    			willUpdateFieldNames.add("handleResult");
	    			PlatformSavingCenter.logger.info("[充值调用] [成功] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:"+rs.getR1_Code()+"/"+yeepayResultCodeDespMap.get(rs.getR1_Code())+"] [url:"+yeepayUrl+"] [角色id:"+others[0]+"]");
	    		}
   		}
   		order.setHandleResultDesp(r1code + ":" + yeepayResultCodeDespMap.get(String.valueOf(r1code)));
   		willUpdateFieldNames.add("handleResultDesp");
   		...
   		e.printStackTrace();
   		PlatformSavingCenter.logger.error("[充值调用] [异常] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [url:"+yeepayUrl+"] [角色id:"+others[0]+"]", e);
   		order.setHandleResult(OrderForm.HANDLE_RESULT_FAILED);
   		willUpdateFieldNames.add("handleResult");
   		order.setHandleResultDesp(com.xuanzhi.language.translate.text_1576 + e.getMessage());
   		willUpdateFieldNames.add("handleResultDesp");
   	...
   	try {
   		...
   		e.printStackTrace();
   		PlatformSavingCenter.logger.error("[充值调用] [异常:更新订单异常] [支付平台:易宝] [介质:"+cardtype+"] [介质信息:"+cardinfo+"] [面值:"+mianzhi+"] [订单:"+order.getOrderId()+"] [用户:"+passport.getUserName()+"] [结果:-----] [-----] [角色id:"+others[0]+"]", e);
   	}
   	return orderId;
   }
    ...
    public static GameServerYeepaySavingManager getInstance() {
		return m_self;
	}
