...
public class GameServerYeepaySavingBackServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse res)
		...
		String privateKey = GameServerYeepaySavingManager.getInstance().getPrivateKey();
		...
		if (!hmac.equals(newHmac)) {
			PlatformSavingCenter.logger.info("[充值回调] [支付平台:易宝] [MD5验证出错] [介质:----] [用户:----] [游戏:----] [支付结果:"+(payResult==1?"成功":"失败")+"] [订单:"+orderId+"] [充值额:"+payMoney+"分]");
		} else {
			...
			if(order == null) {
				PlatformSavingCenter.logger.info("[充值回调] [支付平台:易宝] [订单不存在] [介质:----] [用户:----] [游戏:----] [支付结果:"+(payResult==1?"成功":"失败")+"] [订单:"+p2_Order+"] [充值额:"+payMoney+"分]");
			...
			if(order != null) {
				...
				if(order.getResponseResult() != OrderForm.RESPONSE_NOBACK) {
					PlatformSavingCenter.logger.info("[充值回调] [支付平台:易宝] [错误的返回:此订单已经返回过了] [介质:] [用户:"+passport.getUserName()+"] [支付结果:"+r1_Code+"] [订单金额:"+order.getPayMoney()+"分] [订单:"+p2_Order+"] [充值额:"+payMoney+"分]");
					res.getWriter().write("success");
					return;
				}
				//必须在完成更新订单后才能进行兑换操作，否则可能造成重复的兑换
				String resultdesp = p8_cardStatus + ":" + GameServerYeepaySavingManager.yeepayPayStatusDespMap.get(p8_cardStatus);
				try {
					...
					PlatformSavingCenter.logger.error("[充值回调] [支付平台:易宝] [更新订单发生异常,请手动进行兑换并更新订单] [介质:] [用户:"+passport.getUserName()+"] [支付结果:"+(payResult==1?"成功":"失败")+"] [订单:"+orderId+"] [订单金额:"+order.getPayMoney()+"分] [充值额:"+payMoney+"分]", e);
					return;
				}
		    	PlatformSavingCenter.logger.info("[充值回调] [支付平台:易宝] [介质:] [用户:"+passport.getUserName()+"] [支付结果:"+(payResult==1?"成功":"失败")+"] ["+resultdesp+"] [订单:"+orderId+"] [订单金额:"+order.getPayMoney()+"分] [充值额:"+payMoney+"分]");
			} else {
				PlatformSavingCenter.logger.info("[充值回调] [支付平台:易宝] [订单找不到] [支付平台:----] [介质:----] [用户:----] [游戏:----] [支付结果:"+(payResult==1?"成功":"失败")+"] [订单:"+orderId+"] [订单金额:"+order.getPayMoney()+"分] [充值额:"+payMoney+"分]");
			}
		}
		res.getWriter().write("success");
	}
