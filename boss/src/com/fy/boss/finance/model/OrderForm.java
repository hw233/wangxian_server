 ...
 */
@SimpleEntity(name="OrderForm")
@SimpleIndices({
	@SimpleIndex(members={"orderId"}),
	@SimpleIndex(members={"passportId"}),
...
public class OrderForm implements java.io.Serializable {
	...
	public String getStatusDesp() {
		if(this.isNotifySucc()) {
			...
			return "充值成功，同步游戏服失败";
		} else if(this.getResponseResult() == OrderForm.RESPONSE_SUCC) {
			return "充值成功，还未同步游戏服";
		} else if(this.getResponseResult() == OrderForm.RESPONSE_FAILED) {
			return "充值失败，原因:" + this.getResponseDesp();
		} else if(this.getHandleResult() == OrderForm.HANDLE_RESULT_SUCC) {
			return "充值已提交，未支付或还未返回";
		} else if(this.getHandleResult() == OrderForm.HANDLE_RESULT_NOBACK) {
			return "订单已生成，等待支付";
		} else {
	...
	public String getLogStr() {
		return "[id:"+id+"] [userChannel:"+(userChannel!=null?userChannel:"")+"] [orderId:"+orderId+"] [passport:"+passportId+"] [money:"+payMoney+"] [userChannel:"+userChannel+"] [platform:"+savingPlatform+"] " +
				...
				"[responseResult:"+responseResult+"] [notified:"+notified+"] [notifySucc:"+notifySucc+"] [chargeValue:"+chargeValue+"] [chargeType:"+chargeType+"]";
	}
