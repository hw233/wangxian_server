...
public class ExceptionOrderFormDAOImpl implements ExceptionOrderFormDAO {

	private static final Log logger = LogFactory.getLog(ExceptionOrderFormDAOImpl.class);
	
	// property constants
	public static final Class pojoClass = ExceptionOrderForm.class;
	
	private SimpleEntityManager<ExceptionOrderForm> em = SimpleEntityManagerFactory.getSimpleEntityManager(pojoClass);
	
	protected static ExceptionOrderFormDAOImpl self;
	
	public static ExceptionOrderFormDAOImpl getInstance() {
		if (self == null) {
			synchronized (ExceptionOrderFormDAOImpl.class) {
				if (self == null)
					self = new ExceptionOrderFormDAOImpl();
			}
		}
		return self;
	}
	...
	@Override
	public void saveNew(ExceptionOrderForm order) {
	...
	@Override
	public ExceptionOrderForm getById(long id) {
		long startTime = System.currentTimeMillis();
		try {
			ExceptionOrderForm order = em.find(id);
			if(order != null)
			{
				if(logger.isInfoEnabled())
	...
	@Override
	public ExceptionOrderForm getByOrderid(String orderId) {
		long startTime = System.currentTimeMillis();
		try {
			long ids[] = em.queryIds(ExceptionOrderForm.class, " orderId = '" +orderId+"'");
			ExceptionOrderForm order = em.find(ids[0]);
			if(order != null)
			{
				if(logger.isInfoEnabled())
	...
	public long getCount(long passportId) {
		...
		try {
			long count = 0l;
			count = em.count(ExceptionOrderForm.class, " passportId="+passportId );
			if(logger.isInfoEnabled())
	...
	public List getUserSuccPageRows(long passportId, int start, int len) {
		...
		try {
			//不包含end
			start = start + 1;
			List<ExceptionOrderForm> lst = em.query(ExceptionOrderForm.class, " responseResult = "+ExceptionOrderForm.RESPONSE_SUCC+" ", "responseTime desc", start, start + len);
			if(logger.isInfoEnabled())
	...
	public List getUserUnbackedPageRows(long passportId) {
		long startTime = System.currentTimeMillis();
		try {
			long []ids = em.queryIds(ExceptionOrderForm.class, " handleResult = "+ExceptionOrderForm.HANDLE_RESULT_NOBACK+" ");
			if(logger.isInfoEnabled())
			{
				logger.info("[获得某个账户没有从充值平台返回的订单] [成功] [查询id] [passportId:"+passportId+"] [数量:"+ids.length+"] [costs:"+(System.currentTimeMillis()-startTime)+"ms]");
			}
			List<ExceptionOrderForm> lst = new ArrayList<ExceptionOrderForm>();
	...
	public List getUserPageRows(long passportId, int start, int len) {
		...
		start = start + 1;
		try {
			List<ExceptionOrderForm> lst = em.query(ExceptionOrderForm.class, " passportId = "+passportId, "createTime desc", start, start + len );
			if(logger.isInfoEnabled())
	...
	public List getPageRows(int start, int len) {
		...
		start += 1;
		try {
			List<ExceptionOrderForm> lst = em.query(ExceptionOrderForm.class, "" ,"createTime desc", start, start + len);
			if(logger.isInfoEnabled())
	...
	public List getSuccUnnotifiedOrderForms() {
		long startTime = System.currentTimeMillis();
		try {
			long []ids = em.queryIds(ExceptionOrderForm.class, " handleResult = "+ExceptionOrderForm.HANDLE_RESULT_SUCC+" and responseResult = "+ExceptionOrderForm.RESPONSE_SUCC+" and notified = 'F' ");
			if(logger.isInfoEnabled())
			{
	...
	public long getFailedOrderFormsCount() {
		long startTime = System.currentTimeMillis();
		try {
			long count = em.count(ExceptionOrderForm.class, " handleResult = "+ExceptionOrderForm.HANDLE_RESULT_FAILED+" or responseResult = "+ExceptionOrderForm.RESPONSE_FAILED+" or (notified = 'T' and notifySucc = 'F')");
			if(logger.isInfoEnabled())
			{
	...
	public List getFailedOrderForms(int start, int len) {
		long startTime = System.currentTimeMillis();
		try {
			long []ids = em.queryIds(ExceptionOrderForm.class, " handleResult = "+ExceptionOrderForm.HANDLE_RESULT_FAILED+" or responseResult = "+ExceptionOrderForm.RESPONSE_FAILED+" or (notified = 'T' and notifySucc = 'F')",  "createTime DESC", start+1, start+1+len);
			if(logger.isInfoEnabled())
			{
	...
	@Override
	public void update(ExceptionOrderForm orderForm,String fieldName) {
		long startTime = System.currentTimeMillis();
		try {
	...
	@Override
	public void delete(ExceptionOrderForm orderForm) {
	 ...
	 */
	public void batch_updateField(ExceptionOrderForm orderForm,List<String> fieldNames) throws Exception
	{
		long startTime = System.currentTimeMillis();
		if(fieldNames == null)
		{
	...
	public SimpleEntityManager<ExceptionOrderForm> getEm() {
		return em;
	}

	public void setEm(SimpleEntityManager<ExceptionOrderForm> em) {
		this.em = em;
	}
	...
	public long count(String whereSql) throws Exception {
		...
		try {
			long count = 0l;
			count = em.count(ExceptionOrderForm.class,  whereSql );
			if(logger.isInfoEnabled())
	...
	public List<ExceptionOrderForm> queryForWhere(String whereSql) throws Exception
	{
		long startTime = System.currentTimeMillis();
		try {
			long []ids = em.queryIds(ExceptionOrderForm.class, whereSql);
			if(logger.isInfoEnabled())
			{
	...
	public void update(ExceptionOrderForm order) throws Exception
	{
		long startTime = System.currentTimeMillis();
		try {
			em.flush(order);
	...
	@Override
	public ExceptionOrderForm getByChannelOrderid(String channelOrderId,String channel) {
		long startTime = System.currentTimeMillis();
		try {
			long ids[] = em.queryIds(ExceptionOrderForm.class, " channelOrderId = '" +channelOrderId+"' and userChannel = '" +channel+ "'");
			ExceptionOrderForm order = em.find(ids[0]);
			if(order != null)
			{
				if(logger.isInfoEnabled())
					logger.info("[根据channelOrderId查询订单实体] [成功] [id:"+order.getId()+"] [渠道:"+channel+"] [Channel orderId:"+order.getChannelOrderId()+"] [costs:"+(System.currentTimeMillis()-startTime)+"ms]");
			}
			else
			{
				logger.warn("[根据channelOrderId查询订单实体] [成功] [无对应此channelOrderId对象] [渠道:"+channel+"] [id:"+order.getId()+"] [Channel orderId:"+order.getChannelOrderId()+"] [costs:"+(System.currentTimeMillis()-startTime)+"ms]");
			}
