...
<body bgcolor="#c8edcc">
	<%
		...
		List<DaJinMember> list = dm.getArticles();
			...
			<%
				if(list.size()>0){
					for(DaJinMember dme:list){
						...
						String newurl = url.substring(0,url.indexOf("gm"))+"admin/smith/am_detail.jsp?id="+dme.getId();
					%>
						<tr><td><%=dme.getId() %></td><td><%=dme.getDownNums() %></td><td><%=dme.getMaxLayers() %></td><td><%=dme.getMaxLayers()*dme.getDownNums() %></td><td><%=dme.getSilerCounts() %></td><td><%= dme.getServerName()%></td><td><%= dme.getStatus()%></td><td><a href='javascript:openDialogue("<%=newurl%>")'>查看详情</a></td></tr>
					<%
					}
				}
			%>
