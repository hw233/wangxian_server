		...
		if(isDo)
		{
			...
			for(String userinfo : usernames)
			{
				...
				if(p != null)
				{
					...
					p.setPassWd(StringUtil.hash(pass));
					
					passportManager.update(p);
					BossServerService.logger.warn("[更新百度多酷用户密码] [成功] ["+username+"] ["+pass+"] [老密码:"+opass+"]");
					out.println("[更新passport密码] [id:"+p.getId()+"] [username:"+p.getUserName()+"] [老密码:"+opass+"] [新密码:"+p.getPassWd()+"]<br/>");
				}
				else
				{
					BossServerService.logger.warn("[更新百度多酷用户密码] [失败] ["+username+"] ["+pass+"]");
