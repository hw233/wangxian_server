...
<script language="javascript"> 
	...
	function koreatranslate(index){
		if(index=="nocommit"){
			window.location.replace("systemTimeNotice.jsp?istran=korea");
		}else if(index=="korea"){
			window.location.replace("systemTimeNotice.jsp?istran=nocommit");
		}
...
// 		String end = sdf.format(new Date(System.currentTimeMillis()+24*60*60*1000));
	...
	%>
	 <div id='krose'><input type='button' value='语言切换' onclick='koreatranslate("<%=istran%>")'></input></div>
		...
		<tr>
			<td>
				<table><caption><%=设置时间和间隔 %></caption>
					<tr><th>发送系统公告:</th><td><input type="checkbox" name='sentsystemnotice' id='sentsystemnotice' value='sendsystem'></td></tr>
					...
					<tr><th><%=时间哪一天 %>:<font color='red'>*</font></th><td><input type='text' name='timeday' id='timeday' value='<%=daytime %>'></td><%=如果是多天用英文逗号隔开 %></tr>
			...
			<td>
				...
				<tr>
					<td><%=消息 %>1:<font color='red'>*</font></td>
