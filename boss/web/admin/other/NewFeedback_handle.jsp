...
// 						  alert(result);
			...
			self.setInterval("newmess('"+isshow+"','"+gmnum+"')",2000);
		}
	}
...
//					  complete:function(result){alert("completed"+result)},
				  dataType: "html",
// 					  error:function(result){alert("服务器重启，请您耐心等待或询问相关人员");},
					  success: function(result)
					  {
						  if(result!=null){
	...
	function send(id,servername,playername,gmnum,url,sendtime){
		alert("id:"+id+"--servername:"+servername+"--playername:"+playername+"--gmnum:"+gmnum+"--url:"+url+"--sendtime:"+sendtime);
		var myDate = new Date();
...
// 					  error:function(result){alert("服务器重启，请您耐心等待或询问相关人员");},
	...
	function lookPlayerMess(servername,username,playername,viplevel,qudao){
		if(username){
			var servernames = "昆侖仙境,飄渺王城,千年古城,華山之巔,雪域冰城,天降神兵,斬龍屠魔,皇圖霸業,人間仙境,伏虎沖天,無相如來,仙侶奇緣,仙人指路,瓊漿玉露,道骨仙風,仙姿玉貌,步雲登仙,蜀山之道,禦劍伏魔,縱橫九州,傲視遮天,神魔仙界,裂天之刃,氣衝霄漢,煙暖初妝";
			if(servername=="太虚幻境"||servername=="幽冥山谷"||servername=="昆仑圣殿"||servername=="凌霄宝殿"||servername=="霸气乾坤"||servername=="烟雨青山"||servername=="仙山琼阁"||servername=="霸气无双"||servername=="华山之巅"||servername=="神龙摆尾"||servername=="柳暗花明"){
				window.open("http://117.135.128.177:8801/game_gateway/admin/gm/account.jsp?&username="+username,'newwindow','width=1000,height=460,toolbar=no,menubar=no,scrollbars=yes, resizable=no,location=no, status=no,z-look=no');
			}else if(servernames.indexOf(servername)>-1){
				window.open("http://211.72.246.23:8882/game_gateway/admin/gm/account.jsp?&username="+username,'newwindow','width=1000,height=460,toolbar=no,menubar=no,scrollbars=yes, resizable=no,location=no, status=no,z-look=no');
			}else{
				window.open("http://116.213.192.216:8882/game_gateway/admin/gm/account.jsp?&username="+username,'newwindow','width=1000,height=460,toolbar=no,menubar=no,scrollbars=yes, resizable=no,location=no, status=no,z-look=no');
			}
		}else{
			alert("没有信息可以查询！");
		}
							...
							if(isgetmess!=null && isgetmess.trim().equals("true")){
								if(nfq.getVipqueue().size()>0){
									...
									if(nf!=null){
										...
										//
// 										nf.setGmstat(0);
// 										NewFeedbackQueueManager.getInstance().sem.notifyFieldChange(nf, "gmstat");
								...
								%>
									<a onclick="getMess('<%=handler%>')"><font size="6" color='red'><B>领取VIP问题</B></font></a>
								<%
							}
							%>
